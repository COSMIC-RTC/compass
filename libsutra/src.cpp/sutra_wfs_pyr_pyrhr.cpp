...
sutra_wfs_pyr_pyrhr::sutra_wfs_pyr_pyrhr(carma_context *context, sutra_telescope *d_tel, sutra_sensors *sensors, long nxsub,
...
int sutra_wfs_pyr_pyrhr::comp_generic() {
  ...
  */
  ...
  for (int cpt = 0; cpt < this->npup; cpt++) {
    //int cpt = 0;
    carmaSafeCall(cudaMemset(this->d_camplipup->getData(), 0,
    ...
    */
    ...
    abs2(this->d_hrimg->getData(), this->d_fttotim->getData(), this->nfft*this->nfft, fact,
         this->current_context->get_device(device));
  ...
  pyr_submask(this->d_fttotim->getData(), this->d_sincar->getData(),
	      this->nfft, this->current_context->get_device(device));
  
  carma_fft(this->d_fttotim->getData(), this->d_fttotim->getData(), 1,
	    *this->d_camplipup->getPlan());

  cgetrealp(this->d_hrimg->getData(), this->d_fttotim->getData(),this->d_hrimg->getNbElem(), 
	    this->current_context->get_device(device));
...
//      blocks, threads);
  ...
  pyr_fact(this->d_binimg->getData(), this->nphot, this->d_subsum->getData(),
      this->nfft / this->nrebin, 1, this->current_context->get_device(device));
