...
sutra_wfs_pyr_pyrhr::sutra_wfs_pyr_pyrhr(carma_context *context,
    sutra_telescope *d_tel, sutra_sensors *sensors, long nxsub, long nvalid,
...
sutra_wfs_pyr_pyrhr::sutra_wfs_pyr_pyrhr(carma_context *context,
    sutra_telescope *d_tel, sutra_sensors *sensors, long nxsub, long nvalid,
    long npix, long nphase, long nrebin, long nfft, long ntot, long npup,
    float pdiam, float nphotons, float nphot4imat, int lgs, int nbdevices,
    int* devices) :
  sutra_wfs_pyr(context, d_tel, sensors, nxsub, nvalid, npix, nphase,
                nrebin, nfft, ntot, npup, pdiam, nphotons, nphot4imat, lgs,
                devices[0], "pyrhr") {
...
sutra_wfs_pyr_pyrhr::~sutra_wfs_pyr_pyrhr() {
  ...
  for (std::vector<carma_obj<cuFloatComplex> *>::iterator it =
         this->d_fttotim_ngpu.begin(); this->d_fttotim_ngpu.end() != it;
       ++it) {
...
void sutra_wfs_pyr_pyrhr::comp_modulation(int cpt) {
  ...
  if (ngpu < 2) {
    ...
    carmaSafeCall(
      cudaMemset(this->d_camplipup_ngpu[cur_device]->getData(), 0,
                 2 * sizeof(float)
    ...
    abs2(this->d_hrimg_ngpu[cur_device]->getData(),
         this->d_fttotim_ngpu[cur_device]->getData(),
         this->nfft * this->nfft, fact,
         this->current_context->get_device(cur_device));
...
int sutra_wfs_pyr_pyrhr::comp_generic() {
  ...
  dims_data2[1] = nfft;
  dims_data2[2] = nfft;
  carma_obj<float> *tmp_vector = new carma_obj<float>(current_context,
      dims_data2);

  for (std::vector<carma_obj<float> *>::iterator it =
         this->d_hrimg_ngpu.begin(); this->d_hrimg_ngpu.end() != it; ++it) {
    if (*it != d_hrimg) {
      if (current_context->canP2P(d_hrimg->getDevice(),
                                  (*it)->getDevice())) {
