...
sutra_controller_mv::sutra_controller_mv(carma_context *context, long nvalid,
    long nactu, long delay) : sutra_controller(context, nvalid*2, nactu) {
...
int sutra_controller_mv::build_cmat(const char *dmtype){
	...
	// carma_obj<float> *d_tmp4;
	  ...
	  carma_potri(d_tmp2);
	  carma_gemm(cublas_handle,'n','t',nactu,nslope,nactu,one,d_tmp2->getData(),nactu,d_imat->getData(),nslope,zero,d_cmat->getData(),nactu);
...
int sutra_controller_mv::comp_com() {
	...
	// POLC equations
	carma_geam<float>(cublas_handle,'n','n',nactu,1,(float)delay,*d_com2,nactu,1.0f-delay,*d_com1,nactu,d_tmp,nactu);
	//carma_gemv<float>(cublas_handle,'n',nslope,nactu,-1.0f,*d_imat,nslope,d_tmp,1,1.0f,*d_centroids,1);
	carma_gemv<float>(cublas_handle,'n',nslope,nactu,1.0f,*d_imat,nslope,d_tmp,1,0.0f,d_tmp2,1);
	carma_geam<float>(cublas_handle,'n','n',nslope,1,1.0f,*d_centroids,nslope,-1.0f,d_tmp2,nslope,d_olmeas,nslope);
     ...
     //         1, &beta, &((this->d_err->getData())[istart2]), 1));
      carma_checkCublasStatus(
          cublasSgemv(cublas_handle, trans, this->d_cmat->getDims(1) / nstreams,
              this->d_cmat->getDims(2), &alpha,
              &((this->d_cmat->getData())[istart1]),
              this->d_cmat->getDims(1) / nstreams, d_olmeas,
              1, &beta, &((this->d_err->getData())[istart2]), 1));
    }
  ...
  //      this->d_centroids, 1, 0.0f, 1);
   ...
   // this->d_com->copy(this->d_err,1,1);
	...
	mult_int(d_tmp4, this->d_err->getData(),
	        this->d_gain->getData(),this->gain, this->nactu, this->device);
	mult_int(d_tmp3, this->d_com1->getData(),
		        this->d_gain->getData(), 1-(this->gain), this->nactu, this->device);
	carma_geam<float>(cublas_handle,'n','n',nactu,1,1.0f,d_tmp4,nactu,1.0f,d_tmp3,nactu,*d_com,nactu);
