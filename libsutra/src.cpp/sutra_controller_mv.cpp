...
SutraControllerMv<Tcomp, Tout>::SutraControllerMv(
    CarmaContext *context, int64_t nslope_, int64_t nactu_, float delay,
...
template <typename Tcomp, typename Tout>
string SutraControllerMv<Tcomp, Tout>::get_type() {
  return "mv";
}

template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::set_modal_gains(Tcomp *mgain) {
  this->current_context->set_active_device(this->device, 1);
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::set_cmat(Tcomp *cmat) {
  this->current_context->set_active_device(this->device, 1);
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::set_imat(Tcomp *imat) {
  this->current_context->set_active_device(this->device, 1);
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::load_noisemat(Tcomp *noise) {
  this->current_context->set_active_device(this->device, 1);
...
// Florian features
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::compute_Cmm(
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::compute_Cphim(
    SutraAtmos *atmos, SutraSensors *sensors, SutraDms *dms, double *L0,
    double *cn2, double *alphaX, double *alphaY, double *X, double *Y,
    double *xactu, double *yactu, double diamTel, double *k2, int64_t *NlayerDm,
    int64_t *indLayerDm, double FoV, double *pitch, double *alt_dm) {
  this->current_context->set_active_device(this->device, 1);
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::filter_cphim(Tcomp *F, Tcomp *Nact) {
  // Piston filter
  piston_filt_cphim(this->d_Cphim, F);
  // Init and inverse the coupling matrix
  int64_t dims_data2[3] = {2, this->d_Cphim->get_dims(1),
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::do_covmat(SutraDm *ydm, char *method,
                                                int32_t *indx_pup, int64_t dim,
                                                Tcomp *xpos, Tcomp *ypos,
                                                int64_t Nkl, Tcomp norm,
                                                Tcomp ampli) {
  this->current_context->set_active_device(this->device, 1);
  int64_t dims_data[3];
  dims_data[0] = 2;
  ...
  dims_data2[1] = dim;
  CarmaObj<int32_t> d_indx(this->current_context, dims_data2);
  ...
  carma_syevd<Tcomp>(SOLVER_EIG_MODE_NOVECTOR, &d_statcov, &d_eigenvals);
  ...
  if (ydm->type == "kl") {
    ...
    if (strcmp(method, "inv") == 0) {
      for (int32_t i = 0; i < this->nactu(); i++) {
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::do_geomat(CarmaObj<Tcomp> *d_geocov,
                                                CarmaObj<Tcomp> *d_IF,
                                                int64_t n_pts, Tcomp ampli) {
  this->current_context->set_active_device(this->device, 1);
  carma_gemm<Tcomp>(cublas_handle, 't', 'n', this->nactu(), this->nactu(),
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::piston_filt(CarmaObj<Tcomp> *d_statcov) {
  this->current_context->set_active_device(this->device, 1);
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::piston_filt_cphim(
    CarmaObj<Tcomp> *d_cphim, Tcomp *F) {
  this->current_context->set_active_device(this->device, 1);

  int64_t Nmod = d_cphim->get_dims(1);
  int64_t dims_data[3];
  dims_data[0] = 2;
  dims_data[1] = Nmod;
  dims_data[2] = Nmod;
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::invgen(CarmaObj<Tcomp> *d_mat, Tcomp cond,
                                             int32_t job) {
  this->current_context->set_active_device(this->device, 1);
  const int64_t dims_data[3] = {2, d_mat->get_dims(1), d_mat->get_dims(2)};
  CarmaObj<Tcomp> d_U(this->current_context, dims_data);
  CarmaObj<Tcomp> d_tmp(this->current_context, dims_data);
  int32_t i;

  const int64_t dims_data2[2] = {1, d_mat->get_dims(1)};
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::invgen(CarmaObj<Tcomp> *d_mat,
  ...
  int32_t cpt = 0;
  for (int32_t i = 0; i < d_mat->get_dims(1); i++) {
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::invgen_cpu(CarmaObj<Tcomp> *d_mat,
                                                 CarmaHostObj<Tcomp> *h_eigen,
                                                 Tcomp cond) {
  this->current_context->set_active_device(this->device, 1);
  const int64_t dims_data[3] = {2, d_mat->get_dims(1), d_mat->get_dims(2)};
...
/*
template<typename T>
 int32_t SutraControllerMv<Tcomp, Tout>::do_statmat(Tcomp *statcov, Tcomp *xpos,
Tcomp *ypos){ int32_t dim_x = sizeof(xpos)/sizeof(xpos[0]); int32_t ind; for (i=0 ;
 ...
 */
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::DDiago(CarmaObj<Tcomp> *d_statcov,
                                             CarmaObj<Tcomp> *d_geocov) {
  this->current_context->set_active_device(this->device, 1);
  const int64_t dims_data[3] = {2, this->nactu(), this->nactu()};
  CarmaObj<Tcomp> d_M1(this->current_context, dims_data);
  CarmaObj<Tcomp> d_tmp(this->current_context, dims_data);
  CarmaObj<Tcomp> d_tmp2(this->current_context, dims_data);

  const int64_t dims_data2[2] = {1, this->nactu()};
  CarmaObj<Tcomp> d_eigenvals(this->current_context, dims_data2);
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::load_covmat(Tcomp *covmat) {
  this->current_context->set_active_device(this->device, 1);
  this->d_covmat->host2device(covmat);
  return EXIT_SUCCESS;
}
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::load_klbasis(Tcomp *klbasis) {
  this->current_context->set_active_device(this->device, 1);
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::filter_cmat(Tcomp cond) {
  this->current_context->set_active_device(this->device, 1);

  if (this->h_eigenvals != 0L) delete this->h_eigenvals;
  int64_t Nactu = this->d_Cphim->get_dims(1);
  if (Nactu < this->nactu()) {
    int64_t *dims_data = new int64_t[3];
    dims_data[0] = 2;
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::build_cmat(const char *dmtype,
                                                 char *method) {
  Tcomp one = 1.;
  Tcomp zero = 0.;
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::frame_delay() {
  ...
  if ((int32_t)this->delay > 0) {
    ...
    carma_safe_call(
        cudaMemcpy(&(this->d_cenbuff[(int32_t)this->delay * this->nslope()]),
...
template <typename Tcomp, typename Tout>
int32_t SutraControllerMv<Tcomp, Tout>::comp_com() {
  this->current_context->set_active_device(this->device, 1);
...
template class SutraControllerMv<float, float>;
template class SutraControllerMv<float, uint16_t>;
