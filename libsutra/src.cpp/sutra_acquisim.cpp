...
int sutra_acquisim::comp_image(long *dims, float *bimage, carma_obj<float> *d_bincube) {
  this->current_context->set_activeDevice(this->device, 1);
  carma_obj<float> tmp_yObj(this->current_context, dims, bimage);
  return fillbincube<float>(tmp_yObj, *d_bincube, this->npix,
                            this->nvalid, this->npix * this->nxsub,
                            *(this->d_validsubsx), *(this->d_validsubsy),
