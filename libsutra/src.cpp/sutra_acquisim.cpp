...
int sutra_acquisim::comp_image(long *dims, float *bimage) {
	...
	return fillbincube<float>(tmp_yObj, *(wfs->d_bincube), wfs->npix,
			wfs->nvalid, wfs->npix * wfs->nxsub, *(wfs->d_validsubsx),
			*(wfs->d_validsubsy), wfs->device);
}
...
int sutra_acquisim::comp_image_tele(long *dims, float *bimage) {
	...
	return fillbincube_async<float>(wfs->image_telemetry, tmp_yObj,
			...
			*(wfs->d_validsubsx), *(wfs->d_validsubsy),
			wfs->d_binimg->getNbElem(), wfs->device);
}
