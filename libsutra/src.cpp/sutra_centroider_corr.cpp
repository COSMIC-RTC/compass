...
int sutra_centroider_corr::init_corr(sutra_wfs *wfs, int isizex, int isizey,
	...
	this->d_corrfnct = new carma_obj<cuFloatComplex>(current_context,
			dims_data3);
	this->d_corrspot = new carma_obj<cuFloatComplex>(current_context,
			dims_data3);
...
int sutra_centroider_corr::get_cog(sutra_wfs *wfs, float *slopes) {
	...
	corr_norm(*(this->d_corr), *(this->d_corrnorm),
	...
	subap_pinterp<float>(this->interp_sizex * this->interp_sizey, this->nvalid,
			*(this->d_corr), *(this->d_corrmax), slopes, *(this->d_interpmat),
			this->interp_sizex, this->interp_sizey, this->nvalid,
			2 * this->npix - 1, this->scale, this->offset);
	return EXIT_SUCCESS;
}
