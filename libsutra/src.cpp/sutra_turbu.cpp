...
int sutra_tscreen::generate_vk(float l0, int nalias) {
	...
	gene_vonkarman(data, data_o, k0, nalias, this->screen_size,
			this->screen_size, block_size);

	roll(data, this->screen_size, this->screen_size);

	carma_fft(data, data, 1, *this->d_tscreen_c->getPlan());

	cgetrealp(this->d_tscreen->d_screen->getData(),
			this->d_tscreen_c->getData(),
			this->d_tscreen->d_screen->getNbElem(), this->device);

	norm_pscreen(data_o, this->d_tscreen->d_screen->getData(),
			this->screen_size, this->screen_size, this->norm_vk, this->device);
...
// dir =1 moving in x
	...
	this->d_ytmp->gemv('n', 1.0f, this->d_A, this->d_A->getDims(1), this->d_z,
			1, 0.0f, 1);
	...
	addai(this->d_ytmp->getData(), this->d_tscreen->d_screen->getData(), x0,
			1.0f, this->d_ytmp->getNbElem(), this->device);
	...
	fillarr2d(this->d_tscreen->d_screen->getData(),
			this->d_tscreen_o->getData(), x0, Ncol, NC, N, this->device);
...
sutra_atmos::sutra_atmos(carma_context *context, int nscreens, float *r0,
		...
		float *winddir, float *deltax, float *deltay, float *pupil,
		int device) {
	...
	for (int i = 0; i < nscreens; i++) {
		d_screens.insert(
				pair<float, sutra_tscreen *>(altitude[i],
						new sutra_tscreen(context, size[i], size2[i], r0[i],
