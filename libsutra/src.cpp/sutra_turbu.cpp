...
sutra_tscreen::sutra_tscreen(carma_context *context, long size, long size2, float r0, float altitude, float windspeed, float winddir,float deltax, float deltay, int device)
{
...
int sutra_tscreen::init_screen(float *h_A, float *h_B, unsigned int *h_istencilx, unsigned int *h_istencily, int seed)
{
 // initial memcopies
...
int sutra_tscreen::init_vk(int seed, int pupd)
{
  long *dims_data2 = new long[3];
  dims_data2[0] = 2; dims_data2[1] = this->screen_size; dims_data2[2] = this->screen_size; 
  this->d_tscreen_c = new carma_obj<cuFloatComplex>(current_context, dims_data2);
...
sutra_atmos::sutra_atmos(carma_context *context, int nscreens,float *r0,long *size,long *size2, float *altitude, float *windspeed, float *winddir, float *deltax, float *deltay, float *pupil, int device)
{
...
{
  for ( map<float,sutra_tscreen *>::iterator it = d_screens.begin(); it != d_screens.end(); ++it )
    {
      delete it->second;
      it->second = 0;
    }  
...
int sutra_atmos::init_screen(float altitude,float *h_A, float *h_B, unsigned int *h_istencilx,unsigned int *h_istencily, int seed) {
