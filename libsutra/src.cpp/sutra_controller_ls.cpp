...
SutraControllerLs<T, Tout>::SutraControllerLs(CarmaContext *context,
                                                  int64_t nslope, int64_t nactu, float delay,
...
template <typename T, typename Tout>
SutraControllerLs<T, Tout>::~SutraControllerLs() {
...
template <typename T, typename Tout>
string SutraControllerLs<T, Tout>::get_type() {
  return "ls";
}

template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::svdec_imat() {
  // doing U = Dt.D where D is i_mat
  T one = 1., zero = 0.;
  int32_t nCols = this->d_imat->get_dims(2);
  ...
  if (d_U->get_dims(1) !=
    ...
    this->d_U = new CarmaObj<T>(this->current_context,
                                 std::vector<int64_t>{2, nCols, nCols}.data());
    delete d_eigenvals;
    delete h_eigenvals;
    this->d_eigenvals = new CarmaObj<T>(this->current_context,
                                         std::vector<int64_t>{1, nCols}.data());
    this->h_eigenvals =
        new CarmaHostObj<T>(std::vector<int64_t>{1, nCols}.data(), MA_PAGELOCK);
  }
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::set_cmat(T *cmat) {
  this->current_context->set_active_device(this->device, 1);
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::set_imat(T *imat) {
  this->current_context->set_active_device(this->device, 1);
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::set_modal_gains(T *mgain) {
  this->current_context->set_active_device(this->device, 1);
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::build_cmat(int32_t nfilt, bool filt_tt) {
  this->current_context->set_active_device(this->device, 1);
  ...
  int32_t nb_elem = this->h_eigenvals->get_nb_elements();
  ...
  if (filt_tt)
    nb_elem -= 2;
  */
  for (int32_t cc = nfilt; cc < nb_elem; cc++) {
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::build_cmat(int32_t nfilt) {
  this->current_context->set_active_device(this->device, 1);
  return this->build_cmat(nfilt, false);
}

template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::frame_delay() {
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::comp_com() {
  this->current_context->set_active_device(this->device, 1);
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::build_cmat_modopti() {
  this->current_context->set_active_device(this->device, 1);
  int64_t dims_data2[3] = {2, this->nactu(), this->nmodes};
  CarmaObj<T> d_tmp(this->current_context, dims_data2);
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::init_modalOpti(int32_t nmodes, int32_t nrec, T *M2V,
                                                 T gmin, T gmax, int32_t ngain,
                                                 T Fs) {
  this->current_context->set_active_device(this->device, 1);
  this->is_modopti = 1;
  this->cpt_rec = 0;
  ...
  int64_t dims_data2[3] = {2, this->nactu(), nmodes};
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::modalControlOptimization() {
  this->current_context->set_active_device(this->device, 1);
  if (this->cpt_rec >= this->delay) {
    ...
    carma_geam<T>(this->cublas_handle(), 'n', 'n', this->nslope(), 1, 1.0f,
                  *this->d_centroids, this->nslope(), -1.0f, *d_compbuff2,
                  this->nslope(),
                  this->d_slpol->get_data_at((this->cpt_rec - (int32_t)this->delay) *
                                           this->nslope()),
                  this->nslope());
  }
  this->cpt_rec++;

  int64_t dims_data[2] = {1, this->nrec / 2 + 1};
  ...
  for (int32_t i = 0; i < this->nmodes; i++) {
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::loadopen_loopSlp(T *ol_slopes) {
  this->current_context->set_active_device(this->device, 1);
...
template <typename T, typename Tout>
int32_t SutraControllerLs<T, Tout>::compute_Hcor() {
  this->current_context->set_active_device(this->device, 1);
  int64_t dims_data[3] = {2, this->ngain, this->nrec / 2};
...
template class SutraControllerLs<float, float>;
template class SutraControllerLs<float, uint16_t>;
