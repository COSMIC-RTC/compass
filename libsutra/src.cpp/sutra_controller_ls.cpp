...
int sutra_controller_ls::svdec_imat() {
	...
	if (carma_syevd<float>('V', d_U, h_eigenvals) == EXIT_FAILURE) {
		...
		if (carma_cula_svd<float>(&d_tmp, d_eigenvals, d_U,
				&d_tmp2) == EXIT_FAILURE) {
			return EXIT_FAILURE;
		}
...
int sutra_controller_ls::frame_delay() {
	...
	if (delay > 0) {
		...
		cutilSafeCall(
				cudaMemcpy(
						&(this->d_cenbuff->getData()[delay * this->nslope()]),
						this->d_centroids->getData(),
						sizeof(float) * this->nslope(),
						cudaMemcpyDeviceToDevice));

		cutilSafeCall(
				cudaMemcpy(this->d_centroids->getData(),
						this->d_cenbuff->getData(),
						sizeof(float) * this->nslope(),
						cudaMemcpyDeviceToDevice));
	}
...
int sutra_controller_ls::comp_com() {
	...
	if (nstreams > 1) {
		...
		for (int i = 0; i < nstreams; i++) {
			int istart1 = i * this->d_cmat->getDims(2)
