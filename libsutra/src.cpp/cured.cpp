...
sysCure* cureSystem() {
	...
	for(i = 0; i < linenum; i++)
		for(j = 0; j < linenum; j++)
			sys->I_fried[i*(linenum+1)+j+1] = sys->I_fried[i*(linenum+1)+j+1] | sys->I_sub[i*linenum+j];
	for(i = 0; i < linenum; i++)
		for(j = 0; j < linenum; j++)
			sys->I_fried[(i+1)*(linenum+1)+j] = sys->I_fried[(i+1)*(linenum+1)+j] | sys->I_sub[i*linenum+j];
	for(i = 0; i < linenum; i++)
		for(j = 0; j < linenum; j++)
			sys->I_fried[(i+1)*(linenum+1)+j+1] = sys->I_fried[(i+1)*(linenum+1)+j+1] | sys->I_sub[i*linenum+j];
...
int cure(p_par part_par, p_arrays part_arrays, float *dataX, float *dataY, float *result, float *connect) {
...
for(i = 1 ; i < linenumX ; i++)
    trendX[i] = trendX[i-1] + meanbX[i-1]-meanaX[i] + avgbY[i-1]+avgaY[i];
for(i = 1 ; i < linenumY ; i++)
    trendY[i] = trendY[i-1] + meanbY[i-1]-meanaY[i] + avgbX[i-1]+avgaX[i];
...
for(i = 0 ; i < linenumX ; i++) {
    ...
    if(i > 0 && par_lineX_start[i-1]+par_lineX_length[i-1] > par_lineX_start[i]+par_lineX_length[i])
        startresult += par_lineX_start[i-1]+par_lineX_length[i-1]-par_lineX_start[i]-par_lineX_length[i];
...
for(i = 0 ; i < linenumX ; i++) {
    ...
    if(i < linenumX-1 && par_lineX_start[i]+par_lineX_length[i] < par_lineX_start[i+1]+par_lineX_length[i+1])
        startresult += par_lineX_start[i+1]+par_lineX_length[i+1]-par_lineX_start[i]-par_lineX_length[i];
    }
for(i = 0 ; i < numofresults ; i++) {
...
if(part_par.connect_length[0] > 1) {
	...
	connect[0] += result[part_par.connect_pos[part_par.connect_length[0]-1]]/2;
	}
connect[1] = 0;
if(part_par.connect_length[1] > 1) {
	...
	connect[1] += result[part_par.connect_pos[linenumY+part_par.connect_length[1]]]/2;
	}
connect[2] = 0;
if(part_par.connect_length[2] > 1) {
	...
	connect[2] += result[part_par.connect_pos[2*linenumY+1+part_par.connect_length[2]]]/2;
	}
connect[3] = 0;
if(part_par.connect_length[3] > 1) {
	...
	for(i = 1; i < part_par.connect_length[3]-1; i++)
		connect[3] += result[part_par.connect_pos[2*linenumY+linenumX+3+i]];
	connect[3] += result[part_par.connect_pos[2*linenumY+linenumX+2+part_par.connect_length[3]]]/2;
	}

return 0;
}

int cured(sysCure* sys, parCure *par, float *data, float *result_vec, float gain) {
