...
sysCure* cureSystem() {
	...
	for (i = 0; i < linenum; i++)
		for (j = 0; j < linenum; j++)
			sys->I_fried[i * (linenum + 1) + j + 1] = sys->I_fried[i
...
//float **S_p_iter2 = (float**)malloc(parts*parts*sizeof(float*));
	...
	p_arrays *part_arrays = (p_arrays*) malloc(
			parts * parts * sizeof(p_arrays));
	...
	for (k = 0; k < parts * parts; k++) {
		...
		part_arrays[k].linesX = (float*) malloc(
				(part_par[k].linenumX + part_par[k].numofelems)
						* sizeof(float)); /* x lines */
		part_arrays[k].linesY = (float*) malloc(
				(part_par[k].linenumY + part_par[k].numofelems)
		...
		for (i = 0; i < part_par[k].linenumY; i++) { /* remove empty lines */
			if (part_par[k].lineY_length[i] == 0) {
				for (j = i; j < part_par[k].linenumY - 1; j++) {
					...
					part_par[k].lineY_length[j] =
							part_par[k].lineY_length[j + 1];
				}
				i--;
				part_par[k].linenumY--;
				if (i < 0)
		...
		for (i = 1; i < part_par[k].linenumX; i++) {
			for (j = 0; j < part_par[k].actX_length[i]; j++) {
				...
				if (part_par[k].actX_start[i - 1] <= row
						&& part_par[k].actX_start[i - 1]
								+ part_par[k].actX_length[i - 1] > row)
					sum++;
				if (part_par[k].actX_start[i + 1] <= row
						&& part_par[k].actX_start[i + 1]
								+ part_par[k].actX_length[i + 1] > row)
	...
	for (i = 1; i < parts; i++) {
		for (j = 0; j < parts; j++) {
			...
			if (part_par[start].connect_length[0] != -1
				...
				for (k = 0; k < sum; k++) {
					part_par[start].connect_pos[k] = row
							- part_par[start].actX_start[0]
		...
		for (j = 1; j < parts; j++) {
			...
			if (part_par[start].connect_length[2] != -1
				...
				for (k = 0; k < sum + 1; k++) {
					...
					if (part_par[start].actX_start[k + row
							- part_par[start].empty[0]] == 0
							&& part_par[start - 1].actX_start[k + row
									- part_par[start - 1].empty[0]]
									+ part_par[start - 1].actX_length[k + row
											- part_par[start - 1].empty[0]]
									== part_par[start - 1].linenumY + 1) {
						part_par[start].connect_pos[2 * part_par[start].linenumY
								+ 2 + part_par[start].connect_length[2]] = help;
						part_par[start].connect_length[2]++;
						part_par[start - 1].connect_pos[2
								* part_par[start - 1].linenumY
								+ part_par[start - 1].linenumX + 3
								+ part_par[start - 1].connect_length[3]] =
								help2;
						part_par[start - 1].connect_length[3]++;
					}
					help += part_par[start].actX_length[k + row
							- part_par[start].empty[0]];
				}
			}
		}
	}
	...
	for (j = 0; j < linenum - 1; j++) {
		...
		if (I_sub[(linenum - 1) * linenum + j + 1] != 0
			...
			(*startweight)++;
			*(I_sub_p_iter[I_sub[(linenum - 1) * linenum + j + 1] - 1]++) =
					start;
		}
		if (*startweight) {
			startweight++;
...
//int *I_sub = part_par.I_sub;
	...
	for (i = 0; i < linenumY; i++) {
		...
		meanaY[i] -= (*(startline + par_lineY_starta[i] - par_lineY_start[i]))
				/ 2;
		meanaY[i] += (*(startline + par_lineY_enda[i] - par_lineY_start[i]))
				/ 2;
		meanaY[i] /= par_lineY_enda[i] - par_lineY_starta[i];
		meanbY[i] -= (*(startline + par_lineY_startb[i] - par_lineY_start[i]))
				/ 2;
		meanbY[i] += (*(startline + par_lineY_endb[i] - par_lineY_start[i]))
				/ 2;
	...
	for (i = 0; i < linenumX; i++) {
		...
		startextra[par_lineX_length[i]] = startdataX[par_lineX_length[i] - 1]
				/ 2;
	...
	if (part_par.connect_length[0] > 1) {
		...
		connect[0] +=
				result[part_par.connect_pos[part_par.connect_length[0] - 1]]
						/ 2;
	}
	connect[1] = 0;
	...
	if (part_par.connect_length[3] > 1) {
		connect[3] += result[part_par.connect_pos[2 * linenumY + linenumX + 3]]
				/ 2;
		for (i = 1; i < part_par.connect_length[3] - 1; i++)
			connect[3] += result[part_par.connect_pos[2 * linenumY + linenumX
					+ 3 + i]];
		...
		for (j = 0; j < parts; j += (2 << i))
			for (k = 0; k < parts; k += (2 << i)) {
				...
				for (l = 0; l < (1 << i); l++) {
					...
					v1 += shift[help + l]
							* (part_par[help + l].connect_length[1] - 1)
							+ connect[help + l][1];
				...
				for (l = 0; l < (1 << i); l++) {
					...
					v2 += shift[help + 1 + l * parts]
							* (part_par[help + 1 + l * parts].connect_length[2]
									- 1) + connect[help + 1 + l * parts][2];
				...
				for (l = 0; l < (1 << i); l++)
					for (m = 0; m < (1 << i); m++)
						shift[start + (1 << i) * parts + m + l * parts] +=
								shift1;
				for (l = 0; l < (1 << i); l++)
					for (m = 0; m < (1 << i); m++)
						shift[start + (1 << i) * (parts + 1) + m + l * parts] +=
								shift3;
			}
	}
