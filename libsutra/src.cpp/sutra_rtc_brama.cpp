...
sutra_rtc_brama::sutra_rtc_brama(carma_context *context, ACE_TCHAR* name) : sutra_rtc(context), BRAMA_supervisor(name) {
  framecounter=0;
  initDDS();
}
...
void sutra_rtc_brama::initDDS(){
  ...
  try {
    ...
    cmd_dr = create_datareader(topics[2], cmd_listener);

    superframe_base_dw = create_datawriter(topics[1]);
    if (CORBA::is_nil(superframe_base_dw.in())) {
      cerr << "create_datawriter for " << topics[1] << " failed."
          << endl;
      ACE_OS::exit(1);
    }
    ...
    int nvalid = 0;
    for(int i=0; i<d_centro.size(); i++){
      nvalid += d_centro[i]->nvalid;
    }

    int ncmd = 0;
    for(int i=0; i<d_control.size(); i++){
...
void sutra_rtc_brama::publish(){
  ...
  zFrame.ints.data = BRAMA::Values(dims_intensities[1], dims_intensities[1], buff_intensities, 0);
  ...
  zFrame.slps.data = BRAMA::Values(dims_slopes[1], dims_slopes[1], buff_slopes, 0);
  ...
  zFrame.cmds.data = BRAMA::Values(dims_commands[1], dims_commands[1], buff_commands, 0);
