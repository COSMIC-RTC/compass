...
int fft_goodsize(long size) {
	...
	mradix = (
			tmpf
					> (logf(size) / logf(mradix)
							- (long) (logf(size) / logf(mradix))) ? 3 : mradix);
	...
	mradix = (
			tmpf
					> (logf(size) / logf(mradix)
							- (long) (logf(size) / logf(mradix))) ? 5 : mradix);
	...
	mradix = (
			tmpf
					> (logf(size) / logf(mradix)
							- (long) (logf(size) / logf(mradix))) ? 7 : mradix);

	return mradix;
}

sutra_source::sutra_source(carma_context *context, float xpos, float ypos,
		float lambda, float mag, long size, string type, float *pupil,
		int device) {
...
inline int sutra_source::init_source(carma_context *context, float xpos,
		float ypos, float lambda, float mag, long size, string type,
		int device) {
		...
		if (types.find("atmos") == 0) {
			...
			if ((p == xoff.end()) && async) {
				...
				target_raytrace_async(this->phase_telemetry,
						...
						xoff[make_pair("atmos", alt)],
						yoff[make_pair("atmos", alt)], this->block_size);
			} else {
...
int sutra_source::raytrace(sutra_dms *ydms, int rst, bool async) {
	...
	if (type != "wfs") {
		...
		fillindx(this->d_phasepts->getData(),
				this->d_phase->d_screen->getData(),
...
int sutra_source::comp_image(int puponly) {
	...
	fill_amplipup(this->d_amplipup->getData(),
			this->d_phase->d_screen->getData(), this->d_pupil->getData(),
			this->scale, puponly, this->d_phase->d_screen->getDims(1),
			this->d_phase->d_screen->getDims(2), this->d_amplipup->getDims(1),
			this->device);
...
sutra_target::sutra_target(carma_context *context, int ntargets, float *xpos,
	...
	for (int i = 0; i < ntargets; i++) {
		d_targets.push_back(
				new sutra_source(context, xpos[i], ypos[i], lambda[i], mag[i],
						sizes[i], "target", pupil, device));
	}
}
