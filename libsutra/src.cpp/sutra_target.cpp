...
sutra_source::sutra_source(carma_context *context, float xpos,float ypos,float lambda, float mag, long size, string type, float *pupil, int device)
{
  this->init_source(context,xpos,ypos,lambda,mag,size,type,device);
  this->d_pupil = new carma_obj<float>(this->current_context,this->d_phase->d_screen->getDims());
...
sutra_source::sutra_source(carma_context *context, float xpos,float ypos,float lambda, float mag, long size, string type, int device)
{
  this->init_source(context,xpos,ypos,lambda,mag,size,type,device);
}

int sutra_source::init_source(carma_context *context, float xpos,float ypos,float lambda, float mag, long size, string type, int device)
{
  ...
  this->phase_telemetry = new carma_host_obj<float>(dims_data2,MA_WRICOMB,nstreams);
...
int sutra_source::init_strehlmeter()
{
  ...
  if (this->d_leimage == 0L)
    this->d_leimage = new carma_obj<float>(this->current_context, this->d_image->getDims());
  else
...
int sutra_source::comp_image(int puponly)
{
  ...
  carma_fft(this->d_amplipup->getData(),this->d_amplipup->getData(),1,*this->d_amplipup->getPlan());
...
sutra_target::sutra_target(carma_context *context, int ntargets,float *xpos,float *ypos,float *lambda,float *mag, long *sizes, float *pupil, int device)
{
  this->ntargets = ntargets;

  for (int i=0;i<ntargets;i++) {
    d_targets.push_back(new sutra_source(context, xpos[i],ypos[i],lambda[i],mag[i],sizes[i],"target",pupil,device));
  }
}
