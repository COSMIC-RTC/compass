...
sutra_controller_kalman::sutra_controller_kalman(carma_context* context_,
                                                 int nvalid_, int nactu_)
    : sutra_controller(context_, nvalid_ * 2, nactu_) {
  core_sparse = NULL;
  core_full = NULL;
  cusparseHandle = NULL;
  isGPU = true;
...
void sutra_controller_kalman::init_kalman(carma_host_obj<float>& chD_Mo,
                                          carma_host_obj<float>& chN_Act,
                                          carma_host_obj<float>& chPROJ,
                                          bool is_zonal, bool is_sparse,
                                          bool is_GPU) {
  current_context->set_activeDevice(device);
  if (isInit) {
    cerr << "Error |sutra_controller_kalman::init_kalman | Kalman controller "
  ...
  if (is_sparse) {
    ...
    sD_Mo.init_from_matrix(kD_Mo, pow(10.0, -12.0));
    sD_Mo.resize2colMajor();
    sN_Act.init_from_matrix(kN_Act, pow(10.0, -12.0));
    sN_Act.resize2colMajor();
    sPROJ.init_from_matrix(kPROJ, pow(10.0, -12.0));
    sPROJ.resize2colMajor();

    if (is_GPU) {
      cusparseStatus_t cusparseStat = cusparseCreate(&cusparseHandle);
      if (cusparseStat != CUSPARSE_STATUS_SUCCESS) {
        cerr << "Error | sutra_controller_kalman::sutra_controller_kalman  | "
                "cusparseCreate failed "
             << endl;
        exit(EXIT_FAILURE);
      }

      core_sparse = new kp_kalman_core_sparse_GPU(
          sD_Mo, sN_Act, sPROJ, is_zonal, current_context->get_cublasHandle(),
          cusparseHandle);
    } else
      core_sparse =
          new kp_kalman_core_sparse_CPU(sD_Mo, sN_Act, sPROJ, is_zonal);

  } else if (is_GPU)
    core_full = new kp_kalman_core_full_GPU(
        kD_Mo, kN_Act, kPROJ, is_zonal, current_context->get_cublasHandle());
...
void sutra_controller_kalman::calculate_gain(float bruit,
                                             carma_host_obj<float>& chSigmaV,
                                             carma_host_obj<float>& chatur,
                                             carma_host_obj<float>& chbtur) {
  current_context->set_activeDevice(device);

  if (!isInit) {
    cerr << "Error | sutra_controller_kalman::calculate_gain | Kalman "
            "controller has not been initialiez"
         << endl;
    exit(EXIT_FAILURE);
  }
...
sutra_controller_kalman::sutra_controller_kalman(carma_context* context_,
                                                 ...
                                                 int ndm)
    : sutra_controller(context_, nvalid_ * 2, nactu_, 0.0f, dms, idx_dms, ndm) {
...
void sutra_controller_kalman::calculate_gain(float bruit,
                                             ...
                                             carma_host_obj<float>& chbtur) {}
//
int sutra_controller_kalman::comp_com() { return -378; }
