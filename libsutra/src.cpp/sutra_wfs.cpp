...
sutra_wfs::sutra_wfs(carma_context *context, sutra_telescope *d_tel,
		sutra_sensors *sensors, string type, long nxsub, long nvalid, long npix,
		long nphase, long nrebin, long nfft, long ntot, long npup, float pdiam,
		float nphotons, float nphot4imat, int lgs, int device) :
		device(device), type(type), nxsub(nxsub), nvalid(nvalid), npix(npix), nrebin(
				nrebin), nfft(nfft), ntot(ntot), npup(npup), nphase(nphase), nmaxhr(
				nvalid), nffthr(1), subapd(pdiam), nphot(nphotons), nphot4imat(
				nphot4imat), noise(0), lgs(lgs), kernconv(false), error_budget(
				sensors->error_budget), campli_plan(nullptr), fttotim_plan(
				nullptr), d_ftkernel(nullptr), d_camplipup(nullptr), d_camplifoc(
				nullptr), d_fttotim(nullptr), d_pupil(d_tel->d_pupil_m), d_bincube(
				nullptr), d_bincube_notnoisy(nullptr), d_binimg(nullptr), d_binimg_notnoisy(
				nullptr), d_subsum(nullptr), d_offsets(nullptr), d_fluxPerSub(
				nullptr), d_sincar(nullptr), d_hrmap(nullptr), d_slopes(
				nullptr), image_telemetry(nullptr), d_gs(nullptr), streams(
				nullptr), nstreams(0), d_phasemap(nullptr), d_validsubsx(
				nullptr), d_validsubsy(nullptr), current_context(context), offset(
				0), nvalid_tot(nvalid), rank(0), displ_bincube(nullptr), count_bincube(
				nullptr) {
	if (sensors != nullptr) {
...
int sutra_wfs::wfs_initgs(sutra_sensors *sensors, float xpos, float ypos,
		float lambda, float mag, float zerop, long size, float noise, long seed,
		float G, float thetaML, float dx, float dy) {
	current_context->set_activeDevice(device,1);
	this->d_gs = new sutra_source(current_context, xpos, ypos, lambda, mag,
			zerop, size, "wfs", this->device);
	this->d_gs->G = G;
	...
	if (this->lgs) {
		this->d_gs->d_lgs = new sutra_lgs(current_context, sensors,
...
sutra_sensors::sutra_sensors(carma_context *context, sutra_telescope *d_tel,
		char **type, int nwfs, long *nxsub, long *nvalid, long *npix,
		long *nphase, long *nrebin, long *nfft, long *ntot, long *npup,
		float *pdiam, float *nphot, float *nphot4imat, int *lgs, int device,
		bool error_budget) :
		device(device), error_budget(error_budget), current_context(context), d_camplipup(
				nullptr), d_camplifoc(nullptr), d_fttotim(nullptr), d_ftlgskern(
				nullptr), d_lgskern(nullptr) {
	current_context->set_activeDevice(device,1);
	...
	for (int i = 0; i < nwfs; i++) {
		...
		if (strcmp(type[i], "sh") == 0)
			wfs = new sutra_wfs_sh(context, d_tel, this, nxsub[i], nvalid[i],
					npix[i], nphase[i], nrebin[i], nfft[i], ntot[i], npup[i],
					pdiam[i], nphot[i], nphot4imat[i], lgs[i], device);
		if (strcmp(type[i], "pyr") == 0)
			wfs = new sutra_wfs_pyr_pyr4(context, d_tel, this, nxsub[i],
					nvalid[i], npix[i], nphase[i], nrebin[i], nfft[i], ntot[i],
					npup[i], pdiam[i], nphot[i], nphot4imat[i], lgs[i], device);
		if (strcmp(type[i], "pyrhr") == 0) {
			const int ngpu = context->get_ndevice();
			DEBUG_TRACE("using pyrhr with %d GPUs", ngpu);
			if (ngpu == 1) {
				wfs = new sutra_wfs_pyr_pyrhr(context, d_tel, this, nxsub[i],
						nvalid[i], npix[i], nphase[i], nrebin[i], nfft[i],
						ntot[i], npup[i], pdiam[i], nphot[i], nphot4imat[i],
						lgs[i], device);
			} else {
				int devices[ngpu];
		...
		if (strcmp(type[i], "roof") == 0)
			wfs = new sutra_wfs_pyr_roof(context, d_tel, this, nxsub[i],
					nvalid[i], npix[i], nphase[i], nrebin[i], nfft[i], ntot[i],
					npup[i], pdiam[i], nphot[i], nphot4imat[i], lgs[i], device);
		d_wfs.push_back(wfs);
...
sutra_sensors::sutra_sensors(carma_context *context, sutra_telescope *d_tel,
		int nwfs, long *nxsub, long *nvalid, long *nphase, long npup,
		float *pdiam, int device) :
		device(device), error_budget(false), current_context(context), d_camplipup(
				nullptr), d_camplifoc(nullptr), d_fttotim(nullptr), d_ftlgskern(
				nullptr), d_lgskern(nullptr) {
	...
	for (int i = 0; i < nwfs; i++) {
		d_wfs.push_back(
				new sutra_wfs_geom(context, d_tel, nxsub[i], nvalid[i],
...
int sutra_sensors::sensors_initgs(float *xpos, float *ypos, float *lambda,
		float *mag, float zerop, long *size, float *noise, long *seed, float *G,
		float * thetaML, float *dx, float *dy) {
	current_context->set_activeDevice(device,1);
	for (size_t idx = 0; idx < this->d_wfs.size(); idx++) {
		this->d_wfs[idx]->wfs_initgs(this, xpos[idx], ypos[idx], lambda[idx],
				mag[idx], zerop, size[idx], noise[idx], seed[idx], G[idx],
				thetaML[idx], dx[idx], dy[idx]);
	}
	return EXIT_SUCCESS;
}
int sutra_sensors::sensors_initgs(float *xpos, float *ypos, float *lambda,
		float *mag, float zerop, long *size, float *noise, float *G,
		float * thetaML, float *dx, float *dy) {
	current_context->set_activeDevice(device,1);
	for (size_t idx = 0; idx < this->d_wfs.size(); idx++) {
		this->d_wfs[idx]->wfs_initgs(this, xpos[idx], ypos[idx], lambda[idx],
				mag[idx], zerop, size[idx], noise[idx], 1234 * idx, G[idx],
				thetaML[idx], dx[idx], dy[idx]);
	}
	return EXIT_SUCCESS;
}
int sutra_sensors::sensors_initgs(float *xpos, float *ypos, float *lambda,
		float *mag, float zerop, long *size, float *G, float * thetaML,
		float *dx, float *dy) {
	current_context->set_activeDevice(device,1);
	for (size_t idx = 0; idx < this->d_wfs.size(); idx++) {
		this->d_wfs[idx]->wfs_initgs(this, xpos[idx], ypos[idx], lambda[idx],
				mag[idx], zerop, size[idx], -1, 1234, G[idx], thetaML[idx],
				dx[idx], dy[idx]);
	}
	return EXIT_SUCCESS;
}
