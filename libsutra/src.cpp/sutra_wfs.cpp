...
sutra_wfs::sutra_wfs(carma_context *context, const char* type, long nxsub, long nvalid, long npix, long nphase, 
...
int sutra_wfs::wfs_initgs(float xpos,float ypos,float lambda, float mag, long size,float noise,long seed)
{
  this->d_gs = new sutra_source(current_context,xpos,ypos,lambda,mag,size,"wfs",this->device);
  this->noise = noise;
  if (noise > -1) {
  ...
  if (this->lgs) {
    this->d_gs->d_lgs  = new sutra_lgs(current_context,this->nvalid,this->ntot,this->nmaxhr);
...
int sutra_wfs::wfs_initarrays(int *phasemap,int *hrmap, int *binmap,float *offsets, 
...
int sutra_wfs::wfs_initarrays(cuFloatComplex *halfxy,cuFloatComplex *offsets, float *focmask, 
...
sutra_sensors::sutra_sensors(carma_context *context, const char* type, int nwfs,long *nxsub,long *nvalid,long *npix,long *nphase, long *nrebin,
...
int sutra_sensors::sensors_initgs(float *xpos,float *ypos,float *lambda, float *mag, long *size, 
