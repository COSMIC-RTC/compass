...
sutra_wfs::sutra_wfs(carma_context *context, const char* type, long nxsub,
	...
	if (this->type == "sh") {
		if (this->ntot != this->nfft) {
			...
			dims_data3[2] = ntot;
			dims_data3[3] = nmaxhr;
			this->d_fttotim = new carma_obj<cuFloatComplex>(context,
					dims_data3);
			mdims[0] = (int) dims_data3[1];
			mdims[1] = (int) dims_data3[2];
...
int sutra_wfs::wfs_initgs(float xpos, float ypos, float lambda, float mag,
	...
	if (this->lgs) {
		this->d_gs->d_lgs = new sutra_lgs(current_context, this->nvalid,
	...
	if (this->ntot != this->nfft) {
		...
		for (int cc = 0; cc < this->nffthr; cc++) {
			...
			if (this->lgs) {
				...
				// convolve with psf
				carma_fft(this->d_fttotim->getData(),
						this->d_fttotim->getData(), 1,
		...
		if (this->lgs) {
			...
			carma_fft(this->d_camplifoc->getData(), this->d_fttotim->getData(),
					1, *this->d_fttotim->getPlan());

			convolve(this->d_fttotim->getData(),
					this->d_gs->d_lgs->d_ftlgskern->getData(),
					this->d_fttotim->getNbElem(), this->device);

			carma_fft(this->d_fttotim->getData(), this->d_fttotim->getData(),
					-1, *this->d_fttotim->getPlan());
			...
			if (this->kernconv) {
				carma_fft(this->d_camplifoc->getData(),
						this->d_camplifoc->getData(), 1,
						*this->d_camplipup->getPlan());
...
sutra_sensors::sutra_sensors(carma_context *context, const char* type, int nwfs,
	...
	for (int i = 0; i < nwfs; i++) {
		d_wfs.push_back(
				new sutra_wfs(context, type, nxsub[i], nvalid[i], npix[i],
...
sutra_sensors::sutra_sensors(carma_context *context, int nwfs, long *nxsub,
	...
	for (int i = 0; i < nwfs; i++) {
		d_wfs.push_back(
				new sutra_wfs(context, nxsub[i], nvalid[i], nphase[i], npup,
						pdiam[i], device));
	}
}
...
int sutra_sensors::sensors_initgs(float *xpos, float *ypos, float *lambda,
	...
	for (size_t idx = 0; idx < (this->d_wfs).size(); idx++) {
		(this->d_wfs)[idx]->wfs_initgs(xpos[idx], ypos[idx], lambda[idx],
