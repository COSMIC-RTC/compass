...
sutra_sensors::sutra_sensors(carma_context *context, string type, int nwfs, long *nxsub,
  ...
  for (int i = 0; i < nwfs; i++) {
    ...
    if (type == "sh")
      wfs = new sutra_wfs_sh(context, this, nxsub[i], nvalid[i], npix[i],
                             nphase[i], nrebin[i], nfft[i], ntot[i], npup,
...
sutra_sensors::sutra_sensors(carma_context *context, int nwfs, long *nxsub,
  ...
  for (int i = 0; i < nwfs; i++) {
    d_wfs.push_back(
        new sutra_wfs_geom(context, nxsub[i], nvalid[i], nphase[i], npup, pdiam[i],
            device));
  }
  this->d_camplifoc = 0L;
  this->d_camplipup = 0L;
