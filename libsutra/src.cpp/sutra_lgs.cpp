...
int sutra_lgs::lgs_init(int nprof, float hg, float h0, float deltah,
		float pixsize, float *doffaxis, float *prof1d, float *profcum,
		float *beam, cuFloatComplex *ftbeam, float *azimuth) {
	this->nprof = nprof;
	this->hg = hg;
	this->h0 = h0;
	this->deltah = deltah;
...
int sutra_lgs::lgs_update(int device) {
	...
	// mult by beamft
	times_ftbeam(this->d_prof2d->getData(), this->d_ftbeam->getData(),
			this->npix, this->d_prof2d->getNbElem(), device);
...
int sutra_lgs::lgs_makespot(int device, int nin) {
	...
	rollbeamexp(this->d_lgskern->getData(), &(data[nin]),
			this->d_beam->getData(), this->npix, this->d_lgskern->getNbElem(),
			device);
