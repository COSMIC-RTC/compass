...
sutra_lgs::sutra_lgs(carma_context *context, long nvalid, long npix, long nmaxhr)
{
...
int sutra_lgs::lgs_init(int nprof, float hg, float h0, float deltah, float pixsize, float *doffaxis, float *prof1d,
...
int sutra_lgs::load_prof(float *prof1d,float *profcum, float hg, float h0, float deltah)
{
  this->d_prof1d->host2device(prof1d);
  this->d_profcum->host2device(profcum);
  this->hg = hg;
...
int sutra_lgs::lgs_update(int device)
{
  ...
  // convolution by beam
  // do fft on prof2d
  carma_fft(this->d_prof2d->getData(),this->d_prof2d->getData(),1,*this->d_prof2d->getPlan());
...
int sutra_lgs::lgs_makespot(int device, int nin)
{
  ...
  */
  ...
  carma_fft(this->d_ftlgskern->getData(),this->d_ftlgskern->getData(),1,*this->d_ftlgskern->getPlan());
