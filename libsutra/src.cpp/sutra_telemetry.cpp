...
sutra_telemetry::sutra_telemetry(type_telemetry_pair obj,carma_host_obj<float> *host_obj, unsigned int nbStreams)
{
...
sutra_telemetry::sutra_telemetry(std::string type_obj,int num_obj,carma_host_obj<float> *host_obj, unsigned int nbStreams)
{
...
sutra_telemetry::~sutra_telemetry()
{
	for( std::map< type_telemetry_pair, carma_host_obj<float>* >::iterator it = objs.begin();
			objs.end()!= it ; it++) {
		delete it->second;
	}
	this->objs.clear();
...
int sutra_telemetry::add_obj(type_telemetry_pair obj,carma_host_obj<float> *host_obj)
{
	this->objs[obj] = host_obj;
	return get_nbObjs();
}

int sutra_telemetry::add_obj(std::string type_obj,int num_obj,carma_host_obj<float> *host_obj)
{
...
carma_host_obj<float>* sutra_telemetry::get_carma_host_obj(type_telemetry_pair obj)
{
	return this->objs[obj];
}

carma_host_obj<float>* sutra_telemetry::get_carma_host_obj(std::string type_obj,int num_obj)
{
	return this->objs[make_pair(type_obj, num_obj)];
}

int sutra_telemetry::cpy_obj(std::string type_obj,int num_obj,carma_obj<float> *d_obj, cudaMemcpyKind flag)
{
