...
class sutra_source {
 ...
 public:
  sutra_source(carma_context *context, float xpos,float ypos,float lambda,float mag,long size,string type, float *pupil, int device);
  ...
  int raytrace(sutra_dms *ydms, int rst, bool async);
...
class sutra_target {
 ...
 public:
  sutra_target(carma_context *context, int ntargets,float *xpos,float *ypos,float *lambda,float *mag,long *sizes, float *pupil, int device);
  ~sutra_target();
...
int target_raytrace(float *d_odata,float *d_idata,int nx, int ny,int Nx,float xoff, float yoff,  
		    int block_size);
int target_raytrace_async(carma_streams streams, float *d_odata,float *d_idata,int nx, int ny,int Nx,float xoff, float yoff,
		    int block_size);
int target_raytrace_async(carma_host_obj<float> *phase_telemetry,float *d_odata,float *d_idata,
