...
__global__ void combikl_krnl(float *com, int nkl, float *odata, float *rabas,
	...
	if ((xid < nx) && (yid < nx)) {
		...
		for (int cc = 0; cc < nkl; cc++) {
			...
			kl_interp(1.0f, com, cc, odata, rabas_cc, azbas_cc, xbi, ybi, nr,
					np, tido);
		}
		__syncthreads();
	}
...
int getkl(float ampli, float *d_odata, float *rabas, float *azbas, float *cr,
	...
	return getkl(0.0f, ampli, d_odata, rabas, azbas, cr, cp, nr, np, nx, Nx,
			xoff, yoff);
}
...
int getkl(float *d_odata, float *rabas, float *azbas, float *cr, float *cp,
	...
	return getkl(0.0f, 1.0f, d_odata, rabas, azbas, cr, cp, nr, np, nx, Nx,
			xoff, yoff);
}
...
int combikl(float *com, int nkl, float *d_odata, float *rabas, int *d_ord,
	...
	//for (int cc=0;cc<nkl;cc++)
	combikl_krnl<<<blocks, threads>>>(com, nkl, d_odata, rabas, d_ord, azbas,
			cr, cp, nr, np, nx, Nx, xoff, yoff);
