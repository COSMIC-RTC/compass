...
__global__ void iprof_krnl(cuFloatComplex *profout, float *profin,
	...
	while (tid < Ntot) {
		...
		if (pixsize > deltah * norm) {
			profout[tid].x = interp_transf(profinc, tidim + 1, npix, pixsize,
					norm, hg - h0, deltah, hmax)
...
// rotate a cube manually
	...
	while (tid < Ntot) {
		...
		odata[tid].x = (idata[ind1] * (1 - weightu) + idata[ind2] * weightu)
				* (1 - weightv)
				+ (idata[ind3] * (1 - weightu) + idata[ind4] * weightu)
						* weightv;
		odata[tid].y = 0.0f;
...
int lgs_rotate(cuFloatComplex *odata, float *idata, int width, int height,
	...
	rotate_krnl<<<grid, threads>>>(odata, idata, width, height, theta, center,
			N, Ntot);
...
int rotate3d(cuFloatComplex *d_odata, cudaMemcpy3DParms copyParams,
	...
	rotate3d_krnl<<<grid, threads, 0>>>(d_odata, width, height, N, theta,
			center, Ntot);
