...
template<class T> __device__ void scanmax_krnl(T *sdata, int *values, int size,
		int n) {
	if (!((size & (size - 1)) == 0)) {
		unsigned int s;
		if ((size & 1) != 0)
...
template<class T> __device__ inline void sortmax_krnl(T *sdata,
		unsigned int *values, int size, int n) {
...
template<class T> __global__ void sortmax(T *g_idata, T *g_odata, int *values,
		int nmax) {
	...
	for (int cc = 0; cc < nmax; cc++) {
		...
		if (tid >= cc)
			sortmax_krnl(&(sdata[cc]), &(svalues[cc]), blockDim.x - cc,
					tid - cc);
...
template<class T> __device__ inline void sortmaxi_krnl(T *sdata,
		unsigned int *values, int size, int n) {
...
template<class T> __global__ void sortmaxi(T *g_idata, int *values, int nmax,
		int offx, int offy, int npix, int Npix, int Npix2) {
	extern __shared__ uint svalues[];
	T *sdata = (T*) &svalues[blockDim.x];
	...
	for (int cc = 0; cc < nmax; cc++) {
		...
		if (tid >= cc)
			sortmaxi_krnl(&(sdata[cc]), &(svalues[cc]), blockDim.x - cc,
					tid - cc);
...
template<class T> __global__ void centroid_max(T *g_idata, T *g_odata, int n,
...
template<class T> __global__ void centroidx(T *g_idata, T *g_odata, T *alpha,
...
template<class T> __global__ void centroidy(T *g_idata, T *g_odata, T *alpha,
		unsigned int n, unsigned int N, T scale, T offset) {
	T *sdata = SharedMemory<T>();
...
template<class T> __global__ void centroidx_async(T *g_idata, T *g_odata,
		T *alpha, unsigned int n, unsigned int N, T scale, T offset,
		int stream_offset) {
	T *sdata = SharedMemory<T>();
	...
	if (tid == 0)
		g_odata[blockIdx.x + stream_offset] = ((sdata[0] * 1.0
				/ (alpha[blockIdx.x + stream_offset] + 1.e-6)) - offset)
				* scale;
}

template<class T> __global__ void centroidy_async(T *g_idata, T *g_odata,
		T *alpha, unsigned int n, unsigned int N, T scale, T offset,
		int stream_offset) {
	T *sdata = SharedMemory<T>();
...
template<class T> void get_centroids_async(int threads, int blocks, int n,
	...
	for (int i = 0; i < nstreams; i++) {
		centroidx_async<T> <<<dimGrid, dimBlock, smemSize,
				streams->get_stream(i)>>>(d_idata, d_odata, alpha, n, nbelem,
				scale, offset, i * blocks / nstreams);

		cutilCheckMsg("centroidx_kernel<<<>>> execution failed\n");

		centroidy_async<T> <<<dimGrid, dimBlock, smemSize,
				streams->get_stream(i)>>>(d_idata, &(d_odata[blocks]), alpha, n,
				nbelem, scale, offset, i * blocks / nstreams);
...
template<class T> __global__ void centroidx(T *g_idata, T *g_odata, T *alpha,
		T thresh, unsigned int n, unsigned int N, T scale, T offset) {
	T *sdata = SharedMemory<T>();
...
template<class T> __global__ void centroidx(T *g_idata, T *g_odata, T *alpha,
		T *weights, unsigned int n, unsigned int N, T scale, T offset) {
	T *sdata = SharedMemory<T>();
...
template<class T> __global__ void interp_parab(T *g_idata, T *g_centroids,
		int *g_values, T *g_matinterp, int sizex, int sizey, int nvalid,
	...
	if (threadIdx.x < 2) {
		...
		if (denom == 0) {
			...
			if (threadIdx.x == 0) {
				...
				g_centroids[blockIdx.x] = (g_centroids[blockIdx.x] - offset)
						* scale;
			}
			if (threadIdx.x == 1) {
				...
				g_centroids[blockIdx.x + nvalid] = (g_centroids[blockIdx.x
						+ nvalid] - offset) * scale;
			}
		}
	}
...
__global__ void corr_krnl(cuFloatComplex *odata, cuFloatComplex *idata, int N) {
	...
	while (tid < N) {
		...
		tmp.y = -1.0f * idata[tid].y * odata[tid].x
				+ idata[tid].x * odata[tid].y;
		odata[tid].x = tmp.x;
		odata[tid].y = tmp.y;
		tid += blockDim.x * gridDim.x;
	}
...
template void subap_sortmax<float>(int size, int threads, int blocks,
		float *d_idata, float *d_odata, int *values, int nmax);

template void subap_sortmax<double>(int size, int threads, int blocks,
		double *d_idata, double *d_odata, int *values, int nmax);
...
template void subap_centromax<float>(int threads, int blocks, float *d_idata,
		float *d_odata, int npix, int nmax, float scale, float offset);

template void subap_centromax<double>(int threads, int blocks, double *d_idata,
...
template void get_centroids<float>(int size, int threads, int blocks, int n,
		float *d_idata, float *d_odata, float *alpha, float scale,
		float offset);

template void get_centroids<double>(int size, int threads, int blocks, int n,
		double *d_idata, double *d_odata, double *alpha, double scale,
		double offset);
...
template void get_centroids<float>(int size, int threads, int blocks, int n,
		float *d_idata, float *d_odata, float *alpha, float thresh, float scale,
		float offset);

template void get_centroids<double>(int size, int threads, int blocks, int n,
		double *d_idata, double *d_odata, double *alpha, double thresh,
...
template void get_centroids<float>(int size, int threads, int blocks, int n,
		float *d_idata, float *d_odata, float *alpha, float *weights,
		float scale, float offset);

template void get_centroids<double>(int size, int threads, int blocks, int n,
		double *d_idata, double *d_odata, double *alpha, double *weights,
		double scale, double offset);
...
int fillcorr(cuFloatComplex *d_out, float *d_in, int npix_in, int npix_out,
	...
	if (nvalid == 1) {
		...
		fillcorrim_krnl<<<grid, threads>>>(d_out, d_in, npix_in,
				npix_in * npix_in, npix_out, npix_out * npix_out, N);
		//cout << "2d" << endl;
	}
...
template void subap_pinterp<float>(int threads, int blocks, float *d_idata,
		int *values, float *d_centroids, float *d_matinterp, int sizex,
		int sizey, int nvalid, int Npix, float scale, float offset);
/*
...
template<class T> __global__ void pyrslopes_krnl(T *g_odata, T *g_idata,
		int *subindx, int *subindy, T *subsum, unsigned int ns,
	...
	if (i < nvalid) {
		...
		g_odata[i + nvalid] = ((g_idata[i2 + 2 * ns * ns]
				+ g_idata[i2 + 3 * ns * ns])
				- (g_idata[i2] + g_idata[i2 + ns * ns])) / subsum[i];
	}
}
...
template void pyr_slopes<float>(float *d_odata, float *d_idata, int *subindx,
		int *subindy, float *subsum, int ns, int nvalid, int nim, int device);
template void pyr_slopes<double>(double *d_odata, double *d_idata, int *subindx,
		int *subindy, double *subsum, int ns, int nvalid, int nim, int device);
