 ...
 */
{
  ...
  const T_data Ga[11] = { 0, 12.067619015983075, 5.17183672113560444,
...
__device__ T_data asymp_macdo_gpu_gb(Fn const &ptr_pow, Fne const &ptr_exp, T_data x)
 ...
 */
{
  ...
  x_1 = 1. / x;
  res = k2
        - k3 * ptr_exp(-x) * ptr_pow(x, (T_data)(1 / 3.))
...
template
__device__ float unMoinsJ0<float>(float x);
template
__device__ double unMoinsJ0<double>(double x);
...
template
__global__ void intfrominftomin<float>(float *data, float smallInt, int N);
template
__global__ void intfrominftomin<double>(double *data, double smallInt, int N);
...
template
__global__ void cuda_zcen_krnl<float>(float *idata, float *odata, int N);
template
__global__ void cuda_zcen_krnl<double>(double *idata, double *odata, int N);
...
template
void cumsum<float>(float *odata, float *idata, int N);
template
void cumsum<double>(double *odata, double *idata, int N);
...
__device__ T_data Ij0t83_gen(Fn const &ptr_pow,Fne const &ptr_exp,Fnl const &ptr_log,
                             T_data x, T_data *tab_x, T_data *tab_y, long npts) {
  if (x<=ptr_exp(-3.0))
    ...
    return  (((x - tab_x[i0]) * tab_y[i1]
              + (tab_x[i1] - x) * tab_y[i0])) / (tab_x[i1] - tab_x[i0]);
  }
}
...
__device__ T_data DPHI_lowpass_gen(Fn const &ptr_sqrt, T_data x, T_data y, T_data L0,
                                   T_data fc, T_data *tab_int_x, T_data *tab_int_y,
                                   long npts) {
...
__device__ void compute_Cerr_element_gen(Fnc const &ptr_cos, Fns const &ptr_sin,
  ...
  while(tid < N*N) {
    ...
    Cerr[tid] += 0.5 * (DPHI_lowpass(xij_hvdt,yij_hvdt, L0, fc, tab_int_x, tab_int_y, Ntab)
                        - DPHI_lowpass(xij_ht,yij_ht, L0, fc, tab_int_x, tab_int_y, Ntab)
                        - DPHI_lowpass(xij_vdt,yij_vdt, L0, fc, tab_int_x, tab_int_y, Ntab)
                        + DPHI_lowpass(xij,yij, L0, fc, tab_int_x, tab_int_y, Ntab)) * scale;
    Cerr[tid] += 0.5 * (DPHI_lowpass(xij_ht,yij_ht, L0, fc, tab_int_x, tab_int_y, Ntab)
                        - DPHI_lowpass(xij,yij, L0, fc, tab_int_x, tab_int_y, Ntab)) * scale;
    Cerr[tid] += 0.5 * (DPHI_lowpass(xij_vdt,yij_vdt, L0, fc, tab_int_x, tab_int_y, Ntab)
                        - DPHI_lowpass(xij,yij, L0, fc, tab_int_x, tab_int_y, Ntab)) * scale;
...
template
__global__ void add_transpose_krnl<float>(float *Cerr, int N);
template
__global__ void add_transpose_krnl<double>(double *Cerr, int N);
...
template
int compute_Cerr_layer<float>(float *Cerr, int N, float *tab_int_x, float *tab_int_y,
                              float *xpos, float *ypos, float vdt,
                              float Htheta, float L0, float fc, float winddir, float gsangle,
                              float scale, int Ntab, carma_device *device);
template
int compute_Cerr_layer<double>(double *Cerr, int N, double *tab_int_x, double *tab_int_y,
                               double *xpos, double *ypos, double vdt,
                               double Htheta, double L0, double fc, double winddir, double gsangle,
                               double scale, int Ntab, carma_device *device);
