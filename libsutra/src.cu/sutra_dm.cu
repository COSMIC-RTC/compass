...
void bindTexture(	float *cmdVector, float *influ, tuple_t<float> *inData,
                  int *istart, int *npoints, int *influpos,
                  int n_npoints, int n_influpos, int n_vector, int n_influ) {
...
__device__ inline T getData(
  const int pos,
  const T * cmdVector, const T *influData, const int *iPos) {
...
__device__ inline T getData(
  const int pos,
  const T * cmdVector, const struct tuple_t<T> *inData) {
...
__global__ void compShape1(
  ...
  if (id < N) {
    ...
    for (int pos = interval.start; pos < (interval.start + interval.nbInflu); ++pos)
...
template<class T>
__global__ void sharedCompShape1(
  T *outData,
  const T * cmdVector,
  const int N
) {
  ...
  if (bId < N) {
    ...
    if(tId == 0)
      outData[bId] = tempVector[0];
  }
}

template<class T>
__global__ void sharedCompShape2(
  T *outData,
  const T * cmdVector,
  const int N
) {
...
template<class T>
__global__ void sharedCompShape3(
  T *outData,
  const T * cmdVector,
  const int N
) {
...
__global__ void compShape1(
  T *outData,
  const T * cmdVector, const T *influData,
...
__global__ void sharedCompShape1(
  ...
  if (bId < N) {
    ...
    if(tId < interval.nbInflu)
      tempVector[tId] = getData(tId + interval.start, cmdVector, influData, iPos);
...
#if(COMPN == 1)
  sharedCompShape1<T> <<<blocks, threads, shared>>>(outData, cmdVector, influData, iStart_t, nbInflu_t, iPos, roiLength);
#elif(COMPN == 2)
  sharedCompShape2<T> <<<blocks, threads, shared>>>(outData, cmdVector, inData, iStart_t, roiLength);
#elif(COMPN == 3)
  sharedCompShape3<T> <<<blocks, threads, shared>>>(outData, cmdVector, inData, roiLength);
#endif

#else

#if(COMPN == 1)
  compShape1<T> <<<blocks, threads>>>(outData, cmdVector, influData, iStart_t, nbInflu_t, iPos, roiLength);
#elif(COMPN == 2)
  compShape2<T> <<<blocks, threads>>>(outData, cmdVector, inData, iStart_t, roiLength);
#elif(COMPN == 3)
...
template void
comp_dmshape2<float>(float *outData,
                     const float * cmdVector, const float *influData, const struct tuple_t<float> *inData,
                     const int *iStart_t, const int *nbInflu_t, const int *iPos,
                     const int roiLength,
                     const dim3 threads, const dim3 blocks, const int shared);

template void
comp_dmshape2<double>(double *outData,
                      const double * cmdVector, const double *influData, const struct tuple_t<double> *inData,
                      const int *iStart_t, const int *nbInflu_t, const int *iPos,
                      const int roiLength,
                      const dim3 threads, const dim3 blocks, const int shared);
...
template<class T>
__global__ void
getIFfull_krnl(T *IF, float *dmshape, long nb_pts, long column, long nb_col) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  while (tid < nb_pts) {
...
template
int
getIF<float>(float *IF, float *dmshape, int *indx_pup, long nb_pts, int column, long nb_col, int puponly, carma_device *device);
template
int
getIF<double>(double *IF, float *dmshape, int *indx_pup, long nb_pts, int column, long nb_col, int puponly, carma_device *device);
