...
template int fillbincube<double>(double *bimage, double *bcube, int npix,
		int nsub, int Nsub, int *ivalid, int *jvalid, int device);
...
int fillbincube_async(carma_host_obj<T> *image_telemetry, T *bimage, T *bcube,
	...
	for (int i = 0; i < nstreams; i++) {
		cudaMemcpyAsync(&(bimage[i * nim / nstreams]),
				(*image_telemetry)[i * nim / nstreams],
				sizeof(float) * nim / nstreams, cudaMemcpyHostToDevice,
				image_telemetry->get_cudaStream_t(i));
		bcube_krnl_async<T> <<<grid, threads, 0,
				image_telemetry->get_cudaStream_t(i)>>>(bimage, bcube, npix,
				Npix, Nsub, ivalid, jvalid, N, i * N / nstreams);
...
template int fillbincube_async<float>(carma_host_obj<float> *image_telemetry,
		float *bimage, float *bcube, int npix, int nsub, int Nsub, int *ivalid,
		int *jvalid, int nim, int device);
template int fillbincube_async<double>(carma_host_obj<double> *image_telemetry,
		double *bimage, double *bcube, int npix, int nsub, int Nsub,
		int *ivalid, int *jvalid, int nim, int device);
...
template int fillbincube_async<float>(carma_streams *streams,
		carma_obj<float> *bimage, carma_obj<float> *bcube, int npix, int nsub,
		int Nsub, int *ivalid, int *jvalid, int device);
template int fillbincube_async<double>(carma_streams *streams,
		carma_obj<double> *bimage, carma_obj<double> *bcube, int npix, int nsub,
		int Nsub, int *ivalid, int *jvalid, int device);
