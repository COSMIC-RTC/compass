...
template<class T>
  __global__ void
  bcube_krnl(T *bimage, T *bcube, int npix, int npix2, int nsub, int *ivalid,
      int *jvalid, int N) {
    /*
...
template<class T>
  __global__ void
  bcube_krnl_async(T *bimage, T *bcube, int npix, int npix2, int nsub,
      int *ivalid, int *jvalid, int N, int idstart) {
    /*
     indx is an array nrebin^2 * npix^2
     it gives the nrebin x nrebin pixels in the hrimage per npix x npix pixels of the subap
     Npix = npix x npix
