...
__global__ void
camplipup_krnl(cuFloatComplex *amplipup, float *phase, float *offset,
    float *mask, float scale, int *istart, int *jstart, int *ivalid,
    int *jvalid, int nphase, int nphase2, int npup, int Nfft, int N) {
...
__global__ void
bimg_krnl(float *bimage, float *bcube, int npix, int npix2, int nsub,
    int *ivalid, int *jvalid, float alpha, int N) {
  /*
...
__global__ void
bimg_krnl_async(float *bimage, float *bcube, int npix, int npix2, int nsub,
    int *ivalid, int *jvalid, float alpha, int N, int idstart) {
  /*
   indx is an array nrebin^2 * npix^2
   it gives the nrebin x nrebin pixels in the hrimage per npix x npix pixels of the subap
   Npix = npix x npix
...
__global__ void
fillbincube_krnl(float *bcube, cuFloatComplex *hrimage, int *indxpix, int Nfft,
    int Npix, int Nrebin, int N) {
  /*
   indx is an array nrebin^2 * npix^2
   it gives the nrebin x nrebin pixels in the hrimage per npix x npix pixels of the subap
   Npix = npix x npix
...
__global__ void
fillbincube_krnl_async(float *bcube, cuFloatComplex *hrimage, int *indxpix,
    int Nfft, int Npix, int Nrebin, int N, int idstart) {
  /*
   indx is an array nrebin^2 * npix^2
   it gives the nrebin x nrebin pixels in the hrimage per npix x npix pixels of the subap
   Npix = npix x npix
...
fillbincube_async(carma_streams *streams, float *bcube, cuFloatComplex *hrimage,
    int *indxpix, int Nfft, int Npix, int Nrebin, int Nsub, int device) {
  int N = Npix * Nsub;
  int nthreads = 0, nblocks = 0;
  int nstreams = streams->get_nbStreams();
  getNumBlocksAndThreads(device, N, nblocks, nthreads);
...
__global__ void
indexfill_krnl(cuFloatComplex *odata, cuFloatComplex *idata, int *indx,
    int ntot, int Ntot, int N) {
  int nim, idim;
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void
conv_krnl(cuFloatComplex *odata, cuFloatComplex *idata, int N) {
...
int
convolve(cuFloatComplex *d_odata, cuFloatComplex *d_idata, int N, int device) {
  int nthreads = 0, nblocks = 0;
  getNumBlocksAndThreads(device, N, nblocks, nthreads);
...
__global__ void
conv_krnl(cuFloatComplex *odata, cuFloatComplex *idata, int N, int n) {
...
template<class T>
  __device__ void
  red_krnl(T *sdata, int size, int n) {
    if (!((size & (size - 1)) == 0)) {
      unsigned int s;
      if (size % 2 != 0)
        s = size / 2 + 1;
...
template<class T>
  __global__ void
  reduce2(T *g_idata, T *g_odata, T thresh, unsigned int n) {
    T *sdata = SharedMemory<T>();
...
template<class T>
  __global__ void
  reduce2(T *g_idata, T *g_odata, T *weights, unsigned int n) {
    T *sdata = SharedMemory<T>();
...
template<class T>
  __global__ void
  reduce2_async(T *g_idata, T *g_odata, unsigned int n, int stream_offset) {
    T *sdata = SharedMemory<T>();
...
template<class T>
  __global__ void
  reduce_phasex(T *g_idata, T *g_odata, int *indx, unsigned int n, T alpha) {
    T *sdata = SharedMemory<T>();
...
template<class T>
  __global__ void
  reduce_phasey(T *g_idata, T *g_odata, int *indx, unsigned int n, T alpha)
// FIXME
...
template<class T>
  __global__ void
  derive_phasex(T *g_idata, T *g_odata, int *indx, T *mask, T alpha,
      unsigned int n, unsigned int N, float *fluxPerSub) {
    T *sdata = SharedMemory<T>();
...
template<class T>
  __global__ void
  derive_phasey(T *g_idata, T *g_odata, int *indx, T *mask, T alpha,
      unsigned int n, unsigned int N, float *fluxPerSub) {
    T *sdata = SharedMemory<T>();
...
template<class Tout, class Tin>
  __global__ void
  pyrgetpup_krnl(Tout *g_odata, Tin *g_idata, Tout *offsets, Tin *pup,
      unsigned int n) {
...
template<class T>
  __global__ void
  rollmod_krnl(T *g_odata, T *g_idata, T *g_mask, int cx, int cy,
...
template<class T>
  __global__ void
  fillbinpyr_krnl(T *g_odata, T *g_idata, unsigned int nrebin, unsigned int n,
...
template<class Tout, class Tin>
  __global__ void
  abspyr_krnl(Tout *g_odata, Tin *g_idata, unsigned int ns, unsigned int nim) {
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
    int x, y;
...
template<class Tin, class Tout>
  __global__ void
  abs2pyr_krnl(Tout *g_odata, Tin *g_idata, Tout fact, unsigned int ns,
      unsigned int nim) {
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
    int x, y;
...
template<class Tout, class Tin>
  __global__ void
  submask_krnl(Tout *g_odata, Tin *g_mask, unsigned int n) {
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
...
template<class Tout, class Tin>
  __global__ void
  submask3d_krnl(Tout *g_odata, Tin *g_mask, unsigned int n, unsigned int nim) {
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
...
template<class T>
  __global__ void
  subsum_krnl(T *g_odata, T *g_idata, int *subindx, int *subindy,
...
template<class T>
  __global__ void
  pyrfact_krnl(T *g_data, T fact, unsigned int n, unsigned int nim) {
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void
pyrfact_krnl(cuFloatComplex *g_data, float fact, unsigned int n,
    unsigned int nim) {
  unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void
pyrfact_krnl(float *g_data, float fact1, float *fact2, unsigned int n,
    unsigned int nim) {
  unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
