...
int fillbinimg(float *bimage, float *bcube, int npix, int nsub, int Nsub,
	...
	bimg_krnl<<<grid, threads>>>(bimage, bcube, npix, Npix, Nsub, ivalid,
			jvalid, alpha, N);
...
int fillbinimg_async(carma_host_obj<float> *image_telemetry, float *bimage,
	...
	for (int i = 0; i < nstreams; i++) {
		...
		cudaMemcpyAsync(&(hdata[i * nim / nstreams]),
				&(bimage[i * nim / nstreams]), sizeof(float) * nim / nstreams,
...
int fillbinimg_async(carma_streams *streams, carma_obj<float> *bimage,
	...
	for (int i = 0; i < nstreams; i++) {
		...
		bimg_krnl_async<<<grid, threads, 0, streams->get_stream(i)>>>(g_image,
				g_cube, npix, Npix, Nsub, ivalid, jvalid, alpha, N,
				i * N / nstreams);
		// asynchronously launch nstreams memcopies.  Note that memcopy in stream x will only
		//   commence executing when all previous CUDA calls in stream x have completed
	}
...
int fillbincube_async(carma_streams *streams, float *bcube,
	...
	for (int i = 0; i < nstreams; i++) {
		fillbincube_krnl_async<<<grid, threads, 0, streams->get_stream(i)>>>(
				bcube, hrimage, indxpix, Nfft, Npix, Nrebin, N,
				i * N / nstreams);
...
template<class T> __global__ void reduce2(T *g_idata, T *g_odata,
		unsigned int n) {
...
template<class T> __global__ void reduce2(T *g_idata, T *g_odata, T thresh,
		unsigned int n) {
	T *sdata = SharedMemory<T>();
...
template<class T> __global__ void reduce2(T *g_idata, T *g_odata, T *weights,
		unsigned int n) {
	T *sdata = SharedMemory<T>();
...
template<class T> void subap_reduce(int size, int threads, int blocks,
		T *d_idata, T *d_odata) {
...
template<class T> __global__ void reduce2_async(T *g_idata, T *g_odata,
		unsigned int n, int stream_offset) {
	T *sdata = SharedMemory<T>();
...
template<class T> void subap_reduce_async(int threads, int blocks,
		carma_streams *streams, T *d_idata, T *d_odata) {
	int nstreams = streams->get_nbStreams();
...
template void subap_reduce_async<float>(int threads, int blocks,
		carma_streams *streams, float *d_idata, float *d_odata);
template void subap_reduce_async<double>(int threads, int blocks,
		carma_streams *streams, double *d_idata, double *d_odata);

template<class T> void subap_reduce(int size, int threads, int blocks,
		T *d_idata, T *d_odata, T thresh) {
	dim3 dimBlock(threads, 1, 1);
	dim3 dimGrid(blocks, 1, 1);
...
template void subap_reduce<double>(int size, int threads, int blocks,
		double *d_idata, double *d_odata, double thresh);

template<class T> void subap_reduce(int size, int threads, int blocks,
		T *d_idata, T *d_odata, T *weights) {
	dim3 dimBlock(threads, 1, 1);
	dim3 dimGrid(blocks, 1, 1);
...
template void subap_reduce<float>(int size, int threads, int blocks,
		float *d_idata, float *d_odata, float *weights);

template void subap_reduce<double>(int size, int threads, int blocks,
		double *d_idata, double *d_odata, double *weights);

template<class T> __global__ void reduce_phasex(T *g_idata, T *g_odata,
		int *indx, unsigned int n, T alpha) {
	T *sdata = SharedMemory<T>();
	...
	sdata[tid] = g_idata[indx[i + tid * n + n - 1]]
			- g_idata[indx[i + tid * n]];
...
template<class T> __global__ void reduce_phasey(T *g_idata, T *g_odata,
		int *indx, unsigned int n, T alpha)
// FIXME
...
template<class T> __global__ void derive_phasex(T *g_idata, T *g_odata,
		int *indx, T *mask, T alpha, unsigned int n, unsigned int N,
		float *fluxPerSub) {
	T *sdata = SharedMemory<T>();
	...
	if (i < N) {
		if (tid % n == 0) {    //start of a column
			sdata[tid] = (g_idata[indx[i + 1]] - g_idata[indx[i]])
					* mask[indx[i]];
		} else {
			if ((tid + 1) % n == 0) {    //end of a column
				sdata[tid] = (g_idata[indx[i]] - g_idata[indx[i - 1]])
						* mask[indx[i]];
			} else
...
template<class T> __global__ void derive_phasey(T *g_idata, T *g_odata,
		int *indx, T *mask, T alpha, unsigned int n, unsigned int N,
		float *fluxPerSub) {
	T *sdata = SharedMemory<T>();
	...
	if (i < N) {
		if (tid < n) {    //start of a column
			sdata[tid] = (g_idata[indx[i + n]] - g_idata[indx[i]])
					* mask[indx[i]];
		} else {
			if (tid >= n * (n - 1)) {    //end of a column
				sdata[tid] = (g_idata[indx[i]] - g_idata[indx[i - n]])
						* mask[indx[i]];
			} else
...
template<class T> void phase_reduce(int threads, int blocks, T *d_idata,
		T *d_odata, int *indx, T alpha) {
	dim3 dimBlock(threads, 1, 1);
	dim3 dimGrid(blocks, 1, 1);
...
template<class T> void phase_derive(int size, int threads, int blocks, int n,
		T *d_idata, T *d_odata, int *indx, T *mask, T alpha,
		float *fluxPerSub) {
	dim3 dimBlock(threads, 1, 1);
	dim3 dimGrid(blocks, 1, 1);
...
template void phase_derive<float>(int size, int threads, int blocks, int n,
		float *d_idata, float *d_odata, int *indx, float *mask, float alpha,
		float *fluxPerSub);

template void phase_derive<double>(int size, int threads, int blocks, int n,
		double *d_idata, double *d_odata, int *indx, double *mask, double alpha,
		float *fluxPerSub);

template<class Tout, class Tin> __global__ void pyrgetpup_krnl(Tout *g_odata,
		Tin *g_idata, Tout *offsets, Tin *pup, unsigned int n) {
...
template<class Tout, class Tin> void pyr_getpup(Tout *d_odata, Tin *d_idata,
		Tout *d_offsets, Tin *d_pup, int np, int device) {
	struct cudaDeviceProp deviceProperties;
	cudaGetDeviceProperties(&deviceProperties, device);
...
template void pyr_getpup<cuFloatComplex, float>(cuFloatComplex *d_odata,
		float *d_idata, cuFloatComplex *d_offsets, float *d_pup, int np,
		int device);
template void pyr_getpup<cuDoubleComplex, double>(cuDoubleComplex *d_odata,
		double *d_idata, cuDoubleComplex *d_offsets, double *d_pup, int np,
		int device);

template<class T> __global__ void rollmod_krnl(T *g_odata, T *g_idata,
		T *g_mask, int cx, int cy, unsigned int n, unsigned int ns,
		unsigned int nim) {
	// roll( pup * exp(i*phase) ) * offsets
...
template<class T> void pyr_rollmod(T *d_odata, T *d_idata, T *d_mask, float cx,
		float cy, int np, int ns, int device) {
	struct cudaDeviceProp deviceProperties;
	cudaGetDeviceProperties(&deviceProperties, device);
...
template void pyr_rollmod<cuFloatComplex>(cuFloatComplex *d_odata,
		cuFloatComplex*d_idata, cuFloatComplex*d_mask, float cx, float cy,
		int np, int ns, int device);
template void pyr_rollmod<cuDoubleComplex>(cuDoubleComplex *d_odata,
		cuDoubleComplex *d_idata, cuDoubleComplex *d_mask, float cx, float cy,
		int np, int ns, int device);

template<class T> __global__ void fillbinpyr_krnl(T *g_odata, T *g_idata,
		unsigned int nrebin, unsigned int n, unsigned int ns,
		unsigned int nim) {
	// bin2d(hrimg)
...
template<class T> void pyr_fillbin(T *d_odata, T *d_idata, int nrebin, int np,
...
template void pyr_fillbin<float>(float *d_odata, float *d_idata, int nrebin,
		int np, int ns, int nim, int device);
template void pyr_fillbin<double>(double *d_odata, double *d_idata, int nrebin,
		int np, int ns, int nim, int device);

template<class Tout, class Tin> __global__ void abspyr_krnl(Tout *g_odata,
		Tin *g_idata, unsigned int ns, unsigned int nim) {
	unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
	int x, y;
	...
	if (i < ns * ns) {
		for (int cpt = 0; cpt < nim; cpt++) {
			g_odata[i + cpt * ns * ns] = sqrtf(
					g_idata[i2 + cpt * ns * ns].x
							* g_idata[i2 + cpt * ns * ns].x
							+ g_idata[i2 + cpt * ns * ns].y
									* g_idata[i2 + cpt * ns * ns].y);
			g_odata[i2 + cpt * ns * ns] = sqrtf(
					g_idata[i + cpt * ns * ns].x * g_idata[i + cpt * ns * ns].x
							+ g_idata[i + cpt * ns * ns].y
									* g_idata[i + cpt * ns * ns].y);
		}
	}
}

template<class Tout, class Tin> void pyr_abs(Tout *d_odata, Tin *d_idata,
		int ns, int nim, int device) {
	struct cudaDeviceProp deviceProperties;
	cudaGetDeviceProperties(&deviceProperties, device);
...
template void pyr_abs<float, cuFloatComplex>(float *d_odata,
		cuFloatComplex *d_idata, int ns, int nim, int device);
template void pyr_abs<double, cuDoubleComplex>(double *d_odata,
		cuDoubleComplex *d_idata, int ns, int nim, int device);

template<class Tin, class Tout> __global__ void abs2pyr_krnl(Tout *g_odata,
		Tin *g_idata, Tout fact, unsigned int ns, unsigned int nim) {
	unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
	int x, y;
	...
	if (i < ns * ns / 2) {
		for (int cpt = 0; cpt < nim; cpt++) {
			...
			g_odata[i + cpt * ns * ns] += (tmp2.x * tmp2.x + tmp2.y * tmp2.y)
					* fact;
			g_odata[i2 + cpt * ns * ns] += (tmp1.x * tmp1.x + tmp1.y * tmp1.y)
					* fact;
...
template<class Tin, class Tout> void pyr_abs2(Tout *d_odata, Tin *d_idata,
		Tout fact, int ns, int nim, int device) {
	struct cudaDeviceProp deviceProperties;
	cudaGetDeviceProperties(&deviceProperties, device);
...
template void pyr_abs2<cuFloatComplex, float>(float *d_odata,
		cuFloatComplex *d_idata, float fact, int ns, int nim, int device);
template void pyr_abs2<cuDoubleComplex, double>(double *d_odata,
		cuDoubleComplex *d_idata, double fact, int ns, int nim, int device);

template<class Tout, class Tin> __global__ void submask_krnl(Tout *g_odata,
		Tin *g_mask, unsigned int n) {
	unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
...
template<class Tout, class Tin> void pyr_submask(Tout *d_odata, Tin *d_mask,
		int n, int device) {
	struct cudaDeviceProp deviceProperties;
	cudaGetDeviceProperties(&deviceProperties, device);
...
template void pyr_submask<cuFloatComplex, float>(cuFloatComplex *d_odata,
		float*d_mask, int n, int device);
template void pyr_submask<cuDoubleComplex, double>(cuDoubleComplex *d_odata,
		double *d_idata, int n, int device);

template<class Tout, class Tin> __global__ void submask3d_krnl(Tout *g_odata,
		Tin *g_mask, unsigned int n, unsigned int nim) {
	unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
...
template<class Tout, class Tin> void pyr_submask3d(Tout *d_odata, Tin *d_mask,
...
template void pyr_submask3d<cuFloatComplex, float>(cuFloatComplex *d_odata,
		float*d_mask, int n, int nim, int device);
template void pyr_submask3d<cuDoubleComplex, double>(cuDoubleComplex *d_odata,
		double *d_idata, int n, int nim, int device);

template<class T> __global__ void subsum_krnl(T *g_odata, T *g_idata,
		int *subindx, int *subindy, unsigned int ns, unsigned int nvalid,
		unsigned int nim) {
	unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
...
template<class T> void pyr_subsum(T *d_odata, T *d_idata, int *subindx,
		int *subindy, int ns, int nvalid, int nim, int device) {
	struct cudaDeviceProp deviceProperties;
	cudaGetDeviceProperties(&deviceProperties, device);
...
template<class T> __global__ void pyrfact_krnl(T *g_data, T fact,
...
__global__ void pyrfact_krnl(float *g_data, float fact1, float *fact2,
	...
	if (i < n * n) {
		for (int cpt = 0; cpt < nim; cpt++) {
			g_data[i + cpt * n * n] = g_data[i + cpt * n * n] * fact1
					/ fact2[0];
		}
	}
}
