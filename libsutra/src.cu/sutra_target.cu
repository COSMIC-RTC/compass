...
int target_texraytrace(float *d_odata, float *d_idata, int nx, int ny, int Nx,
		...
		cudaChannelFormatDesc channelDesc, int device) {
...
__device__ void generic_raytrace(float *odata, float *idata, int nx, int ny,
	...
	if ((x < nx) && (y < ny)) {
		...
		odata[tido] +=
				(wx1 * wy1 * cache[threadIdx.x + threadIdx.y * blockSize]
						+ wx2 * wy1
								* cache[threadIdx.x + 1
										+ threadIdx.y * blockSize]
						+ wx1 * wy2
								* cache[threadIdx.x
										+ (threadIdx.y + 1) * blockSize]
						+ wx2 * wy2
								* cache[threadIdx.x + 1
										+ (threadIdx.y + 1) * blockSize]);
	}
...
int target_raytrace_async(carma_streams *streams, float *d_odata,
	...
	for (int i = 0; i < nstreams; i++)
		raytrace_krnl<<<blocks, threads, smemSize, streams->get_stream(i)>>>(
				d_odata, d_idata, nx, ny, xoff, yoff, Nx, block_size,
				i * block_size);
...
int target_raytrace_async(carma_host_obj<float> *phase_telemetry,
	...
	for (int i = 0; i < nstreams; i++)
		raytrace_krnl<<<blocks, threads, smemSize,
				phase_telemetry->get_cudaStream_t(i)>>>(d_odata, d_idata, nx,
				ny, xoff, yoff, Nx, block_size, i * block_size);
