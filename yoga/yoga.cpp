...
{
	const char NKEYS=1;
	static char *knames[NKEYS+1] = { "noComputeU" };
	static long kglobs[NKEYS+1] = { 0 , -1 };
	int kiargs[NKEYS];
	yarg_kw_init(knames, kglobs, kiargs);
	int piargs[]={-1};
	int iarg=argc-1, found=0;
	while (iarg>=0) {
	    iarg = yarg_kw(iarg, kglobs, kiargs)-1;
	}
	...
	if (yarg_subroutine()) {
		...
		if (nbarg == 2) {
			if (handle_mat->type == Y_FLOAT) {
				caObjS *carma_obj_handler_mat =
						(caObjS *) (handle_mat->carma_object);
				carma_syevd(jobz,carma_obj_handler_mat, (float*) eigenvals);
			} else if (handle_mat->type == Y_DOUBLE) {
				caObjD *carma_obj_handler_mat =
						(caObjD *) (handle_mat->carma_object);
				carma_syevd(jobz,carma_obj_handler_mat, (double*) eigenvals);
			} else {
 ...
 */
{
	if (yarg_subroutine()) {
		...
		if (yType == Y_FLOAT) {
			carma_syevd_m(ngpu, 'V', (float*) mat, (float*) eigenvals, (float*) U,
					N);
		} else if (yType == Y_DOUBLE) {
			carma_syevd_m(ngpu, 'V', (double*) mat, (double*) eigenvals, (double*) U,
					N);
		} else {
 ...
 */
{
	if (yarg_subroutine()) {
		...
		int x0 = carma_out_handler->getDims(1) * (mrange[5] - 1)
				+ (mrange[1] - 1);
		//if (Ncol == 1) x0 = (mrange[1]-1);
		fillarray2d(carma_out_handler->getData(), carma_in_handler->getData(),
				x0, Ncol, carma_out_handler->getDims(1), Nlig * Ncol);
	} else
...
void Y_yoga_plusai(int argc) {
	if (yarg_subroutine()) {
		...
		if (handle_out->type == Y_FLOAT) {
			caObjS *carma_in_handler = (caObjS *) (handle_in->carma_object);
			caObjS *carma_out_handler = (caObjS *) (handle_out->carma_object);
			carma_plusai(carma_out_handler->getData(),
					carma_in_handler->getData(), idx, sgn,
