 ...
 */
{
  ...
  if (yarg_subroutine()) {
    ...
    if (nbarg == 2) {
      if (handle_mat->type == Y_FLOAT) {
        caObjS *carma_obj_handler_mat = (caObjS *) (handle_mat->carma_object);
        carma_syevd<float>(jobz, ntot, *carma_obj_handler_mat, (float*) eigenvals);
      } else if (handle_mat->type == Y_DOUBLE) {
        caObjD *carma_obj_handler_mat = (caObjD *) (handle_mat->carma_object);
        carma_syevd<double>(jobz, ntot, *carma_obj_handler_mat, (double*) eigenvals);
      } else {
      ...
      if (handle_mat->type == Y_FLOAT) {
        ...
        carma_syevd<float>(jobz, ntot, *carma_obj_handler_U, (float*) eigenvals);
      } else if (handle_mat->type == Y_DOUBLE) {
        caObjD *carma_obj_handler_mat = (caObjD *) (handle_mat->carma_object);
        caObjD *carma_obj_handler_U = (caObjD *) (handle_U->carma_object);
        carma_obj_handler_U->copy(carma_obj_handler_mat, 1, 1);
        carma_syevd<double>(jobz, ntot, *carma_obj_handler_U, (double*) eigenvals);
      } else {
        y_error("carma_syevd not implemented for this type");
      }
    } else {
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if (handle_mat->type == Y_FLOAT) {
      caObjS *carma_obj_handler_mat = (caObjS *) (handle_mat->carma_object);
      carma_potri_m<float>(ngpus, dims[1], (float*) mat, *carma_obj_handler_mat);
    } else if (handle_mat->type == Y_DOUBLE) {
      caObjD *carma_obj_handler_mat = (caObjD *) (handle_mat->carma_object);
      carma_potri_m<double>(ngpus, dims[1], (double*) mat, *carma_obj_handler_mat);
    } else {
      y_error("carma_potri not implemented for this type");
    }
  } else {
