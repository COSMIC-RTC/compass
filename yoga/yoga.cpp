	...
	if (yarg_subroutine()) {
		...
		if (handle_mat->type == Y_FLOAT) {
			...
			carma_obj_handler_vecty->gemv('n', alpha, carma_obj_handler_mat,
					carma_obj_handler_mat->getDims(1), carma_obj_handler_vectx,
					1, beta, 1);
			// here 1 is the increment
		}
		...
		if (handle_mat->type == Y_FLOAT) {
			...
			handle_vecty->carma_object = new caObjS(context_handle,
					dims_data_y);
			caObjS *carma_obj_handler_vecty =
					(caObjS *) (handle_vecty->carma_object);
			...
			dims_data_y[0] = 1;
			dims_data_y[1] = carma_obj_handler_mat->getDims(1);
			handle_vecty->carma_object = new caObjD(context_handle,
					dims_data_y);
			caObjD *carma_obj_handler_vecty =
					(caObjD *) (handle_vecty->carma_object);
 ...
 */
{
	if (yarg_subroutine()) {
		...
		if (handle_vectx->type == Y_FLOAT) {
			...
			handle_matA->carma_object = new caObjS(context_handle,
					dims_data_mat);
			caObjS *carma_obj_handler_matA =
					(caObjS *) (handle_matA->carma_object);
			...
			dims_data_mat[1] = carma_obj_handler_vecty->getDims(1);
			dims_data_mat[2] = carma_obj_handler_vectx->getDims(1);
			handle_matA->carma_object = new caObjD(context_handle,
					dims_data_mat);
			caObjD *carma_obj_handler_matA =
					(caObjD *) (handle_matA->carma_object);
 ...
 */
{
	if (yarg_subroutine()) {
		...
		if (handle_matC->type == Y_FLOAT) {
			...
			caObjS *carma_obj_handler_matA =
					(caObjS *) (handle_matA->carma_object);
			caObjS *carma_obj_handler_matB =
					(caObjS *) (handle_matB->carma_object);
			caObjS *carma_obj_handler_matC =
					(caObjS *) (handle_matC->carma_object);

			carma_obj_handler_matC->gemm(opA, opB, alpha,
					carma_obj_handler_matA, carma_obj_handler_matA->getDims(1),
					carma_obj_handler_matB, carma_obj_handler_matB->getDims(1),
					beta, carma_obj_handler_matC->getDims(1));
		}
		if (handle_matC->type == Y_DOUBLE) {
			...
			caObjD *carma_obj_handler_matA =
					(caObjD *) (handle_matA->carma_object);
			caObjD *carma_obj_handler_matB =
					(caObjD *) (handle_matB->carma_object);
		...
		if (handle_matA->type == Y_FLOAT) {
			...
			handle_matC->carma_object = new caObjS(context_handle,
					dims_data_mat);
			caObjS *carma_obj_handler_matC =
					(caObjS *) (handle_matC->carma_object);
			...
			handle_matC->carma_object = new caObjD(context_handle,
					dims_data_mat);
			caObjD *carma_obj_handler_matC =
					(caObjD *) (handle_matC->carma_object);
			carma_obj_handler_matC->gemm(opA, opB, alpha,
 ...
 */
{
	if (yarg_subroutine()) {
		...
		if (handle_matC->type == Y_FLOAT) {
			...
			carma_obj_handler_matC->syrk(uplo, opA, alpha,
					carma_obj_handler_matA, carma_obj_handler_matA->getDims(2),
					beta, carma_obj_handler_matC->getDims(1));
		}
		if (handle_matC->type == Y_DOUBLE) {
			double alpha = 1.0f;
 ...
 */
{
	if (yarg_subroutine()) {
		...
		if (handle_matC->type == Y_FLOAT) {
			...
			carma_obj_handler_matC->geam(opA, opB, alpha,
					carma_obj_handler_matA, carma_obj_handler_matA->getDims(1),
					beta, carma_obj_handler_matB,
	...
	if (yarg_subroutine()) {
		...
		if (handle_matA->type == Y_FLOAT) {
			...
			carma_obj_handler_matC->dgmm(side, carma_obj_handler_matA,
					carma_obj_handler_matA->getDims(1), carma_obj_handler_vectx,
					1, carma_obj_handler_matC->getDims(1));
			// here 1 is the increment
		}
	...
	if (yarg_subroutine()) {
		...
		if (argc > 1) {
			...
			if (handle_src->type == Y_SCOMPLEX) {
				if (handle_dest->type == Y_SCOMPLEX) {
					...
					carma_fft<cuFloatComplex, cuFloatComplex>(
							*carma_obj_handler_src, *carma_obj_handler_dest,
							dir, *carma_obj_handler_src->getPlan());
				...
				if (handle_dest->type == Y_COMPLEX) {
					...
					if (*carma_obj_handler_src->getPlan() == 0L) {
						...
						carma_initfft<cufftDoubleReal, cuDoubleComplex>(
								carma_obj_handler_src->getDims(),
								carma_obj_handler_src->getPlan(),
								carma_select_plan<cufftDoubleReal,
										cuDoubleComplex>());
					}
				...
				if (handle_dest->type == Y_COMPLEX) {
					...
					carma_fft<cuDoubleComplex, cuDoubleComplex>(
							*carma_obj_handler_src, *carma_obj_handler_dest,
							dir, *carma_obj_handler_src->getPlan());
			...
			if (handle_src->type == Y_SCOMPLEX) {
				...
				carma_fft<cuFloatComplex, cuFloatComplex>(
						*carma_obj_handler_src, *carma_obj_handler_src, dir,
						*carma_obj_handler_src->getPlan());
			} else if (handle_src->type == Y_COMPLEX) {
				caObjZ *carma_obj_handler_src =
						(caObjZ *) (handle_src->carma_object);
		...
		if ((handle_src->type == Y_FLOAT) || (handle_src->type == Y_SCOMPLEX)) {
			if (handle_src->type == Y_FLOAT) {
				...
				caObjC *carma_obj_handler = (caObjC *) (handle->carma_object);
				carma_fft<cuFloatComplex, cuFloatComplex>(
						*carma_obj_handler_src, *carma_obj_handler, dir,
						*carma_obj_handler_src->getPlan());
			}
		} else if ((handle_src->type == Y_DOUBLE)
 ...
 */
{
	if (yarg_subroutine()) {
		...
		if ((handle_res->type == Y_FLOAT) && (handle_im->type == Y_FLOAT)
			...
			caObjS *carma_pdata_handler =
					(caObjS *) (handle_pdata->carma_object);
			caObjC *carma_pspec_handler =
					(caObjC *) (handle_pspec->carma_object);
 ...
 */
{
	...
	try {
		if (handler->type == Y_FLOAT) {
			...
			carma_host_obj<unsigned int> *carma_host_obj_handler =
					(carma_host_obj<unsigned int> *) (handler->carma_host_object);
			delete carma_host_obj_handler;
		} else if (handler->type == Y_COMPLEX) {
	...
	try {
		if (handler->type == Y_FLOAT) {
			...
			mystr << "  Allocation: " << carma_host_obj_handler->getMetAlloc()
					<< endl;
			mystr << "  nb streams: "
					<< carma_host_obj_handler->get_nbStreams();
			y_print(mystr.str().c_str(), 1);
			mystr.str("");
	...
	try {
		...
		if (yType == Y_OPAQUE) { // Copy constructor
			...
			if (handle->type == Y_FLOAT) {
				...
				carma_host_obj<int> *carma_host_obj_handler = (carma_host_obj<
						int> *) (handle_obj->carma_host_object);
			...
			if (oType == Y_FLOAT) {
				...
				if (yType == Y_STRING) {
					handle->carma_host_object =
							new carma_host_obj<unsigned int>((long*) data_input,
									mallocType);
				} else {
					handle->carma_host_object =
							new carma_host_obj<unsigned int>(dims,
									(uint*) data_input, mallocType);
				}
			} else if (oType == Y_COMPLEX) {
				if (yType == Y_STRING) {
					handle->carma_host_object = new carma_host_obj<
							cuDoubleComplex>((long*) data_input, mallocType);
				} else {
					handle->carma_host_object = new carma_host_obj<
							cuDoubleComplex>(dims,
							(cuDoubleComplex*) data_input, mallocType);
				}
			} else if (oType == Y_SCOMPLEX) {
				if (yType == Y_STRING) {
					handle->carma_host_object = new carma_host_obj<
							cuFloatComplex>((long*) data_input, mallocType);
				} else {
					stringstream buf;
					buf << "Type not supported in yorick in " << __FILE__ << "@"
							<< __LINE__ << endl;
 ...
 */
{
	if (yarg_subroutine()) {
		...
		if (handle_mat->type == Y_FLOAT) {
			...
			carma_svd<float>(carma_obj_handler_mat, carma_obj_handler_eigenvals,
			...
			carma_svd<double>(carma_obj_handler_mat,
					carma_obj_handler_eigenvals, carma_obj_handler_VT,
					carma_obj_handler_U);
		} else {
 ...
 */
{
	...
	if (kiargs[0] >= 0) {
		...
		if (ygets_l(kiargs[0]) == 1)
			//y_print("eigenveectors are not computed",1);
			jobz = 'N';
		nbarg -= 2;
	}
	...
	if (yarg_subroutine()) {
		...
		if (nbarg == 2) {
			if (handle_mat->type == Y_FLOAT) {
				...
				carma_syevd<float>(jobz, h_mat, h_eigenvals);
			} else if (handle_mat->type == Y_DOUBLE) {
				carma_host_obj<double> *h_mat =
						(carma_host_obj<double> *) (handle_mat->carma_host_object);
				carma_host_obj<double> *h_eigenvals =
						(carma_host_obj<double> *) (handle_eigenvals->carma_host_object);
				carma_syevd<double>(jobz, h_mat, h_eigenvals);
			} else {
			...
			if (handle_mat->type == Y_FLOAT) {
				...
				carma_host_obj<float> *h_eigenvals =
						(carma_host_obj<float> *) (handle_eigenvals->carma_host_object);
				carma_syevd<float>(jobz, h_U, h_eigenvals);
			} else if (handle_mat->type == Y_DOUBLE) {
				carma_host_obj<double> *h_mat =
						(carma_host_obj<double> *) (handle_mat->carma_host_object);
				carma_host_obj<double> *h_U =
						(carma_host_obj<double> *) (handle_U->carma_host_object);
				h_U->fill_from(*h_mat);
				carma_host_obj<double> *h_eigenvals =
						(carma_host_obj<double> *) (handle_eigenvals->carma_host_object);
				carma_syevd<double>(jobz, h_U, h_eigenvals);
			} else {
				y_error("carma_syevd not implemented for this type");
			}
		} else {
	...
	if (yarg_subroutine()) {
		...
		if (nbarg == 2) {
			if (handle_mat->type == Y_FLOAT) {
				caObjS *carma_obj_handler_mat =
						(caObjS *) (handle_mat->carma_object);
				carma_host_obj<float> *carma_host_obj_eigenvals =
						(carma_host_obj<float> *) (handle_eigenvals->carma_host_object);
				carma_syevd<float, 1>(jobz, carma_obj_handler_mat, carma_host_obj_eigenvals);
			} else if (handle_mat->type == Y_DOUBLE) {
				caObjD *carma_obj_handler_mat =
						(caObjD *) (handle_mat->carma_object);
				carma_host_obj<double> *carma_host_obj_eigenvals =
						(carma_host_obj<double> *) (handle_eigenvals->carma_host_object);
				carma_syevd<double, 1>(jobz, carma_obj_handler_mat, carma_host_obj_eigenvals);
			} else {
				y_error("carma_syevd not implemented for this type");
			}
		} else if (nbarg == 3) {
			...
			if (handle_mat->type == Y_FLOAT) {
				caObjS *carma_obj_handler_mat =
						(caObjS *) (handle_mat->carma_object);
				caObjS *carma_obj_handler_U =
						(caObjS *) (handle_U->carma_object);
				carma_obj_handler_U->copy(carma_obj_handler_mat, 1, 1);
				carma_host_obj<float> *carma_host_obj_eigenvals =
						(carma_host_obj<float> *) (handle_eigenvals->carma_host_object);
				carma_syevd<float, 1>(jobz, carma_obj_handler_U,
						carma_host_obj_eigenvals);
			} else if (handle_mat->type == Y_DOUBLE) {
				caObjD *carma_obj_handler_mat =
						(caObjD *) (handle_mat->carma_object);
				caObjD *carma_obj_handler_U =
						(caObjD *) (handle_U->carma_object);
				carma_obj_handler_U->copy(carma_obj_handler_mat, 1, 1);
				carma_host_obj<double> *carma_host_obj_eigenvals =
						(carma_host_obj<double> *) (handle_eigenvals->carma_host_object);
				carma_syevd<double, 1>(jobz, carma_obj_handler_U,
						carma_host_obj_eigenvals);
			} else {
				y_error("carma_syevd not implemented for this type");
			}
 ...
 */
{
	...
	if (yarg_subroutine()) {
		...
		yHostObj_struct *handle_mat = (yHostObj_struct *) yget_obj(argc - 2, &yHostObj);
		yHostObj_struct *handle_eigenvals = (yHostObj_struct *) yget_obj(argc - 3, &yHostObj);
		yHostObj_struct *handle_U = (yHostObj_struct *) yget_obj(argc - 4, &yHostObj);
		if (handle_mat->type == Y_FLOAT) {
			carma_host_obj<float> *h_mat =
					(carma_host_obj<float> *) (handle_mat->carma_host_object);
			carma_host_obj<float> *h_eigenvals =
			...
			carma_host_obj<float> *h_U =
					(carma_host_obj<float> *) (handle_U->carma_host_object);
			carma_syevd_m<float>(ngpu, jobz, h_mat, h_eigenvals, h_U);
		} else if (handle_mat->type == Y_DOUBLE) {
			carma_host_obj<double> *h_mat =
					(carma_host_obj<double> *) (handle_mat->carma_host_object);
			carma_host_obj<double> *h_eigenvals =
			...
			carma_host_obj<double> *h_U =
					(carma_host_obj<double> *) (handle_U->carma_host_object);
			carma_syevd_m<double>(ngpu, jobz, h_mat, h_eigenvals, h_U);
		} else {
 ...
 */
{
	if (yarg_subroutine()) {
		...
		if (handle_mat->type == Y_FLOAT) {
			caObjS *carma_obj_handler_mat =
					(caObjS *) (handle_mat->carma_object);
			carma_getri<float>(carma_obj_handler_mat);
		} else if (handle_mat->type == Y_DOUBLE) {
			caObjD *carma_obj_handler_mat =
					(caObjD *) (handle_mat->carma_object);
			carma_getri<double>(carma_obj_handler_mat);
		} else {
...
void Y_yoga_potri_cpu(int argc)
/** @brief computes the inverse of a real symmetric positive definite matrix using the Cholesky factorization
 *  @param[in] argc : command line arguments
 *    - first  : the yoga_host_obj to be inversed (warning this object will be replaced by its inverse)
 *  only floating point matrices can be decomposed (single or double precision)
 */
{
	if (yarg_subroutine()) {
		...
		if (handle_mat->type == Y_FLOAT) {
			carma_host_obj<float> *h_mat =
					(carma_host_obj<float> *) (handle_mat->carma_host_object);
			carma_potri<float>(h_mat);
		} else if (handle_mat->type == Y_DOUBLE) {
			carma_host_obj<double> *h_mat =
					(carma_host_obj<double> *) (handle_mat->carma_host_object);
			carma_potri<double>(h_mat);
		} else {
 ...
 */
{
	if (yarg_subroutine()) {
		...
		if (handle_mat->type == Y_FLOAT) {
			...
			carma_host_obj<double> *carma_obj_handler_mat = (carma_host_obj<
					double> *) (handle_mat->carma_host_object);

			carma_host_obj<double> *carma_obj_handler_eigenvals =
					(carma_host_obj<double> *) (handle_eigenvals->carma_host_object);

			carma_host_obj<double> *carma_obj_handler_U =
					(carma_host_obj<double> *) (handle_U->carma_host_object);

			carma_host_obj<double> *carma_obj_handler_VT = (carma_host_obj<
					double> *) (handle_VT->carma_host_object);
 ...
 */
{
	if (yarg_subroutine())
		...
		cudaMemcpy(&res, &(d_res[pos - 1]), sizeof(float),
