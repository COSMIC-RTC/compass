...
void context_free(void *obj) {
  ...
  try {
    carma_context *context_obj_handler = (carma_context *) (handler->carma_context);
 ...
 */
{
  ...
  cout << "Dev Id" << " | " << "multiproc" << " | " << "cores" << " | " << "compute" << " | " << "Perf (GFlops)" << endl;
  for (size_t idx = 0; idx < nDevice; idx++) {
    cout << ((idx == activeDevice) ? "<U>" : "   ") << setw(3) << idx << " | " << setw(9) << context_handler->get_device(idx)->get_properties().multiProcessorCount << " | " << setw(5)
        << context_handler->get_device(idx)->get_sm_per_multiproc() * context_handler->get_device(idx)->get_properties().multiProcessorCount << " | " << setw(5)
        << context_handler->get_device(idx)->get_properties().major << "." << context_handler->get_device(idx)->get_properties().minor << " | "
...
{
  try {
    context_struct *handle = (context_struct *) ypush_obj(&yContext, sizeof(context_struct));
    ...
    stringstream buf;
    buf << "unknown error with carma_context construction in " << __FILE__ << "@" << __LINE__ << endl;
 ...
 */
{
  try {
    ...
    stringstream buf;
    buf << "unknown error with carma_context in " << __FILE__ << "@" << __LINE__ << endl;
    y_error(buf.str().c_str());
  }
}
 ...
 */
{
  ...
  try {
    if (handler->type == Y_FLOAT) {
      carma_obj<float> *carma_obj_handler = (carma_obj<float> *) (handler->carma_object);
      ...
      carma_obj<double> *carma_obj_handler = (carma_obj<double> *) (handler->carma_object);
      mystr << "Carma Object : " << endl;
      ...
      carma_obj<int> *carma_obj_handler = (carma_obj<int> *) (handler->carma_object);
      mystr << "Carma Object : " << endl;
      mystr << "type : int" << endl;
      long *dims = carma_obj_handler->getDims();
      mystr << "Dimensions : " << dims[1];
      ...
      carma_obj<unsigned int> *carma_obj_handler = (carma_obj<unsigned int> *) (handler->carma_object);
      mystr << "Carma Object : " << endl;
      ...
      carma_obj<cuDoubleComplex> *carma_obj_handler = (carma_obj<cuDoubleComplex> *) (handler->carma_object);
      mystr << "Carma Object : " << endl;
 ...
 */
{
  ...
  try {
    if (handler->type == Y_FLOAT) {
      ...
      unsigned int *data = (unsigned int *) ypush_i(carma_obj_handler->getDims());
      carma_obj_handler->device2host(data);
    } else if (handler->type == Y_COMPLEX) {
      caObjZ *carma_obj_handler = (caObjZ *) (handler->carma_object);
      cuDoubleComplex *data = (cuDoubleComplex *) ypush_z(carma_obj_handler->getDims());
      carma_obj_handler->device2host(data);
    } else if (handler->type == Y_SCOMPLEX) {
 ...
 */
{
  ...
  try {
    ...
    if (yType == Y_POINTER) {
      ...
      if (oType == Y_FLOAT) {
        if (yType == Y_STRING) { // based on the input type and dimensions array
          ...
          handle->carma_object = new caObjS(context_handle, dims, (float*) data_input);
        }
      } else if (oType == Y_DOUBLE) {
        if (yType == Y_STRING) {
          handle->carma_object = new caObjD(context_handle, (long*) data_input);
        } else {
          handle->carma_object = new caObjD(context_handle, dims, (double*) data_input);
        }
      } else if (oType == Y_INT) {
        if (yType == Y_STRING) {
          handle->carma_object = new caObjI(context_handle, (long*) data_input);
        } else {
          handle->carma_object = new caObjI(context_handle, dims, (int*) data_input);
        }
      } else if (oType == Y_SHORT) {
        if (yType == Y_STRING) {
          handle->carma_object = new caObjUI(context_handle, (long*) data_input);
        } else {
          handle->carma_object = new caObjUI(context_handle, dims, (uint*) data_input);
        }
      } else if (oType == Y_COMPLEX) {
        if (yType == Y_STRING) {
          handle->carma_object = new caObjZ(context_handle, (long*) data_input);
        } else {
          handle->carma_object = new caObjZ(context_handle, dims, (cuDoubleComplex*) data_input);
        }
        ...
        if (yType == Y_STRING) {
          ...
          stringstream buf;
          buf << "Type not supported in yorick in " << __FILE__ << "@" << __LINE__ << endl;
          throw buf.str();
    ...
    stringstream buf;
    buf << "unknown error with yObj construction in " << __FILE__ << "@" << __LINE__ << endl;
    y_error(buf.str().c_str());
  }
}
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    yObj_struct *handle_dest = (yObj_struct *) ypush_obj(&yObj, sizeof(yObj_struct));
    handle_dest->device = handle_src->device;
    if (handle_src->type == Y_FLOAT) {
      float alpha = ygets_f(argc - 2);
      handle_dest->type = handle_src->type;
      caObjS *carma_obj_handler_src = (caObjS *) (handle_src->carma_object);
      handle_dest->carma_object = new caObjS(context_handle, carma_obj_handler_src->getDims());
      caObjS *carma_obj_handler_dest = (caObjS *) (handle_dest->carma_object);
      carma_obj_handler_dest->axpy(alpha, carma_obj_handler_src, 1, 1);
    } else if (handle_src->type == Y_DOUBLE) {
      double alpha = ygets_d(argc - 2);
      handle_dest->type = handle_src->type;
      caObjD *carma_obj_handler_src = (caObjD *) (handle_src->carma_object);
      handle_dest->carma_object = new caObjD(context_handle, carma_obj_handler_src->getDims());
      caObjD *carma_obj_handler_dest = (caObjD *) (handle_dest->carma_object);
      carma_obj_handler_dest->axpy(alpha, carma_obj_handler_src, 1, 1);
    }
  }
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if ((handle_vecty->device != handle_mat->device) || (handle_vecty->device != handle_vectx->device) || (handle_mat->device != handle_vectx->device))
    ...
    if (handle_mat->type == Y_FLOAT) {
      ...
      if (argc > 4) {
        ...
        beta = 0.0f;
      carma_obj_handler_vecty->gemv('n', alpha, carma_obj_handler_mat, carma_obj_handler_mat->getDims(1), carma_obj_handler_vectx, 1, beta, 1);
      // here 1 is the increment
    }
    ...
    carma_context *context_handle = _getCurrentContext();
    context_handle->set_activeDevice(handle_mat->device);
    yObj_struct *handle_vecty = (yObj_struct *) ypush_obj(&yObj, sizeof(yObj_struct));
    ...
    if (handle_mat->type == Y_FLOAT) {
      ...
      caObjS *carma_obj_handler_vecty = (caObjS *) (handle_vecty->carma_object);
      carma_obj_handler_vecty->gemv('n', 1.0f, carma_obj_handler_mat, carma_obj_handler_mat->getDims(1), carma_obj_handler_vectx, 1, 0.0f, 1);
      // here 1 is the increment
      ...
      caObjD *carma_obj_handler_vecty = (caObjD *) (handle_vecty->carma_object);
      carma_obj_handler_vecty->gemv('n', 1.0, carma_obj_handler_mat, carma_obj_handler_mat->getDims(1), carma_obj_handler_vectx, 1, 0.0, 1);
      // here 1 is the increment
    }
  }
}
  ...
  if (yarg_subroutine()) {
    ...
    if (handle_mat->type == Y_FLOAT) {
      ...
      if (argc > 4) {
        ...
        beta = 0.0f;
      carma_obj_handler_vecty->symv(CUBLAS_FILL_MODE_LOWER, alpha, carma_obj_handler_mat, carma_obj_handler_mat->getDims(1), carma_obj_handler_vectx, 1, beta, 1);
      // here 1 is the increment
    }
    if (handle_mat->type == Y_DOUBLE) {
      caObjD *carma_obj_handler_mat = (caObjD *) (handle_mat->carma_object);
    ...
    if (handle_mat->type == Y_FLOAT) {
      ...
      handle_vecty->carma_object = new caObjS(context_handle, dims_data_y);
      caObjS *carma_obj_handler_vecty = (caObjS *) (handle_vecty->carma_object);
      carma_obj_handler_vecty->symv(CUBLAS_FILL_MODE_LOWER, 1.0f, carma_obj_handler_mat, carma_obj_handler_mat->getDims(1), carma_obj_handler_vectx, 1, 0.0f, 1);
      // here 1 is the increment
    } else if (handle_mat->type == Y_DOUBLE) {
      handle_vecty->type = handle_vectx->type;
      caObjD *carma_obj_handler_mat = (caObjD *) (handle_mat->carma_object);
      ...
      handle_vecty->carma_object = new caObjD(context_handle, dims_data_y);
      caObjD *carma_obj_handler_vecty = (caObjD *) (handle_vecty->carma_object);
      carma_obj_handler_vecty->symv(CUBLAS_FILL_MODE_LOWER, 1.0, carma_obj_handler_mat, carma_obj_handler_mat->getDims(1), carma_obj_handler_vectx, 1, 0.0, 1);
      // here 1 is the increment
    }
  }
}
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if (handle_mat->type == Y_FLOAT) {
      ...
      caObjS *carma_obj_handler_vectx = (caObjS *) (handle_vectx->carma_object);
      caObjS *carma_obj_handler_vecty = (caObjS *) (handle_vecty->carma_object);
      carma_obj_handler_mat->ger(1.0f, carma_obj_handler_vectx, 1, carma_obj_handler_vecty, 1, carma_obj_handler_mat->getDims(1));
      // here 1 is the increment
    }
    if (handle_mat->type == Y_DOUBLE) {
      caObjD *carma_obj_handler_mat = (caObjD *) (handle_mat->carma_object);
      caObjD *carma_obj_handler_vectx = (caObjD *) (handle_vectx->carma_object);
      caObjD *carma_obj_handler_vecty = (caObjD *) (handle_vecty->carma_object);
      carma_obj_handler_mat->ger(1.0, carma_obj_handler_vectx, 1, carma_obj_handler_vecty, 1, carma_obj_handler_mat->getDims(1));
      // here 1 is the increment
    }
  } else {
    // called as a function : need to allocate space
    ...
    yObj_struct *handle_matA = (yObj_struct *) ypush_obj(&yObj, sizeof(yObj_struct));
    ...
    if (handle_vectx->type == Y_FLOAT) {
      ...
      carma_obj_handler_matA->ger(1.0f, carma_obj_handler_vectx, 1, carma_obj_handler_vecty, 1, carma_obj_handler_matA->getDims(1));
      // here 1 is the increment
      ...
      carma_obj_handler_matA->ger(1.0, carma_obj_handler_vectx, 1, carma_obj_handler_vecty, 1, carma_obj_handler_matA->getDims(1));
      // here 1 is the increment
    }
  }
}
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if ((handle_matA->device != handle_matB->device) || (handle_matA->device != handle_matC->device) || (handle_matB->device != handle_matC->device))
    ...
    if (handle_matC->type == Y_FLOAT) {
      ...
      carma_obj_handler_matC->gemm(opA, opB, alpha, carma_obj_handler_matA, carma_obj_handler_matA->getDims(1), carma_obj_handler_matB, carma_obj_handler_matB->getDims(1), beta,
    ...
    if (handle_matA->type == Y_FLOAT) {
      ...
      yObj_struct *handle_matC = (yObj_struct *) ypush_obj(&yObj, sizeof(yObj_struct));
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if (handle_matC->type == Y_FLOAT) {
      ...
      carma_obj_handler_matC->symm(side, CUBLAS_FILL_MODE_LOWER, alpha, carma_obj_handler_matA, carma_obj_handler_matA->getDims(1), carma_obj_handler_matB, carma_obj_handler_matB->getDims(1), beta,
          carma_obj_handler_matC->getDims(1));
    }
    if (handle_matC->type == Y_DOUBLE) {
      double alpha = 1.0;
    ...
    if (handle_matA->type == Y_FLOAT) {
      ...
      carma_obj_handler_matC->gemm(side, CUBLAS_FILL_MODE_LOWER, alpha, carma_obj_handler_matA, carma_obj_handler_matA->getDims(1), carma_obj_handler_matB, carma_obj_handler_matB->getDims(1), beta,
          carma_obj_handler_matC->getDims(1));
    } else if (handle_matA->type == Y_DOUBLE) {
      double alpha = 1.0;
      if (argc > 3)
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if (handle_matC->type == Y_FLOAT) {
      ...
      carma_obj_handler_matC->syrk(uplo, opA, alpha, carma_obj_handler_matA, carma_obj_handler_matA->getDims(2), beta, carma_obj_handler_matC->getDims(1));
    }
    if (handle_matC->type == Y_DOUBLE) {
      double alpha = 1.0f;
      if (argc > 3)
      ...
      carma_obj_handler_matC->syrk(CUBLAS_FILL_MODE_UPPER, opA, alpha, carma_obj_handler_matA, carma_obj_handler_matA->getDims(1), beta, carma_obj_handler_matC->getDims(1));
    }
  } else {
    // called as a function : need to allocate space
    yObj_struct *handle_matA = (yObj_struct *) yget_obj(argc - 1, &yObj);
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if (handle_matC->type == Y_FLOAT) {
      ...
      carma_obj_handler_matC->syrkx(CUBLAS_FILL_MODE_UPPER, opA, alpha, carma_obj_handler_matA, carma_obj_handler_matA->getDims(1), carma_obj_handler_matB, carma_obj_handler_matB->getDims(1), beta,
          carma_obj_handler_matC->getDims(1));
    }
    if (handle_matC->type == Y_DOUBLE) {
      double alpha = 1.0;
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if (handle_matC->type == Y_FLOAT) {
      ...
      carma_obj_handler_matC->geam(opA, opB, alpha, carma_obj_handler_matA, carma_obj_handler_matA->getDims(1), beta, carma_obj_handler_matB, carma_obj_handler_matB->getDims(1),
          carma_obj_handler_matC->getDims(1));
    }
    if (handle_matC->type == Y_DOUBLE) {
      double alpha = 1.0;
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    yObj_struct *handle_matA = (yObj_struct *) yget_obj(argc - 2, &yObj);
    yObj_struct *handle_vectx = (yObj_struct *) yget_obj(argc - 3, &yObj);
    if ((handle_vectx->device != handle_matA->device) || (handle_matC->device != handle_vectx->device) || (handle_matA->device != handle_matC->device))
    ...
    if (handle_matA->type == Y_FLOAT) {
      ...
      carma_obj_handler_matC->dgmm(side, carma_obj_handler_matA, carma_obj_handler_matA->getDims(1), carma_obj_handler_vectx, 1, carma_obj_handler_matC->getDims(1));
      // here 1 is the increment
    }
  ...
  if (yarg_subroutine()) {
    ...
    if (argc > 1) {
      ...
      if (handle_src->type == Y_FLOAT) {
        if (handle_dest->type != Y_SCOMPLEX)
          ...
          caObjS *carma_obj_handler_src = (caObjS *) (handle_src->carma_object);
          caObjC *carma_obj_handler_dest = (caObjC *) (handle_dest->carma_object);
          ...
          if (*carma_obj_handler_src->getPlan() == 0L) {
            ...
            if (carma_obj_handler_src->getDims()[0] == 2)
              ...
              cufftSafeCall(cufftPlan2d(plan, carma_obj_handler_src->getDims()[1], carma_obj_handler_src->getDims()[2], carma_select_plan<float, cuFloatComplex>()));
            else
            /* Create a 3D FFT plan. */{
              int mdims[2];
              mdims[0] = (int) (carma_obj_handler_src->getDims()[1]);
              mdims[1] = (int) (carma_obj_handler_src->getDims()[2]);
              cufftSafeCall(cufftPlanMany(plan, 2 ,mdims,NULL,1,0,NULL,1,0, carma_select_plan<float, cuFloatComplex>(), (int)(carma_obj_handler_src->getDims()[3])));
            }
            /*
          ...
          carma_fft(carma_obj_handler_src->getData(), carma_obj_handler_dest->getData(), dir, *carma_obj_handler_src->getPlan());
        }
      }
      if (handle_src->type == Y_SCOMPLEX) {
        if (handle_dest->type == Y_SCOMPLEX) {
          ...
          if (*carma_obj_handler_src->getPlan() == 0L) {
            ...
            if (carma_obj_handler_src->getDims()[0] == 2)
              /* Create a 2D FFT plan. */
              cufftSafeCall(cufftPlan2d(plan, carma_obj_handler_src->getDims()[1], carma_obj_handler_src->getDims()[2], carma_select_plan<cuFloatComplex, cuFloatComplex>()));
            else
            /* Create a 3D FFT plan. */{
              int mdims[2];
              mdims[0] = (int) (carma_obj_handler_src->getDims()[1]);
              mdims[1] = (int) (carma_obj_handler_src->getDims()[2]);
              cufftSafeCall(cufftPlanMany(plan, 2 ,mdims,NULL,1,0,NULL,1,0, carma_select_plan<cuFloatComplex, cuFloatComplex>(), (int)(carma_obj_handler_src->getDims()[3])));
            }
            /*
          ...
          if (*carma_obj_handler_src->getPlan() == 0L) {
            ...
            if (carma_obj_handler_src->getDims()[0] == 2)
              /* Create a 2D FFT plan. */
              cufftSafeCall(cufftPlan2d(plan, carma_obj_handler_src->getDims()[1], carma_obj_handler_src->getDims()[2], carma_select_plan<cuFloatComplex, float>()));
            else
            /* Create a 3D FFT plan. */{
              int mdims[2];
              mdims[0] = (int) (carma_obj_handler_src->getDims()[1]);
              mdims[1] = (int) (carma_obj_handler_src->getDims()[2]);
              cufftSafeCall(cufftPlanMany(plan, 2 ,mdims,NULL,1,0,NULL,1,0, carma_select_plan<cuFloatComplex, float>(), (int)(carma_obj_handler_src->getDims()[3])));
            }
            /*
        ...
        if (handle_dest->type == Y_COMPLEX) {
          caObjD *carma_obj_handler_src = (caObjD *) (handle_src->carma_object);
          caObjZ *carma_obj_handler_dest = (caObjZ *) (handle_dest->carma_object);
          if (*carma_obj_handler_src->getPlan() == 0L) {
            cufftHandle *plan = carma_obj_handler_src->getPlan(); ///< FFT plan
            if (carma_obj_handler_src->getDims()[0] == 2)
              /* Create a 2D FFT plan. */
        ...
        if (handle_dest->type == Y_COMPLEX) {
          ...
          if (*carma_obj_handler_src->getPlan() == 0L) {
            ...
            if (carma_obj_handler_src->getDims()[0] == 2)
              /* Create a 2D FFT plan. */
              cufftSafeCall(cufftPlan2d(plan, carma_obj_handler_src->getDims()[1], carma_obj_handler_src->getDims()[2], carma_select_plan<cuDoubleComplex, cuDoubleComplex>()));
            else
            /* Create a 3D FFT plan. */{
              int mdims[2];
              mdims[0] = (int) (carma_obj_handler_src->getDims()[1]);
              mdims[1] = (int) (carma_obj_handler_src->getDims()[2]);
              cufftSafeCall(cufftPlanMany(plan, 2 ,mdims,NULL,1,0,NULL,1,0, carma_select_plan<cuDoubleComplex, cuDoubleComplex>(), (int)(carma_obj_handler_src->getDims()[3])));
            }
            /*
          ...
          if (*carma_obj_handler_src->getPlan() == 0L) {
            ...
            if (carma_obj_handler_src->getDims()[0] == 2)
              /* Create a 2D FFT plan. */
              cufftSafeCall(cufftPlan2d(plan, carma_obj_handler_src->getDims()[1], carma_obj_handler_src->getDims()[2], carma_select_plan<cuDoubleComplex, double>()));
            else
            /* Create a 3D FFT plan. */{
              int mdims[2];
              mdims[0] = (int) (carma_obj_handler_src->getDims()[1]);
              mdims[1] = (int) (carma_obj_handler_src->getDims()[2]);
              cufftSafeCall(cufftPlanMany(plan, 2 ,mdims,NULL,1,0,NULL,1,0, carma_select_plan<cuDoubleComplex, double>(), (int)(carma_obj_handler_src->getDims()[3])));
            }
            /*
      ...
      if (handle_src->type == Y_SCOMPLEX) {
        ...
        carma_fft(carma_obj_handler_src->getData(), carma_obj_handler_src->getData(), dir, *carma_obj_handler_src->getPlan());
      } else if (handle_src->type == Y_COMPLEX) {
        caObjD *carma_obj_handler_src = (caObjD *) (handle_src->carma_object);
        if (*carma_obj_handler_src->getPlan() == 0L) {
          cufftHandle *plan = carma_obj_handler_src->getPlan(); ///< FFT plan
    ...
    if ((handle_src->type == Y_FLOAT) || (handle_src->type == Y_SCOMPLEX)) {
      if (handle_src->type == Y_FLOAT) {
        ...
        handle->carma_object = new caObjC(context_handle, carma_obj_handler_src->getDims());
        handle->type = Y_SCOMPLEX;
        caObjC *carma_obj_handler = (caObjC *) (handle->carma_object);
        carma_fft(carma_obj_handler_src->getData(), carma_obj_handler->getData(), dir, *carma_obj_handler_src->getPlan());
      } else {
        caObjC *carma_obj_handler_src = (caObjC *) (handle_src->carma_object);
        if (*carma_obj_handler_src->getPlan() == 0L) {
          cufftHandle *plan = carma_obj_handler_src->getPlan(); ///< FFT plan
    ...
    } else if ((handle_src->type == Y_DOUBLE) || (handle_src->type == Y_COMPLEX)) {
      if (handle_src->type == Y_DOUBLE) {
        ...
        handle->carma_object = new caObjZ(context_handle, carma_obj_handler_src->getDims());
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if (handle->type == Y_FLOAT) {
      ...
      caObjS *carma_obj_handler = (caObjS *) (handle->carma_object);
      if (!keysOnly) {
        carma_val_handler = (caObjUI *) (((yObj_struct *) val_handle)->carma_object);
        carma_obj_handler->device2deviceInd(((caObjUI *) carma_val_handler)->getData());
      }
      carma_obj_handler->sort_init(keysOnly);
      carma_obj_handler->sort();
      if (!keysOnly) {
        cudaMemcpy(((caObjUI *) carma_val_handler)->getData(), ((caObjS *) carma_obj_handler)->getValues(), ((caObjS *) carma_obj_handler)->getNbElem() * sizeof(unsigned int),
    ...
    carma_context *context_handle = _getCurrentContext();
    context_handle->set_activeDevice(handle->device);
    yObj_struct *val_handle = (yObj_struct *) ypush_obj(&yObj, sizeof(yObj_struct));
    ((yObj_struct *) val_handle)->device = handle->device;
    void *carma_val_handler;
    val_handle->type = Y_SHORT;
    if (handle->type == Y_FLOAT) {
      caObjS *carma_obj_handler = (caObjS *) (handle->carma_object);
      val_handle->carma_object = new caObjUI(context_handle, carma_obj_handler->getDims());
      carma_val_handler = (caObjUI *) (((yObj_struct *) val_handle)->carma_object);
      carma_obj_handler->sort_init(false);
      carma_obj_handler->sort();
      cudaMemcpy(((caObjUI *) carma_val_handler)->getData(), ((caObjS *) carma_obj_handler)->getValues(), ((caObjS *) carma_obj_handler)->getNbElem() * sizeof(unsigned int), cudaMemcpyDeviceToDevice);
    } else if (handle->type == Y_DOUBLE) {
      throw "Double not supported";
    }
  }
  ...
  if (yarg_subroutine()) {
    ...
    if (argc > 2) {
      ...
      if (handle_dest->type == Y_FLOAT) {
        ...
        // check if sort_on on src. if not initialize values (needed)
        if (values == NULL) {
          cutilSafeCall(cudaMalloc((void ** )&values, carma_src_handler->getNbElem() * sizeof(unsigned int)));
        }
        cutilSafeCall(cudaMemset(values, 0, sizeof(unsigned int) * carma_src_handler->getNbElem()));

        if (yType == Y_OPAQUE) {
          // the user gave a carma_obj as an array of valids
          yObj_struct *handle_valids = (yObj_struct *) yget_obj(argc - 3, &yObj);
          if (handle_valids->device != handle_dest->device)
            y_error("valids & dest for compact should be on same device");
          caObjUI *carma_valids_handler = (caObjUI *) (handle_valids->carma_object);
          nb_valids = (long) carma_valids_handler->getNbElem();
          if (nb_valids != carma_src_handler->getNbElem()) {
            fillvalues(carma_src_handler->getValues(), carma_valids_handler->getData(), carma_valids_handler->getNbElem());
          } else {
            cutilSafeCall(cudaMemcpy(carma_src_handler->getValues(), carma_valids_handler->getData(), carma_src_handler->getNbElem() * sizeof(unsigned int), cudaMemcpyDeviceToDevice));
          }
        } else {
          // the user gave a yorick array as an array of valids
          long dims[Y_DIMSIZE];
          isValid = (unsigned int *) ygeta_i(argc - 3, &nb_valids, dims);
          if (nb_valids != carma_src_handler->getNbElem()) {
            unsigned int *d_valids;
            cutilSafeCall(cudaMalloc((void ** )&d_valids, nb_valids * sizeof(unsigned int)));
            cudaMemcpy(d_valids, isValid, nb_valids * sizeof(unsigned int), cudaMemcpyHostToDevice);
            fillvalues(carma_src_handler->getValues(), d_valids, nb_valids);
            cudaFree(d_valids);
          } else {
            cudaMemcpy(carma_src_handler->getValues(), isValid, carma_src_handler->getNbElem() * sizeof(unsigned int), cudaMemcpyDeviceToDevice);
          }
        }
    ...
    if (handle_src->type == Y_FLOAT) {
      ...
      if (yType == Y_OPAQUE) {
        // the user gave a carma_obj as an array of valids
        yObj_struct *handle_valids = (yObj_struct *) yget_obj(argc - 2, &yObj);
        caObjUI *carma_valids_handler = (caObjUI *) (handle_valids->carma_object);
        if (handle_valids->device != handle_src->device)
          y_error("valids & src for compact should be on same device");
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if ((handle_res->device != handle_im->device) || (handle_res->device != handle_ker->device) || (handle_res->device != handle_pdata->device) || (handle_res->device != handle_pspec->device))
    ...
    if ((handle_res->type == Y_FLOAT) && (handle_im->type == Y_FLOAT) && (handle_ker->type == Y_FLOAT) && (handle_pdata->type == Y_FLOAT) && (handle_pspec->type == Y_SCOMPLEX)) {
      ...
      carma_initfftconv(carma_im_handler, carma_ker_handler, carma_pdata_handler, carma_pspec_handler, kernelY, kernelX);

      carma_fftconv(carma_res_handler, carma_pdata_handler, carma_pspec_handler, kernelY, kernelX);
    } else
 ...
 */
{
  ...
  try {
    if (handler->type == Y_FLOAT) {
      carma_host_obj<float> *carma_host_obj_handler = (carma_host_obj<float> *) (handler->carma_host_object);
      delete carma_host_obj_handler;
    } else if (handler->type == Y_DOUBLE) {
      carma_host_obj<double> *carma_host_obj_handler = (carma_host_obj<double> *) (handler->carma_host_object);
      delete carma_host_obj_handler;
    } else if (handler->type == Y_INT) {
      carma_host_obj<int> *carma_host_obj_handler = (carma_host_obj<int> *) (handler->carma_host_object);
      delete carma_host_obj_handler;
    } else if (handler->type == Y_SHORT) {
      carma_host_obj<unsigned int> *carma_host_obj_handler = (carma_host_obj<unsigned int> *) (handler->carma_host_object);
      delete carma_host_obj_handler;
    } else if (handler->type == Y_COMPLEX) {
      carma_host_obj<cuFloatComplex> *carma_host_obj_handler = (carma_host_obj<cuFloatComplex> *) (handler->carma_host_object);
      delete carma_host_obj_handler;
    } else if (handler->type == Y_SCOMPLEX) {
      carma_host_obj<cuDoubleComplex> *carma_host_obj_handler = (carma_host_obj<cuDoubleComplex> *) (handler->carma_host_object);
      delete carma_host_obj_handler;
    } else
      throw "Type unknown";
    handler->type = Y_VOID;
 ...
 */
{
  ...
  try {
    if (handler->type == Y_FLOAT) {
      ...
      carma_host_obj<unsigned int> *carma_host_obj_handler = (carma_host_obj<unsigned int> *) (handler->carma_host_object);
      unsigned int *data = (unsigned int *) ypush_i(carma_host_obj_handler->getDims());
      carma_host_obj_handler->fill_into(data);
    } else if (handler->type == Y_COMPLEX) {
      carma_host_obj<cuDoubleComplex> *carma_host_obj_handler = (carma_host_obj<cuDoubleComplex> *) (handler->carma_host_object);
      cuDoubleComplex *data = (cuDoubleComplex *) ypush_z(carma_host_obj_handler->getDims());
      carma_host_obj_handler->fill_into(data);
    } else if (handler->type == Y_SCOMPLEX) {
      carma_host_obj<cuComplex> *carma_host_obj_handler = (carma_host_obj<cuComplex> *) (handler->carma_host_object);
 ...
 */
{
  ...
  // Keywords management
  static char const *knames[6] = { "pagelock", "zerocpy", "portable", "wricomb", "genepin", 0 };
  ...
  try {
    ...
    if (yType == Y_OPAQUE) { // Copy constructor
      yHostObj_struct *handle_obj = (yHostObj_struct *) yget_obj(piargs[0], &yHostObj);
      yHostObj_struct *handle = (yHostObj_struct *) ypush_obj(&yHostObj, sizeof(yHostObj_struct));
      handle->type = handle_obj->type;
      if (handle->type == Y_FLOAT) {
        carma_host_obj<float> *carma_host_obj_handler = (carma_host_obj<float> *) (handle_obj->carma_host_object);
        handle->carma_host_object = new carma_host_obj<float>(carma_host_obj_handler, mallocType);
      } else if (handle->type == Y_DOUBLE) {
        carma_host_obj<double> *carma_host_obj_handler = (carma_host_obj<double> *) (handle_obj->carma_host_object);
        handle->carma_host_object = new carma_host_obj<double>(carma_host_obj_handler, mallocType);
      } else if (handle->type == Y_INT) {
        carma_host_obj<int> *carma_host_obj_handler = (carma_host_obj<int> *) (handle_obj->carma_host_object);
        handle->carma_host_object = new carma_host_obj<int>(carma_host_obj_handler, mallocType);
      } else if (handle->type == Y_COMPLEX) {
        carma_host_obj<cuDoubleComplex> *carma_host_obj_handler = (carma_host_obj<cuDoubleComplex> *) (handle_obj->carma_host_object);
        handle->carma_host_object = new carma_host_obj<cuDoubleComplex>(carma_host_obj_handler, mallocType);
      } else if (handle->type == Y_SCOMPLEX) {
        carma_host_obj<cuFloatComplex> *carma_host_obj_handler = (carma_host_obj<cuFloatComplex> *) (handle_obj->carma_host_object);
        handle->carma_host_object = new carma_host_obj<cuFloatComplex>(carma_host_obj_handler, mallocType);
      }
    } else { // Standard constructor
      ...
      if (oType == Y_FLOAT) {
        if (yType == Y_STRING) { // based on the input type and dimensions array
          handle->carma_host_object = new carma_host_obj<float>((long*) data_input, mallocType);
        } else { // based on the input array
          handle->carma_host_object = new carma_host_obj<float>(dims, (float*) data_input, mallocType);
        }
      } else if (oType == Y_DOUBLE) {
        if (yType == Y_STRING) {
          handle->carma_host_object = new carma_host_obj<double>((long*) data_input, mallocType);
        } else {
          handle->carma_host_object = new carma_host_obj<double>(dims, (double*) data_input, mallocType);
        }
      } else if (oType == Y_INT) {
        if (yType == Y_STRING) {
          handle->carma_host_object = new carma_host_obj<int>((long*) data_input, mallocType);
        } else {
          handle->carma_host_object = new carma_host_obj<int>(dims, (int*) data_input, mallocType);
        }
      } else if (oType == Y_SHORT) {
        if (yType == Y_STRING) {
          handle->carma_host_object = new carma_host_obj<unsigned int>((long*) data_input, mallocType);
        } else {
          handle->carma_host_object = new carma_host_obj<unsigned int>(dims, (uint*) data_input, mallocType);
        }
      } else if (oType == Y_COMPLEX) {
        if (yType == Y_STRING) {
          handle->carma_host_object = new carma_host_obj<cuDoubleComplex>((long*) data_input, mallocType);
        } else {
          handle->carma_host_object = new carma_host_obj<cuDoubleComplex>(dims, (cuDoubleComplex*) data_input, mallocType);
        }
      } else if (oType == Y_SCOMPLEX) {
        if (yType == Y_STRING) {
          handle->carma_host_object = new carma_host_obj<cuFloatComplex>((long*) data_input, mallocType);
        } else {
          stringstream buf;
          buf << "Type not supported in yorick in " << __FILE__ << "@" << __LINE__ << endl;
          throw buf.str();
 ...
 */
{
  ...
  if (type == Y_DOUBLE) {
    carma_host_obj<double>* test = (carma_host_obj<double>*) handle->carma_host_object;
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if (handle_mat->type == Y_FLOAT) {
      caObjS *carma_obj_handler_mat = (caObjS *) (handle_mat->carma_object);
      caObjS *carma_obj_handler_eigenvals = (caObjS *) (handle_eigenvals->carma_object);
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    yHostObj_struct *handle_eigenvals = (yHostObj_struct *) yget_obj(argc - 2, &yHostObj);
    yHostObj_struct *handle_U = (yHostObj_struct *) yget_obj(argc - 3, &yHostObj);
    yHostObj_struct *handle_VT = (yHostObj_struct *) yget_obj(argc - 4, &yHostObj);

    if (handle_mat->type == Y_FLOAT) {
      carma_host_obj<float> *carma_obj_handler_mat = (carma_host_obj<float> *) (handle_mat->carma_host_object);

      carma_host_obj<float> *carma_obj_handler_eigenvals = (carma_host_obj<float> *) (handle_eigenvals->carma_host_object);
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    if (handle_mat->type == Y_FLOAT) {
      ...
      caObjS *carma_obj_handler_U = (caObjS *) (handle_U->carma_object);
      caObjS *carma_obj_handler_VT = (caObjS *) (handle_VT->carma_object);
      carma_cula_svd(carma_obj_handler_mat, carma_obj_handler_eigenvals, carma_obj_handler_VT, carma_obj_handler_U);
    }
    if (handle_mat->type == Y_DOUBLE) {
      caObjD *carma_obj_handler_mat = (caObjD *) (handle_mat->carma_object);
      caObjD *carma_obj_handler_eigenvals = (caObjD *) (handle_eigenvals->carma_object);
 ...
 */
{
  ...
  } else if ((mrange[2] - mrange[1] + 1 > carma_in_handler->getDims(1)) || mrange[2] > carma_in_handler->getDims(1))
    y_error("range out of bounds");
  if (mrange[4] == 0) {
    mrange[5] = 1;
    mrange[6] = carma_in_handler->getDims(2);
  } else if ((mrange[6] - mrange[5] + 1 > carma_in_handler->getDims(2)) || mrange[6] > carma_in_handler->getDims(2))
    y_error("range out of bounds");
  ...
  getarray2d(carma_out_handler->getData(), carma_in_handler->getData(), x0, Ncol, carma_in_handler->getDims(1), Nlig * Ncol);
}
 ...
 */
{
  if (yarg_subroutine()) {
    ...
    } else if ((mrange[2] - mrange[1] + 1 > carma_out_handler->getDims(1)) || mrange[2] > carma_out_handler->getDims(1))
      y_error("range out of bounds");
    if (mrange[4] == 0) {
      mrange[5] = 1;
      mrange[6] = carma_out_handler->getDims(2);
    } else if ((mrange[6] - mrange[5] + 1 > carma_out_handler->getDims(2)) || mrange[6] > carma_out_handler->getDims(2))
      y_error("range out of bounds");
    Ncol = mrange[2] - mrange[1] + 1;
    Nlig = mrange[6] - mrange[5] + 1;
    long dims[3];
    ...
    //if (Ncol == 1) x0 = (mrange[1]-1);
    fillarray2d(carma_out_handler->getData(), carma_in_handler->getData(), x0, Ncol, carma_out_handler->getDims(1), Nlig * Ncol);
  } else
 ...
 */
{
  if (yarg_subroutine())
    ...
    caObjS *carma_in_handler = (caObjS *) (handle_in->carma_object);
    float res;
    cudaMemcpy(&res, &((carma_in_handler->getData())[pos - 1]), sizeof(float), cudaMemcpyDeviceToHost);
...
void Y_yoga_plus(int argc) {
  if (yarg_subroutine()) {
    ...
    if (handle_in->type == Y_FLOAT) {
      float alpha = ygets_f(argc - 2);
      caObjS *carma_in_handler = (caObjS *) (handle_in->carma_object);
      carma_plus(carma_in_handler->getData(), alpha, carma_in_handler->getNbElem());
    } else
...
void Y_yoga_plusai(int argc) {
  if (yarg_subroutine()) {
    ...
    if (handle_out->type == Y_FLOAT) {
      caObjS *carma_in_handler = (caObjS *) (handle_in->carma_object);
      caObjS *carma_out_handler = (caObjS *) (handle_out->carma_object);
      carma_plusai(carma_out_handler->getData(), carma_in_handler->getData(), idx, sgn, carma_out_handler->getNbElem());
    } else
      y_error("double not implemented yet");
  } else
    y_error("can only be called as a function");
