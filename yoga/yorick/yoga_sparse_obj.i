...
func check_yoga_sparse_obj(m, n) {
  ...
  mask = int(random(m, k) + 0.2);
  nz = numberof(where(mask == 0));
  write, format = "putting %d zeros %0.2f %% of the matrix\n", nz, float(nz) / (m * n) * 100;
  matA *= mask;
  mask = int(random(k, n) + 0.2);
  nz = numberof(where(mask == 0));
  matB *= mask;
 ...
 */
{
  ...
  if ((ctype == "all") || (ctype == "gpu")) {
    ...
    vecty_gpu=yoga_mv_sparse(matA_gpu,vectx_gpu);
...
func kblas_test(sizem, sizen, blockdim, ctype=) {
  if (is_void(ctype)) ctype = "all";
  if(is_void(sizem)) sizem=512;
  if(is_void(sizen)) sizen=sizem;
  if(is_void(blockdim)) blockdim=8;

  matA=float(random(sizem, sizen));
  vectx=float(random(sizen));
  vecty=float(array(0., sizem));

  mask=int(random(sizem,sizen)+0.2); nz=numberof(where(mask==0));
  write, format="putting %d zeros %0.2f %% of the matrix A\n", nz, float(nz)/(sizem*sizen)*100;
  matA*=float(mask);

  matA_gpu = yoga_sparse_obj(matA);
  matB_gpu = yoga_csr2bsr(blockdim,matA_gpu);
  matB_gpu;

  matC_gpu = yoga_bsr2csr(matB_gpu);
  matC_gpu;

  "minmax(matA_gpu()-matC_gpu())";
  minmax(matA_gpu()-matC_gpu());
  ...
  if ((ctype == "all") || (ctype == "gpu")) {
    ...
    for(i=0; i<100; i++)
      vecty_gpu=yoga_mv_sparse(matA_gpu,vectx_gpu);
    write,format="MV mult cusparse time : %f\n",tac()/100.;
    
    vectx_gpu2 = yoga_obj(vectx);

    tic;
    for(i=0; i<100; i++)
      vecty_gpu2=yoga_mv_ksparse(matB_gpu,vectx_gpu2);
    write,format="MV mult ksparse time : %f\n",tac()/100.;    

    "minmax(vecty_gpu2()-vecty_gpu())";
    minmax(vecty_gpu2()-vecty_gpu());
  }
  if ((ctype == "all") || (ctype == "cpu")) {
    tic;
    for(i=0; i<100; i++)
      vecty=matA(,+)*vectx(+);
    write,format="MV mult cpu time : %f\n",tac()/100.;
    "minmax(vecty-vecty_gpu())";
    minmax(vecty-vecty_gpu());
    "minmax(vecty-vecty_gpu2())";
    minmax(vecty-vecty_gpu2());
  }
  error;
}
