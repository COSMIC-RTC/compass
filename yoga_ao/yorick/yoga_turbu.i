 ...
 if (numberof(windspeed) != nscreen) error("wrong windspeed dimensions");
  ...
  for (i=1;i<=nscreen;i++) {
    if ((!fileExist(swrite(format=dirsave+"A_%d.fits",screen_size(i)))) ||
      ...
      fits_write,swrite(format=dirsave+"A_%d.fits",screen_size(i)),A, overwrite=1;
      fits_write,swrite(format=dirsave+"B_%d.fits",screen_size(i)),B, overwrite=1;
      fits_write,swrite(format=dirsave+"istx_%d.fits",screen_size(i)),istx, overwrite=1;
      fits_write,swrite(format=dirsave+"isty_%d.fits",screen_size(i)),isty, overwrite=1;
    } else {
