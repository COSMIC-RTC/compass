		       ...
		       float yoff, int Ntot, cudaChannelFormatDesc channelDesc, int device)
{
  ...
  /*
   ...
   need to add this option in yoga_obj
...
int target_raytrace_async(yoga_streams *streams, float *d_odata,float *d_idata,int nx, int ny,int Nx, float xoff, float yoff, int block_size)
{
...
int target_raytrace_async(yoga_host_obj<float> *phase_telemetry, float *d_odata,float *d_idata,int nx, int ny,int Nx, float xoff, float yoff, int block_size)
{
