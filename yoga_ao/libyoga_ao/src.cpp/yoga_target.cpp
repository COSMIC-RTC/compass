...
yoga_source::yoga_source(yoga_context *context, float xpos,float ypos,float lambda, float mag, long size, string type, float *pupil, int device)
{
  this->init_source(context,xpos,ypos,lambda,mag,size,type,device);
  this->d_pupil = new yoga_obj<float>(this->current_context,this->d_phase->d_screen->getDims());
  this->d_pupil->host2device(pupil);
  long sumpup = (long)this->d_pupil->asum(1);
  long *dims_data1 = new long[2];
  dims_data1[0] = 1; 
  dims_data1[1] = pow(2,(long)(logf(sumpup)/logf(2))+1);
  this->d_phasepts = new yoga_obj<float>(this->current_context,dims_data1);
  dims_data1[1] = sumpup;
  this->d_wherephase = new yoga_obj<int>(this->current_context,dims_data1);
...
yoga_source::yoga_source(yoga_context *context, float xpos,float ypos,float lambda, float mag, long size, string type, int device)
{
  this->init_source(context,xpos,ypos,lambda,mag,size,type,device);
}

int yoga_source::init_source(yoga_context *context, float xpos,float ypos,float lambda, float mag, long size, string type, int device)
{
  ...
  this->d_phase = new yoga_phase(context, size);
  ...
  dims_data2[1] = size;
  dims_data2[2] = size; 
  int nstreams = (size + this->block_size - size % this->block_size) / this->block_size;

  this->phase_telemetry = new yoga_host_obj<float>(dims_data2,MA_WRICOMB,nstreams);
  ...
  if (type != "wfs") {
    ...
    this->d_image = new yoga_obj<float>(context, dims_data2);
    this->d_amplipup = new yoga_obj<cuFloatComplex>(context, dims_data2);

    cufftHandle *plan=this->d_amplipup->getPlan();///< FFT plan
    cufftSafeCall(cufftPlan2d(plan, this->d_amplipup->getDims(1),
...
yoga_source::~yoga_source()
{
  ...
  if (d_image != 0L) delete this->d_image;
  if (d_pupil != 0L) delete this->d_pupil;
  if (d_leimage != 0L) delete this->d_leimage;
  if (d_amplipup != 0L) delete this->d_amplipup;
  if (d_phasepts != 0L) delete this->d_phasepts;
  if (d_wherephase != 0L) delete this->d_wherephase;
  /*
...
int yoga_source::init_strehlmeter()
{
  this->strehl_counter = 0;
  this->comp_image(1);

  cudaMemcpy(&(this->ref_strehl), &(this->d_image->getData()[this->d_image->imax(1)-1]), sizeof(float), cudaMemcpyDeviceToHost);

  if (this->d_leimage == 0L)
    this->d_leimage = new yoga_obj<float>(this->current_context, this->d_image->getDims());
  else
    cutilSafeCall(cudaMemset(this->d_leimage->getData(), 0,sizeof(float)*this->d_leimage->getNbElem()));
...
{
  cutilSafeCall(cudaMemset(this->d_phase->d_screen->getData(), 0,
...
int yoga_source::raytrace(yoga_atmos *yatmos, bool async)
{
  cutilSafeCall(cudaMemset(this->d_phase->d_screen->getData(), 0,
			   sizeof(float)*this->d_phase->d_screen->getNbElem()));
...
int yoga_source::raytrace(yoga_dms *ydms, int rst, bool async)
{
  if (rst == 1)
    cutilSafeCall(cudaMemset(this->d_phase->d_screen->getData(), 0,
			     sizeof(float)*this->d_phase->d_screen->getNbElem()));
  ...
  while (p != xoff.end()) {
    string types = p->first.first;
    if((types.find("pzt")==0) || (types.find("tt")==0) || (types.find("kl")==0)){
      float alt = p->first.second;
      yoga_dm *ps = ydms->d_dms[make_pair(types,alt)];
      p++;
      if ((p == xoff.end()) && async) {
	target_raytrace_async(this->phase_telemetry,
			      this->d_phase->d_screen->getData(),
  ...
  // select phase pixels in the valid portion of pupil
    fillindx(this->d_phasepts->getData(), this->d_phase->d_screen->getData(), this->d_wherephase->getData(),this->scale,
    ...
    fillindx(this->d_phasepts->getData(), this->d_phase->d_screen->getData(), this->d_wherephase->getData(),this->scale,
	     -phase_avg,this->d_wherephase->getNbElem(),this->device);
...
int yoga_source::raytrace(yoga_dms *ydms, int rst)
{
...
int yoga_source::comp_image(int puponly)
{
  // set complex amplitude in the pupil plane to zero
  cutilSafeCall(cudaMemset(this->d_amplipup->getData(), 0,sizeof(cuFloatComplex)*this->d_amplipup->getNbElem()));
  ...
  fill_amplipup(this->d_amplipup->getData(), this->d_phase->d_screen->getData(), this->d_pupil->getData(), this->scale,puponly,
  ...
  yoga_fft(this->d_amplipup->getData(),this->d_amplipup->getData(),1,*this->d_amplipup->getPlan());
  
  // take square norm to retreive short exposure image
  abs2(this->d_image->getData(),this->d_amplipup->getData(),this->d_image->getDims(1)*this->d_image->getDims(2),this->device);
...
int yoga_source::comp_strehl()
{
  //this->strehl_se = expf(-this->phase_var);
  //this->strehl_le = expf(-(this->phase_var_avg / this->strehl_counter));
  cudaMemcpy(&(this->strehl_se), &(this->d_image->getData()[this->d_image->imax(1)-1]), sizeof(float), cudaMemcpyDeviceToHost);
  cudaMemcpy(&(this->strehl_le), &(this->d_leimage->getData()[this->d_leimage->imax(1)-1]), sizeof(float), cudaMemcpyDeviceToHost);
...
yoga_target::yoga_target(yoga_context *context, int ntargets,float *xpos,float *ypos,float *lambda,float *mag, long *sizes, float *pupil, int device)
{
  this->ntargets = ntargets;

  for (int i=0;i<ntargets;i++) {
    d_targets.push_back(new yoga_source(context, xpos[i],ypos[i],lambda[i],mag[i],sizes[i],"target",pupil,device));
  }
}
