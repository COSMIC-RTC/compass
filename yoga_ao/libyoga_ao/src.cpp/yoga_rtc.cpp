...
yoga_rtc::yoga_rtc(yoga_context *context)
{
...
int yoga_rtc::add_centroider(long nwfs, long nvalid, float offset, float scale, long device, char *typec)
{
  d_centro.push_back(new yoga_centroider(current_context, nwfs, nvalid,offset,scale,device,typec));

  return EXIT_SUCCESS;
}

int yoga_rtc::add_controler(long nactu, long delay, long device,const char *typec)
{
  int ncentroids = 0;
  ...
  d_control.push_back(new yoga_controler(current_context, ncentroids,nactu,delay,device,typec));

  return EXIT_SUCCESS;
}

int yoga_rtc::rm_controler()
{
  delete this->d_control[(this->d_control).size()-1];
  d_control.pop_back();
...
int yoga_rtc::do_imat(int ncntrl, yoga_sensors *sensors, yoga_dms *ydm)
{
...
int yoga_rtc::do_imat_geom(int ncntrl, yoga_sensors *sensors, yoga_dms *ydm, int type)
{
  map<type_screen,yoga_dm *>::iterator p;
  p = ydm->d_dms.begin();
  int inds1,inds2;
...
int yoga_rtc::do_centroids(yoga_sensors *sensors)
{
  for (size_t idx_cntr = 0; idx_cntr < (this->d_centro).size(); idx_cntr++) {
    int nwfs = this->d_centro[idx_cntr]->nwfs;
...
int yoga_rtc::do_centroids(int ncntrl,yoga_sensors *sensors)
{
  return do_centroids(ncntrl,sensors,false);
}

int yoga_rtc::do_centroids(int ncntrl,yoga_sensors *sensors, bool imat)
{
  int inds2 = 0;
...
int yoga_rtc::do_control(int ncntrl, yoga_dms *ydm)
{
