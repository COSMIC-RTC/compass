...
yoga_tscreen::yoga_tscreen(yoga_context *context, long size, long size2, float r0, float altitude, float windspeed, float winddir,float deltax, float deltay, int device)
{
  ...
  this->d_tscreen = new yoga_phase(current_context, this->screen_size);
  ...
  this->d_tscreen_o = new yoga_obj<float>(current_context, dims_data2);
  this->d_B = new yoga_obj<float>(current_context, dims_data2);
  
  dims_data2[2] = size2; 
  this->d_A = new yoga_obj<float>(current_context, dims_data2);
  
  long *dims_data = new long[2];
  dims_data[0] = 1; dims_data[1] = size2;
  this->d_istencilx = new yoga_obj<unsigned int>(current_context, dims_data);
  this->d_istencily = new yoga_obj<unsigned int>(current_context, dims_data);
  this->d_z = new yoga_obj<float>(current_context, dims_data);

  dims_data[1] = this->screen_size;
  this->d_noise = new yoga_obj<float>(current_context, dims_data);
  this->d_ytmp = new yoga_obj<float>(current_context, dims_data);
...
int yoga_tscreen::init_vk(int seed, int pupd)
{
  long *dims_data2 = new long[3];
  dims_data2[0] = 2; dims_data2[1] = this->screen_size; dims_data2[2] = this->screen_size; 
  this->d_tscreen_c = new yoga_obj<cuFloatComplex>(current_context, dims_data2);
  cufftHandle *plan=this->d_tscreen_c->getPlan();
  cufftSafeCall( cufftPlan2d(plan, this->screen_size, this->screen_size ,CUFFT_C2C ));

  if (this->d_tscreen_o->is_rng_init() == false)
    this->d_tscreen_o->init_prng_host(seed);

  this->norm_vk = pow(pupd * pow(this->amplitude,6.0f/5.0f),5.0f/6.0f)* 0.5f / (2.0f *  3.14159);
...
int yoga_tscreen::generate_vk(float l0,int nalias)
{
  this->d_tscreen_o->prng_host('N');

  cuFloatComplex *data = this->d_tscreen_c->getData();
  cutilSafeCall(cudaMemset(data,0,this->screen_size*this->screen_size * sizeof(cuFloatComplex)));

  float  k0 = (l0 ==0. ? 0.0f : this->screen_size / l0);
  int block_size = 8;

  float *data_o = this->d_tscreen_o->getData();
  gene_vonkarman(data,data_o,k0, nalias,this->screen_size,this->screen_size,block_size);

  roll(data,this->screen_size,this->screen_size);

  yoga_fft(data, data, 1,*this->d_tscreen_c->getPlan());
...
// dir =1 moving in x
{
  ...
  if (dir == 1) {// adding a column to the left
    fillindx(this->d_z->getData(),this->d_tscreen->d_screen->getData(),(int *)this->d_istencilx->getData(),
	     this->d_z->getNbElem(),this->device);
    x0 = this->screen_size - 1;//not in stencil
  } else {
    fillindx(this->d_z->getData(),this->d_tscreen->d_screen->getData(),(int *)this->d_istencily->getData(),
	     this->d_z->getNbElem(),this->device);
    x0 = this->screen_size*(this->screen_size-1);
  }

  addai(this->d_z->getData(),this->d_tscreen->d_screen->getData(),x0,-1.0f,this->d_z->getNbElem(),this->device);

  this->d_ytmp->gemv('n',1.0f,this->d_A,this->d_A->getDims(1),this->d_z,1,0.0f,1);

  this->d_noise->prng_host('N');

  this->d_ytmp->gemv('n',this->amplitude,this->d_B,this->d_B->getDims(1),this->d_noise,1,1.0f,1);

  addai(this->d_ytmp->getData(),this->d_tscreen->d_screen->getData(),x0,1.0f,this->d_ytmp->getNbElem(),this->device);
  ...
  getarr2d(this->d_tscreen_o->getData(),this->d_tscreen->d_screen->getData(), x0,Ncol, NC, N, this->device);
  
  if (dir == 1) x0 = 0;
  else x0 = 0;

  fillarr2d(this->d_tscreen->d_screen->getData(), this->d_tscreen_o->getData(), x0, Ncol, NC, N, this->device);
  ...
  fillarr2d(this->d_tscreen->d_screen->getData(), this->d_ytmp->getData(), x0, Ncol, NC, N, this->device);
...
yoga_atmos::yoga_atmos(yoga_context *context, int nscreens,float *r0,long *size,long *size2, float *altitude, float *windspeed, float *winddir, float *deltax, float *deltay, float *pupil, int device)
{
  ...
  for (int i=0;i<nscreens;i++) {
    d_screens.insert(pair<float,yoga_tscreen *>(altitude[i],new yoga_tscreen(context, size[i],size2[i],r0[i],altitude[i],windspeed[i],winddir[i],deltax[i],deltay[i],device)));
  }
