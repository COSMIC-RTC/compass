...
int
move_atmos(sutra_atmos *atmos, sutra_target *target) {
...
//long nxsub, long nvalid, long npix, long nrebin, long nfft
    ...
    try {
      ...
      handle->sutra_wfs = new sutra_wfs(context_handle, "sh", nxsub, nvalid,
          npix, nphase, nrebin, nfft, ntot, npup, pdiam, nphot, lgs, odevice);
  ...
  Y_sensors_initarr(int argc) {
    ...
    if (sensors_handler->d_wfs.at(nsensor)->type == "sh") {
      ...
      sensors_handler->d_wfs.at(nsensor)->wfs_initarrays(phasemap, hrmap,
          binmap, offsets, pupil, fluxPerSub, isvalid, validsubsx, validsubsy,
    ...
    if (sensors_handler->d_wfs.at(nsensor)->type == "pyr") {
      ...
      sensors_handler->d_wfs.at(nsensor)->wfs_initarrays(
          (cuFloatComplex*) halfxy, (cuFloatComplex*) offsets, focmask, pupil,
          isvalid, cx, cy, sincar, phasemap, validx, validy);
    }
    if (sensors_handler->d_wfs.at(nsensor)->type == "geo") {
      ...
      sensors_handler->d_wfs.at(nsensor)->wfs_initarrays(phasemap, offsets,
          pupil, fluxPerSub, isvalid, validsubsx, validsubsy, istart, jstart);
    }
  }
...
// Florian features
  ...
  Y_yoga_floloadkl(int argc) {
    ...
    dms_handler->d_dms.at(make_pair("kl", alt))->kl_floloadarrays(covmat,
        filter, evals, bas);
    cout << "flag11" << endl;
  }
