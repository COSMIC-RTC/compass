...
int move_atmos(sutra_atmos *atmos,sutra_target *target) {
  ...
  while (p != atmos->d_screens.end()) {
    ...
    if (abs(tmpx) > 0) {
      for (int cc=0;cc<abs(tmpx);cc++) tmp->extrude(1);
      tmp->accumx -= (int)tmp->accumx;
    }
    if (abs(tmpy) > 0) {
      for (int cc=0;cc<abs(tmpy);cc++) tmp->extrude(0);
 ...
 */
  ...
  void atmos_print(void *obj)
  {
    ...
    while (p != atmos_obj_handler->d_screens.end()) {
      ...
      cout << setw(9) << i+1 << " | "
	   << setw(7) << tmp->altitude << " | "
	   << setw(11) << tmp->windspeed << " | "
	   << setw(9) << tmp->winddir << " | "
	   << setw(8) <<  pow(tmp->amplitude,-6.0f/5.0f) << " | "
	   << setw(6) << tmp->deltax << " | "
	   << setw(6) << tmp->deltay << endl;
      p++; i++;
    }
  }
  ...
  Y_yoga_atmos(int argc)
  {
    ...
    try {
      ...
      if (ntot != nscreens) y_error("wrong dimension for screens r0");
 ...
 */
  ...
  void target_print(void *obj)
  {
    ...
    for (int i=0;i<target_obj_handler->ntargets;i++) {
      cout << setw(8) << i+1 << " | "
	   << setw(4) << target_obj_handler->d_targets.at(i)->tposx
	   << " , " << setw(4) << left << target_obj_handler->d_targets.at(i)->tposy << " | "
  ...
  Y_target_getimage(int argc)
  {
    ...
    int puponly = 0;
    if (argc>3) puponly = (int)ygets_l(argc-4);

    float *data = ypush_f(target_handler->d_targets.at(ntarget)->d_image->getDims());
  ...
  Y_target_getphase(int argc)
  {
    ...
    float *data = ypush_f(target_handler->d_targets.at(ntarget)->d_phase->d_screen->getDims());
  ...
  Y_target_getphasetele(int argc)
  {
    ...
    float *data = ypush_f(target_handler->d_targets.at(ntarget)->phase_telemetry->getDims());
    cutilSafeCall( cudaMemcpy(data, target_handler->d_targets.at(ntarget)->phase_telemetry->getData(),
			      target_handler->d_targets.at(ntarget)->phase_telemetry->getNbElem() * sizeof(float),
  ...
  Y_target_getamplipup(int argc)
  {
    ...
    long * ndims_obj = target_handler->d_targets.at(ntarget)->d_amplipup->getDims();
 ...
 */
  ...
  void sensors_print(void *obj)
  {
    ...
    cout << "WFS #" << " | " << "Nsubaps" << " | " << "Nvalid" << " | " << "Npix" << " | "
	 << "Nphase" << " | " << "Nfft" << " | " << "Nrebin" << " | " << "Ntot" << " | " << "Npup" << endl;

    for (int i=0;i<sensors_obj_handler->nsensors;i++) {
      cout << setw(5) << i+1 << " | " << setw(3) << sensors_obj_handler->d_wfs.at(i)->nxsub
	   << "x" << left << setw(3) << sensors_obj_handler->d_wfs.at(i)->nxsub << " | "
	   << right << setw(6) << sensors_obj_handler->d_wfs.at(i)->nvalid << " | "
  ...
  Y_sensors_initlgs(int argc)
  {
    ...
    sensors_handler->d_wfs.at(nsensor)->d_gs->d_lgs->lgs_makespot(handler->device,0);
  }
  ...
  Y_sensors_getimg(int argc)
  {
    ...
    float *data = ypush_f(sensors_handler->d_wfs.at(nsensor)->d_binimg->getDims());
    /*
      if(argc>2) { // With a telemetry object
  ...
  Y_sensors_getdata(int argc)
  {
	...
	long *ndims_obj = sensors_handler->d_wfs.at(nsensor)->d_camplipup->getDims();
	...
	long *ndims_data = new long[5];
	ndims_data[0]=4;ndims_data[1]=2;
	long *ndims_obj = sensors_handler->d_wfs.at(nsensor)->d_camplifoc->getDims();
	memcpy(&ndims_data[2],&(ndims_obj[1]),sizeof(long)*3);
	float *data = ypush_f(ndims_data);
    ...
    if (strcmp(type_data, "hrimg")==0) {
      float *data = ypush_f(sensors_handler->d_wfs.at(nsensor)->d_hrimg->getDims());
      sensors_handler->d_wfs.at(nsensor)->d_hrimg->device2host(data);
    }
    if (strcmp(type_data, "fttotim")==0) {
      long *ndims_data = new long[5];
      ndims_data[0]=4;ndims_data[1]=2;
      long *ndims_obj = sensors_handler->d_wfs.at(nsensor)->d_fttotim->getDims();
      memcpy(&ndims_data[2],&(ndims_obj[1])
	     ,sizeof(long)*3);
      float *data = ypush_f(ndims_data);
      sensors_handler->d_wfs.at(nsensor)->d_fttotim->device2host((cuFloatComplex *)data);
    }
    if (strcmp(type_data, "bincube")==0) {
      float *data = ypush_f(sensors_handler->d_wfs.at(nsensor)->d_bincube->getDims());
      sensors_handler->d_wfs.at(nsensor)->d_bincube->device2host(data);
    }
    if (strcmp(type_data, "phase")==0) {
      float *data = ypush_f(sensors_handler->d_wfs.at(nsensor)->d_gs->d_phase->d_screen->getDims());
      sensors_handler->d_wfs.at(nsensor)->d_gs->d_phase->d_screen->device2host(data);
    }
    if (strcmp(type_data, "pupil")==0) {
      float *data = ypush_f(sensors_handler->d_wfs.at(nsensor)->d_pupil->getDims());
      sensors_handler->d_wfs.at(nsensor)->d_pupil->device2host(data);
    }
    if (strcmp(type_data, "imgtele")==0) {
      float *data = ypush_f(sensors_handler->d_wfs.at(nsensor)->image_telemetry->getDims());
      cutilSafeCall( cudaMemcpy(data, sensors_handler->d_wfs.at(nsensor)->image_telemetry->getData(),
				sensors_handler->d_wfs.at(nsensor)->image_telemetry->getNbElem() * sizeof(float),
				cudaMemcpyHostToHost) );
    }
    if (strcmp(type_data, "phasetele")==0) {
      float *data = ypush_f(sensors_handler->d_wfs.at(nsensor)->d_gs->phase_telemetry->getDims());
      cutilSafeCall( cudaMemcpy(data, sensors_handler->d_wfs.at(nsensor)->d_gs->phase_telemetry->getData(),
				sensors_handler->d_wfs.at(nsensor)->d_gs->phase_telemetry->getNbElem() * sizeof(float),
				cudaMemcpyHostToHost) );
    }
    if (strcmp(type_data, "lgskern")==0) {
      float *data = ypush_f(sensors_handler->d_wfs.at(nsensor)->d_gs->d_lgs->d_lgskern->getDims());
      sensors_handler->d_wfs.at(nsensor)->d_gs->d_lgs->d_lgskern->device2host(data);
    }
    if (strcmp(type_data, "ftlgskern")==0) {
      long *ndims_data = new long[5];
      ndims_data[0]=4;ndims_data[1]=2;
      long *ndims_obj = sensors_handler->d_wfs.at(nsensor)->d_gs->d_lgs->d_ftlgskern->getDims();
      memcpy(&ndims_data[2],&(ndims_obj[1])
	     ,sizeof(long)*3);
      float *data = ypush_f(ndims_data);
	...
	long *ndims_data = new long[4];
	ndims_data[0]=3;ndims_data[1]=2;
	long *ndims_obj = sensors_handler->d_wfs.at(nsensor)->d_poffsets->getDims();
	memcpy(&ndims_data[2],&(ndims_obj[1]),sizeof(long)*2);
	float *data = ypush_f(ndims_data);
	sensors_handler->d_wfs.at(nsensor)->d_poffsets->device2host((cuFloatComplex *)data);
      }
    }
	...
	long *ndims_data = new long[4];
	ndims_data[0]=3;ndims_data[1]=2;
	long *ndims_obj = sensors_handler->d_wfs.at(nsensor)->d_phalfxy->getDims();
	memcpy(&ndims_data[2],&(ndims_obj[1]),sizeof(long)*2);
	float *data = ypush_f(ndims_data);
	sensors_handler->d_wfs.at(nsensor)->d_phalfxy->device2host((cuFloatComplex *)data);
      }
    }
  ...
  Y_sensors_setphase(int argc)
  {
    ...
    sensors_handler->d_wfs.at(nsensor)->d_gs->d_phase->d_screen->host2device(data);
  }
 ...
 */
  ...
  void dms_print(void *obj)
  {
    ...
    while (p != dms_handler->d_dms.end()) {
      ...
      cout << setw(4) << cpt << " | " << setw(5) << types << " | " << setw(7) << alt << " | "
	   << setw(4) << ninflu  << " | " << setw(4) << dim  << endl;
      p++;
    }
  }
  ...
  Y_yoga_addpzt(int argc)
  {
    ...
    dms_handler->add_dm(context_handle, "pzt",alt,dim,ninflu,influsize,ninflupos,n_npts,push4imat,odevice);
  }
  ...
  Y_yoga_loadpzt(int argc)
  {
    ...
    dms_handler->d_dms.at(make_pair("pzt",alt))->pzt_loadarrays(influ,influpos,npoints,istart,xoff,yoff);
  }
  ...
  Y_yoga_addkl(int argc)
  {
    ...
    dms_handler->add_dm(context_handle, "kl",alt,dim,ninflu,influsize,nr,np,push4imat,odevice);
  }
  void
  Y_yoga_loadkl(int argc)
  {
    ...
    dms_handler->d_dms.at(make_pair("kl",alt))->kl_loadarrays(rabas,azbas,ord,cr,cp);
  }
  ...
  Y_yoga_getkl(int argc)
  {
    ...
    int xoff = (int)((dms_handler->d_dms.at(make_pair("kl",alt))->d_shape->d_screen->getDims()[1] - 
		      dms_handler->d_dms.at(make_pair("kl",alt))->d_kl->dim)/2.0f);
    int yoff = xoff;

    dms_handler->d_dms.at(make_pair("kl",alt))->d_kl->do_compute(dms_handler->d_dms.at(make_pair("kl",alt))->d_shape->d_screen->getData(),
								 nkl,dms_handler->d_dms.at(make_pair("kl",alt))->d_shape->d_screen->getDims()[1],
								 xoff,yoff);

    float *data = ypush_f(dms_handler->d_dms.at(make_pair("kl",alt))->d_shape->d_screen->getDims());
    dms_handler->d_dms.at(make_pair("kl",alt))->d_shape->d_screen->device2host(data);
  }
  ...
  Y_yoga_addtt(int argc)
  {
    ...
    if (argc > 4) odevice = ygets_i(argc-5);

    activeDevice = context_handle->set_activeDevice(odevice);

    dms_handler->add_dm(context_handle,"tt",alt,dim,2,dim,1,1,push4imat,odevice);
  }
  ...
  Y_yoga_getdm(int argc)
  {
    ...
    dms_handler->d_dms.at(make_pair(type,alt))->d_shape->d_screen->device2host(data);
  }
 ...
 void
  Y_dms_getdata(int argc)
  {
    ...
    if (strcmp(type_data, "xoff")==0) {
      float *data = ypush_f(dms_handler->d_dms.at(make_pair(type,alt))->d_xoff->getDims());
      memcpy(data,dms_handler->d_dms.at(make_pair(type,alt))->d_xoff->getData(),
	     sizeof(float)*dms_handler->d_dms.at(make_pair(type,alt))->d_xoff->getNbElem());
    }
    if (strcmp(type_data, "yoff")==0) {
      float *data = ypush_f(dms_handler->d_dms.at(make_pair(type,alt))->d_yoff->getDims());
      memcpy(data,dms_handler->d_dms.at(make_pair(type,alt))->d_yoff->getData(),
	     sizeof(float)*dms_handler->d_dms.at(make_pair(type,alt))->d_yoff->getNbElem());
    }
    if (strcmp(type_data, "rabas")==0) {
      float *data = ypush_f(dms_handler->d_dms.at(make_pair(type,alt))->d_kl->d_rabas->getDims());
      memcpy(data,dms_handler->d_dms.at(make_pair(type,alt))->d_kl->d_rabas->getData(),
	     sizeof(float)*dms_handler->d_dms.at(make_pair(type,alt))->d_kl->d_rabas->getNbElem());
    }
     if (strcmp(type_data, "azbas")==0) {
      float *data = ypush_f(dms_handler->d_dms.at(make_pair(type,alt))->d_kl->d_azbas->getDims());
      memcpy(data,dms_handler->d_dms.at(make_pair(type,alt))->d_kl->d_azbas->getData(),
	     sizeof(float)*dms_handler->d_dms.at(make_pair(type,alt))->d_kl->d_azbas->getNbElem());
    }
 }
 ...
 */
  ...
  void rtc_print(void *obj)
  {
    ...
    cout << "Centro #" << " | " << "Type " << " | " << "nwfs" << " | " << "Nvalid" << endl;

    for (size_t idx = 0; idx < rtc_handler->d_centro.size(); idx++) {
      cout << setw(8) << idx+1 << " | " << setw(5) << rtc_handler->d_centro.at(idx)->typec << " | "
	   << setw(4) << rtc_handler->d_centro.at(idx)->nwfs  + 1 << " | "
	   << rtc_handler->d_centro.at(idx)->nvalid  << endl;
    }

    cout << "Contains " << rtc_handler->d_control.size() << " Controler(s) : " << endl;
    cout << "Control #" << " | " << "Type " << " | " << "Nvalid" << " | " << "Nactu" << endl;

    for (size_t idx = 0; idx < rtc_handler->d_control.size(); idx++) {
      cout << setw(9) << idx+1 << " | " << setw(5) << rtc_handler->d_control.at(idx)->typec << " | "
	   << setw(6) << rtc_handler->d_control.at(idx)->nvalid<< " | "
	   << setw(5) << rtc_handler->d_control.at(idx)->nactu  << endl;
    }
  }
  ...
  {    
    ...
    else rtc_handler->d_control[ncontrol]->build_cmat(nfilt);
  }
  ...
  Y_controler_setdata(int argc)
  {
    ...
    if (strcmp(type_data, "mod2act")==0) {
      float *data          = ygeta_f(argc-4,&ntot,dims);
      memcpy(rtc_handler->d_control.at(ncontrol)->h_mod2act->getData(),data,
	     sizeof(float)*rtc_handler->d_control.at(ncontrol)->h_mod2act->getNbElem());
      rtc_handler->d_control.at(ncontrol)->d_mod2act->host2device(data);
    }
    if (strcmp(type_data, "mes2mod")==0) {
      float *data          = ygeta_f(argc-4,&ntot,dims);
      memcpy(rtc_handler->d_control.at(ncontrol)->h_mes2mod->getData(),data,
	     sizeof(float)*rtc_handler->d_control.at(ncontrol)->h_mes2mod->getNbElem());
      rtc_handler->d_control.at(ncontrol)->d_mes2mod->host2device(data);
    }
    if (strcmp(type_data, "eigenvals")==0) {
      float *data          = ygeta_f(argc-4,&ntot,dims);
      memcpy(rtc_handler->d_control.at(ncontrol)->h_eigenvals->getData(),data,
	     sizeof(float)*rtc_handler->d_control.at(ncontrol)->h_eigenvals->getNbElem());
  ...
  Y_controler_getdata(int argc)
  {
    ...
    char *type_data = ygets_q(argc-3);
    if (strcmp(type_data, "mod2act")==0) {
      float *data = ypush_f(rtc_handler->d_control.at(ncontrol)->h_mod2act->getDims());
      memcpy(data,rtc_handler->d_control.at(ncontrol)->h_mod2act->getData(),
	     sizeof(float)*rtc_handler->d_control.at(ncontrol)->h_mod2act->getNbElem());
    }
    if (strcmp(type_data, "mes2mod")==0) {
      float *data = ypush_f(rtc_handler->d_control.at(ncontrol)->h_mes2mod->getDims());
      memcpy(data,rtc_handler->d_control.at(ncontrol)->h_mes2mod->getData(),
	     sizeof(float)*rtc_handler->d_control.at(ncontrol)->h_mes2mod->getNbElem());
    }
    if (strcmp(type_data, "eigenvals")==0) {
      float *data = ypush_f(rtc_handler->d_control.at(ncontrol)->h_eigenvals->getDims());
      memcpy(data,rtc_handler->d_control.at(ncontrol)->h_eigenvals->getData(),
	     sizeof(float)*rtc_handler->d_control.at(ncontrol)->h_eigenvals->getNbElem());
    }
    if (strcmp(type_data, "himat")==0) {
      float *data = ypush_f(rtc_handler->d_control.at(ncontrol)->h_imat->getDims());
      memcpy(data,rtc_handler->d_control.at(ncontrol)->h_imat->getData(),
	     sizeof(float)*rtc_handler->d_control.at(ncontrol)->h_imat->getNbElem());
    }
    if (strcmp(type_data, "hcmat")==0) {
      float *data = ypush_f(rtc_handler->d_control.at(ncontrol)->h_cmat->getDims());
      memcpy(data,rtc_handler->d_control.at(ncontrol)->h_cmat->getData(),
	     sizeof(float)*rtc_handler->d_control.at(ncontrol)->h_cmat->getNbElem());
    }
    if (strcmp(type_data, "mateigens")==0) {
      float *data = ypush_f(rtc_handler->d_control.at(ncontrol)->h_mateigens->getDims());
      memcpy(data,rtc_handler->d_control.at(ncontrol)->h_mateigens->getData(),
	     sizeof(float)*rtc_handler->d_control.at(ncontrol)->h_mateigens->getNbElem());
    }
    if (strcmp(type_data, "dtmp")==0) {
      float *data = ypush_f(rtc_handler->d_control.at(ncontrol)->d_tmp->getDims());
      rtc_handler->d_control.at(ncontrol)->d_tmp->device2host(data);
    }
    if (strcmp(type_data, "cenbuff")==0) {
      float *data = ypush_f(rtc_handler->d_control.at(ncontrol)->d_cenbuff->getDims());
      rtc_handler->d_control.at(ncontrol)->d_cenbuff->device2host(data);
    }
    if (strcmp(type_data, "err")==0) {
      float *data = ypush_f(rtc_handler->d_control.at(ncontrol)->d_err->getDims());
      rtc_handler->d_control.at(ncontrol)->d_err->device2host(data);
    }
    if (strcmp(type_data, "com")==0) {
      float *data = ypush_f(rtc_handler->d_control.at(ncontrol)->d_com->getDims());
 ...
 */
  ...
  Y_sensors_initweights(int argc)
  {
    ...
    rtc_handler->d_centro.at(ncentro)->init_weights(sensors_handler->d_wfs.at(nsensor));
  ...
  Y_sensors_initcorr(int argc)
  {
    ...
    rtc_handler->d_centro.at(ncentro)->init_corr(sensors_handler->d_wfs.at(nsensor),sizex,sizey,interpmat);
  ...
  Y_sensors_compslopes(int argc)
  {
    ...
    if ((rtc_handler->d_centro.at(ncentro)->typec).compare("cog")==0) {
      rtc_handler->d_centro.at(ncentro)->get_cog(sensors_handler->d_wfs.at(nsensor));
    }
    if ((rtc_handler->d_centro.at(ncentro)->typec).compare("tcog")==0) {
      float thresh = 0.;
      if (argc>4) thresh = ygets_f(argc-5);
      else thresh = rtc_handler->d_centro.at(ncentro)->threshold;
      rtc_handler->d_centro.at(ncentro)->get_tcog(thresh,sensors_handler->d_wfs.at(nsensor));
    }
    if ((rtc_handler->d_centro.at(ncentro)->typec).compare("bpcog")==0) {
      int nmax = 10;
      if (argc>4) nmax = ygets_i(argc-5);
      else nmax = rtc_handler->d_centro.at(ncentro)->nmax;
      rtc_handler->d_centro.at(ncentro)->get_bpcog(nmax,sensors_handler->d_wfs.at(nsensor));
    }
    if ((rtc_handler->d_centro.at(ncentro)->typec).compare("wcog")==0) {
      rtc_handler->d_centro.at(ncentro)->get_wcog(sensors_handler->d_wfs.at(nsensor));
    }
    if ((rtc_handler->d_centro.at(ncentro)->typec).compare("corr")==0) {
      rtc_handler->d_centro.at(ncentro)->get_corr(sensors_handler->d_wfs.at(nsensor));
    }
  }
  ...
  Y_centroider_getdata(int argc)
  {
    ...
    if (strcmp(type_data, "weights")==0) {
      float *data = ypush_f(rtc_handler->d_centro.at(ncentro)->d_weights->getDims());
      rtc_handler->d_centro.at(ncentro)->d_weights->device2host(data);
    }
    if (strcmp(type_data, "corrnorm")==0) {
      float *data = ypush_f(rtc_handler->d_centro.at(ncentro)->d_corrnorm->getDims());
    ...
    if (strcmp(type_data, "corrfnct")==0) {
      ...
      memcpy(&ndims_data[2],&(ndims_obj[1]),sizeof(long)*3);
      float *data = ypush_f(ndims_data);
      rtc_handler->d_centro.at(ncentro)->d_corrfnct->device2host((cuFloatComplex *)data);
    }
    if (strcmp(type_data, "corrspot")==0) {
      ...
      memcpy(&ndims_data[2],&(ndims_obj[1]),sizeof(long)*3);
      float *data = ypush_f(ndims_data);
      rtc_handler->d_centro.at(ncentro)->d_corrspot->device2host((cuFloatComplex *)data);
    }
    ...
    if (strcmp(type_data, "corrmax")==0) {
      int *data = ypush_i(rtc_handler->d_centro.at(ncentro)->d_corrmax->getDims());
      rtc_handler->d_centro.at(ncentro)->d_corrmax->device2host(data);
    }
    if (strcmp(type_data, "matinterp")==0) {
      float *data = ypush_f(rtc_handler->d_centro.at(ncentro)->d_interpmat->getDims());
 ...
 *
  */
  ...
  Y_yoga_templatefill(int argc)
  {
    ...
    } else aotemplate_handler->fill_data();
  }
  ...
  Y_yoga_gettemplate(int argc)
  {
    ...
    if (argc > 1)  {
      ...
      float *data = ypush_f(aotemplate_handler->d_data->getDims());
      aotemplate_handler->d_data->device2host(data);
      }
      if (strcmp(type_data, "res")==0) {
      float *data = ypush_f(aotemplate_handler->d_res->getDims());
