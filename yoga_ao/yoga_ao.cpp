...
void atmos_print(void *obj) {
	...
	while (p != atmos_obj_handler->d_screens.end()) {
		...
		cout << setw(9) << i + 1 << " | " << setw(7) << tmp->altitude << " | "
				<< setw(11) << tmp->windspeed << " | " << setw(9)
				<< tmp->winddir << " | " << setw(8)
				<< pow(tmp->amplitude, -6.0f / 5.0f) << " | " << setw(6)
				<< tmp->deltax << " | " << setw(6) << tmp->deltay << endl;
		p++;
		i++;
	}
}
...
void Y_yoga_atmos(int argc) {
	...
	try {
		...
		handle->sutra_atmos = new sutra_atmos(context_handle, nscreens,
				...
				(float *) wspeed, (float *) wdir, (float *) deltax,
				(float *) deltay, (float *) pupil, odevice);
...
void Y_get_spupil(int argc) {
	...
	if (yarg_subroutine())
		...
		if (atmos_handler->d_screens.find(0.0f)
				!= atmos_handler->d_screens.end()) {
...
// args are : the sutra_atmos, nscreen A, B, istencilx,istencily and seed
	...
	if (ntot
			!= atmos_obj_handler->d_screens[altitude]->d_istencilx->getNbElem())
		y_error("wrong size for istencilx");
	unsigned int *istencily = (unsigned int *) ygeta_i(argc - 6, &ntot, dims);
	if (ntot
			!= atmos_obj_handler->d_screens[altitude]->d_istencily->getNbElem())
...
void target_print(void *obj) {
	...
	for (int i = 0; i < target_obj_handler->ntargets; i++) {
		cout << setw(8) << i + 1 << " | " << setw(4)
				<< target_obj_handler->d_targets.at(i)->tposx << " , "
				<< setw(4) << left << target_obj_handler->d_targets.at(i)->tposy
				<< " | " << right << setw(5)
				<< target_obj_handler->d_targets.at(i)->mag << " | "
...
void Y_target_getstrehl(int argc) {
	...
	strehl[3] =
			target_handler->d_targets.at(ntarget)->phase_var_avg
					/ (target_handler->d_targets.at(ntarget)->phase_var_count
							+ 0.00001);
	delete ndims_data;
}
...
//long nxsub, long nvalid, long npix, long nrebin, long nfft
	...
	try {
		...
		wfs_struct *handle = (wfs_struct *) ypush_obj(&yWfs,
				sizeof(wfs_struct));
		handle->device = odevice;

		handle->sutra_wfs = new sutra_wfs(context_handle, "sh", nxsub, nvalid,
				npix, nphase, nrebin, nfft, ntot, npup, pdiam, nphot, lgs,
				odevice);
...
void sensors_print(void *obj) {
	...
	cout << "WFS #" << " | " << "Nsubaps" << " | " << "Nvalid" << " | "
			<< "Npix" << " | " << "Nphase" << " | " << "Nfft" << " | "
			<< "Nrebin" << " | " << "Ntot" << " | " << "Npup" << endl;

	for (size_t i = 0; i < sensors_obj_handler->nsensors(); i++) {
		cout << setw(5) << i + 1 << " | " << setw(3)
				<< sensors_obj_handler->d_wfs.at(i)->nxsub << "x" << left
				<< setw(3) << sensors_obj_handler->d_wfs.at(i)->nxsub << " | "
				...
				<< " | " << setw(4) << sensors_obj_handler->d_wfs.at(i)->npup
				<< endl;
	}
}
	...
	try {
		...
		if (strcmp(type_data, "geo") == 0) {
			...
			handle->sutra_sensors = new sutra_sensors(context_handle, nsensors,
					nxsub, nvalid, nphase, npup, pdiam, odevice);
			...
			handle->sutra_sensors = new sutra_sensors(context_handle, type_data,
					nsensors, nxsub, nvalid, npix, nphase, nrebin, nfft, ntota,
					npup, pdiam, nphot, lgs, odevice);
		}
	} catch (string &msg) {
		y_error(msg.c_str());
	} catch (char const * msg) {
...
void Y_sensors_initarr(int argc) {
	...
	if (sensors_handler->d_wfs.at(nsensor)->type == "sh") {
		...
		sensors_handler->d_wfs.at(nsensor)->wfs_initarrays(phasemap, hrmap,
				binmap, offsets, pupil, fluxPerSub, isvalid, validsubsx,
				validsubsy, istart, jstart, (cuFloatComplex*) kernel);
	}
	if (sensors_handler->d_wfs.at(nsensor)->type == "pyr") {
		...
		sensors_handler->d_wfs.at(nsensor)->wfs_initarrays(
				(cuFloatComplex*) halfxy, (cuFloatComplex*) offsets, focmask,
				pupil, isvalid, cx, cy, sincar, phasemap, validx, validy);
	}
	if (sensors_handler->d_wfs.at(nsensor)->type == "geo") {
		...
		sensors_handler->d_wfs.at(nsensor)->wfs_initarrays(phasemap, offsets,
				pupil, fluxPerSub, isvalid, validsubsx, validsubsy, istart,
				jstart);
	}
}
...
void Y_yoga_dms(int argc) {
	try {
		...
		dms_struct *handle = (dms_struct *) ypush_obj(&yDMs,
				sizeof(dms_struct));
...
void Y_yoga_loadpzt(int argc) {
	...
	dms_handler->d_dms.at(make_pair("pzt", alt))->pzt_loadarrays(influ,
			influpos, npoints, istart, xoff, yoff);
}
...
void Y_yoga_addkl(int argc) {
	...
	dms_handler->add_dm(context_handle, "kl", alt, dim, ninflu, influsize, nr,
			np, push4imat, odevice);
}
void Y_yoga_loadkl(int argc) {
	...
	dms_handler->d_dms.at(make_pair("kl", alt))->kl_loadarrays(rabas, azbas,
			ord, cr, cp);
}
...
void Y_yoga_getkl(int argc) {
	...
	int xoff =
			(int) ((dms_handler->d_dms.at(make_pair("kl", alt))->d_shape->d_screen->getDims()[1]
					- dms_handler->d_dms.at(make_pair("kl", alt))->d_kl->dim)
					/ 2.0f);
	int yoff = xoff;
...
void rtc_print(void *obj) {
	...
	for (size_t idx = 0; idx < rtc_handler->d_control.size(); idx++) {
		cout << setw(9) << idx + 1 << " | " << setw(5)
				<< rtc_handler->d_control.at(idx)->get_type() << " | "
				<< setw(6) << rtc_handler->d_control.at(idx)->nslope() << " | "
...
void Y_yoga_rtc(int argc) {
	try {
		...
		rtc_struct *handle = (rtc_struct *) ypush_obj(&yRTC,
				sizeof(rtc_struct));
		handle->device = activeDevice;
...
void Y_controller_setdata(int argc) {
	...
	if (strcmp(type_data, "U") == 0) {
		float *data = ygeta_f(argc - 4, &ntot, dims);
		if (rtc_handler->d_control.at(ncontrol)->get_type().compare("ls")
				== 0) {
			SCAST(sutra_controller_ls *, control,
					rtc_handler->d_control.at(ncontrol));
	...
	if (type_obj.compare("binimg") == 0) {
		...
		for (size_t i = 0; i < sensors_handler->nsensors(); i++) {
			carma_host_obj<float> *yho = new carma_host_obj<float>(
					sensors_handler->d_wfs[i]->d_binimg->getDims(), MA_PAGELOCK,
					1);
...
void Y_yoga_aotemplate(int argc) {
	try {
		...
		stringstream buf;
		buf << "unknown error with sutra_aotemplate construction in "
...
void Y_yoga_gettemplate(int argc) {
	...
	if (argc > 1) {
		...
		if (strcmp(type_data, "data") == 0) {
			float *data = ypush_f(
					(long*) aotemplate_handler->d_data->getDims());
