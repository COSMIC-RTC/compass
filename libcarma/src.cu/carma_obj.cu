...
template<class T> __global__ void krnl_fillindex(T *odata, T *idata, int *indx,
		int N) {
...
template<class T> __global__ void krnl_fillvalues(T *odata, unsigned int *indx,
		int N) {
...
template int fillvalues<unsigned int>(unsigned int *d_odata, unsigned int *indx,
		int N);

template<class T> __global__ void getarray2d_krnl(T *odata, T *idata, int tidx0,
...
template<class T> __global__ void fillarray2d_krnl(T *odata, T *idata,
		int tidx0, int Ncol, int NC, int N) {
...
template<class T> __global__ void plusai_krnl(T *odata, T* idata, int i,
		int sgn, int N) {
...
template<class T> __global__ void fillarray2d2_krnl(T *odata, T *idata,
		int tidx0, int Ncol, int NC, int N) {
...
template<class T> __global__ void kern_fill_sym_matrix(char src_uplo, T *data,
		int Ncol, int N) {
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	int tidB;
...
template<class T> int fill_sym_matrix(char src_uplo, T *d_data, int Ncol, int N) {
...
template int fill_sym_matrix<double>(char uplo, double *d_data, int Ncol,
		int N);
