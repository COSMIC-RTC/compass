...
template<class T> __global__ void carma_curand_uniform(curandState *state,
		T *res, int n, float beta);

template<> __global__ void carma_curand_uniform(curandState *s, float *d, int n,
		float beta) {
...
template<> __global__ void carma_curand_uniform(curandState *s, double *d,
		int n, float beta) {
	const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int delta = blockDim.x * gridDim.x;
	for (int idx = tidx; idx < n; idx += delta)
...
template<> __global__ void carma_curand_uniform(curandState *s,
		cuFloatComplex *d, int n, float beta) {
	const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int delta = blockDim.x * gridDim.x;
	for (int idx = tidx; idx < n; idx += delta) {
...
template<> __global__ void carma_curand_uniform(curandState *s,
		cuDoubleComplex *d, int n, float beta) {
	const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int delta = blockDim.x * gridDim.x;
	for (int idx = tidx; idx < n; idx += delta) {
...
template<class T> __global__ void carma_curand_normal(curandState *state,
		T *res, int n, float alpha, float beta);

template<> __global__ void carma_curand_normal(curandState *s, float *d, int n,
...
template<> __global__ void carma_curand_normal(curandState *s, double *d, int n,
		float alpha, float beta) {
	const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int delta = blockDim.x * gridDim.x;
	for (int idx = tidx; idx < n; idx += delta)
...
template<> __global__ void carma_curand_normal(curandState *s,
		cuFloatComplex *d, int n, float alpha, float beta) {
	const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int delta = blockDim.x * gridDim.x;
	for (int idx = tidx; idx < n; idx += delta) {
...
template<> __global__ void carma_curand_normal(curandState *s,
		cuDoubleComplex *d, int n, float alpha, float beta) {
	const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
	const int delta = blockDim.x * gridDim.x;
	for (int idx = tidx; idx < n; idx += delta) {
...
template<class T> __global__ void carma_curand_poisson(curandState *state,
		T *res, int n);
template<> __global__ void carma_curand_poisson(curandState *state, float *res,
		int n) {
	float xm;
	...
	for (int idx = tidx; idx < n; idx += delta) {
		...
		if (xm > 0.0f) {
			...
			do {
				...
				em = floorf(em);
				t = 0.9f * (1.0 + y * y)
						* expf(em * alxm - lgammaf(em + 1.0f) - g);
				tmp = curand_uniform(&state[tidx]);
			} while (tmp > t);
		} else
			em = 0.0f;
...
template<> __global__ void carma_curand_poisson(curandState *state, double *res,
		int n) {
	double xm;
