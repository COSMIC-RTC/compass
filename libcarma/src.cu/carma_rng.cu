...
// PRNG init kernel
__global__ void
initPRNG(curandState *s, int n, int *seed, int offset) {
...
template<>
  __global__ void
  carma_curand_uniform(curandState *s, float *d, int n, float beta) {
...
template<>
  __global__ void
  carma_curand_uniform(curandState *s, double *d, int n, float beta) {
    const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
    const int delta = blockDim.x * gridDim.x;
    for (int idx = tidx; idx < n; idx += delta)
...
template<>
  __global__ void
  carma_curand_uniform(curandState *s, cuFloatComplex *d, int n, float beta) {
    const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
    const int delta = blockDim.x * gridDim.x;
    for (int idx = tidx; idx < n; idx += delta) {
...
template<>
  __global__ void
  carma_curand_uniform(curandState *s, cuDoubleComplex *d, int n, float beta) {
    const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
    const int delta = blockDim.x * gridDim.x;
    for (int idx = tidx; idx < n; idx += delta) {
...
template<class T>
  __global__ void
  carma_curand_normal(curandState *state, T *res, int n, float alpha,
      float beta);

template<>
  __global__ void
  carma_curand_normal(curandState *s, float *d, int n, float alpha,
      float beta) {
    const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
    const int delta = blockDim.x * gridDim.x;
    for (int idx = tidx; idx < n; idx += delta)
...
template<>
  __global__ void
  carma_curand_normal(curandState *s, double *d, int n, float alpha,
      float beta) {
    const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
    const int delta = blockDim.x * gridDim.x;
    for (int idx = tidx; idx < n; idx += delta)
...
template<>
  __global__ void
  carma_curand_normal(curandState *s, cuFloatComplex *d, int n, float alpha,
      float beta) {
    const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
    const int delta = blockDim.x * gridDim.x;
    for (int idx = tidx; idx < n; idx += delta) {
...
template<>
  __global__ void
  carma_curand_normal(curandState *s, cuDoubleComplex *d, int n, float alpha,
      float beta) {
    const int tidx = blockIdx.x * blockDim.x + threadIdx.x;
    const int delta = blockDim.x * gridDim.x;
    for (int idx = tidx; idx < n; idx += delta) {
...
template<>
  __global__ void
  carma_curand_poisson(curandState *state, float *res, int n) {
    float xm;
...
template<>
  __global__ void
  carma_curand_poisson(curandState *state, double *res, int n) {
    double xm;
...
template<>
  int
  carma_prng_cu(float *results, const int nThreads, const int nBlocks,
...
template<>
  int
  carma_prng_cu(double *results, const int nThreads, const int nBlocks,
      curandState *state, char gtype, int n, float alpha, float beta) {
...
template<>
  int
  carma_prng_cu(cuFloatComplex *results, const int nThreads, const int nBlocks,
      curandState *state, char gtype, int n, float alpha, float beta) {
...
template<>
  int
  carma_prng_cu(cuDoubleComplex *results, const int nThreads, const int nBlocks,
      curandState *state, char gtype, int n, float alpha, float beta) {
