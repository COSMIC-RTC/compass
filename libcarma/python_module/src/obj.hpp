...
template <typename T>
void declare_carmaWrap_obj(py::module &mod, std::string suffix)
{

  using Class = carma_obj<T>;

  py::class_<Class>(mod, ("obj_" + suffix).c_str(), py::buffer_protocol())
      ...
      // int getOData()
      .def_property_readonly("o_data", &Class::getODataValue, "TODO") // TODO do the documentation...
      ...
      // void init_reduceCub();
      .def("init_reduceCub", &Class::init_reduceCub, "TODO") // TODO do the documentation...
      // void reduceCub();
      .def("reduceCub", &Class::reduceCub, "TODO") // TODO do the documentation...
      ...
      .def("gemv", [](carma_obj<half> &mat, carma_obj<half> &vectx, float alpha, char op, carma_obj<half> *vecty, float beta) {
        ...
        vecty->gemv(op, __float2half(alpha), &mat, mat.getDims(1), &vectx, 1, __float2half(beta), 1);
        return vecty;
      },
           "this method performs one of the matrix‐vector operations vecty = alpha * op(mat) * vectx + beta * vecty",
           py::arg("vectx"), py::arg("alpha") = 1, py::arg("op") = 'N', py::arg("vecty")=nullptr, py::arg("beta") = 0) // &Class::gemv)
...
*/
  ...
  mod.def(("getri_" + suffix).c_str(), &carma_getri<T>);

  // template<class T>
  // int carma_potri(carma_obj<T> *d_iA);
  mod.def(("potri_" + suffix).c_str(), &carma_potri<T>);
