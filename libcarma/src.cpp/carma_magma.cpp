...
int carma_syevd_gen(magma_vec_t jobz, magma_int_t N, magma_int_t ldda, T *d_mat,
  ...
  CHECK_MAGMA(
    ptr_syevd_gpu(jobz, magma_uplo_const('L'), N, NULL, ldda, NULL, NULL, lda, aux_work, -1,
                  aux_iwork, -1, &info),
    info, info = 0);
...
template<class T, magma_int_t(*ptr_syevd_m) (magma_int_t nrgpu,
                                             ...
                                             magma_int_t liwork,
                                             magma_int_t *info)>
int carma_syevd_m_gen(magma_int_t ngpu, magma_vec_t jobz, magma_int_t N,
                      T *mat, T *eigenvals) {
  magma_int_t *iwork;
  magma_int_t  info = 0, lwork, liwork, lda = N, aux_iwork[1];
  T  aux_work[1];
  ...
  /* Query for workspace sizes */
  CHECK_MAGMA(
    ptr_syevd_m(ngpu, jobz, magma_uplo_const('L'), N, NULL, lda, NULL, aux_work, -1, aux_iwork, -1,
                &info),
    info, info = 0);
...
int carma_getri_gen(long N, T *d_iA) {
  ...
  if (lda * ldda != N * N) std::cerr << "Please provide arrays of proper size" << std::endl; ;
  ...
  CHECK_MAGMA(ptr_getri(N, d_iA, ldda, ipiv, dwork, ldwork, &info), info, cudaFree(
                dwork); magma_free_cpu(ipiv));
...
int carma_getri_cpu_gen(magma_int_t N, T *h_A) {
  ...
  magma_int_t info = 0;
  CHECK_MAGMA(ptr_getrf(&N, &N, h_A, &lda, ipiv, &info), info, magma_free_cpu(work);
              magma_free_cpu(ipiv));
  CHECK_MAGMA(ptr_getri(&N, h_A, &lda, ipiv, work, &lwork, &info), info, magma_free_cpu(
                work); magma_free_cpu(ipiv));
...
template<class T,
         ...
         void(*ptr_getmatrix_1D_col_bcyclic) (magma_int_t m, magma_int_t N, T *dA[],
                                              magma_int_t ldda, T *hA, magma_int_t lda,
                                              magma_int_t num_gpus,
                                              magma_int_t nb)>
# else // if (MAGMA_VERSION_MAJOR == 1) && (MAGMA_VERSION_MINOR <= 5)
template<class T, magma_int_t(*ptr_potrf) (magma_int_t num_gpus,
         ...
         void(*ptr_getmatrix_1D_col_bcyclic) (magma_int_t m, magma_int_t N,
                                              ...
                                              magma_int_t num_gpus, magma_int_t nb)>
# endif // if (MAGMA_VERSION_MAJOR == 1) && (MAGMA_VERSION_MINOR <= 5)
int carma_potri_m_gen(long num_gpus, T *h_A, T *d_iA, long N,
  ...
  CHECK_MAGMA(ptr_potrf(num_gpus, magma_uplo_const('L'), N, d_lA, N, &info),
              ...
              for (long dev = 0; dev < num_gpus; dev++) { magma_setdevice(dev); cudaFree(d_lA[dev]); });
  CHECK_MAGMA(
...
int carma_syevd<float>(char jobz, long N, float *mat, float *eigenvals) {
  TEST_USE_MAGMA(return carma_syevd_gen<float,
                                        magma_ssyevd_gpu>(magma_vec_const(jobz), N, N, mat,
                                                          eigenvals));
}
...
int carma_syevd_m<float>(long ngpu, char jobz, long N, float *mat,
  ...
  TEST_USE_MAGMA(
    magma_int_t N_ = N; return carma_syevd_m_gen<float,
                                                 magma_ssyevd_m>(ngpu, magma_vec_const(jobz), N_,
                                                                 mat, eigenvals));
}
...
int carma_syevd_m<double>(long ngpu, char jobz, long N, double *mat,
  ...
  TEST_USE_MAGMA(
    magma_int_t N_ = N; return carma_syevd_m_gen<double,
                                                 magma_dsyevd_m>(ngpu, magma_vec_const(jobz), N_,
                                                                 mat, eigenvals));
}
...
int carma_syevd_m<float>(long ngpu, char jobz, carma_host_obj<float> *mat,
  ...
  TEST_USE_MAGMA(
    return carma_syevd_m_gen<float,
                             magma_ssyevd_m>(ngpu, magma_vec_const(jobz), N, *mat, *eigenvals));
}
...
int carma_syevd_m<double>(long ngpu, char jobz, carma_host_obj<double> *mat,
  ...
  TEST_USE_MAGMA(
    return carma_syevd_m_gen<double,
                             magma_dsyevd_m>(magma_int_t(ngpu), magma_vec_const(jobz), N, *mat,
                                             *eigenvals));
}
...
int carma_syevd_m<float>(long ngpu, char jobz, carma_host_obj<float> *mat,
  ...
  TEST_USE_MAGMA(
    ...
    if (N != mat->getDims(
          2)) { std::cerr << "Matrix must be symmetric" << std::endl; return EXIT_FAILURE; }
...
int carma_potri_m<float>(long num_gpus, carma_host_obj<float> *h_A,
  ...
  TEST_USE_MAGMA(
    return carma_potri_m_gen<float,
                             ...
                             magma_sgetmatrix_1D_col_bcyclic>(num_gpus, *h_A, *d_iA, N,
                                                              d_iA->getContext()->get_device(d_iA->
                                                                                             getDevice())));
}
...
int carma_potri_m<double>(long num_gpus, carma_host_obj<double> *h_A,
  ...
  TEST_USE_MAGMA(
    return carma_potri_m_gen<double,
                             ...
                             magma_dgetmatrix_1D_col_bcyclic>(num_gpus, *h_A, *d_iA, N,
                                                              d_iA->getContext()->get_device(d_iA->
                                                                                             getDevice())));
}
...
int carma_syevd_cpu<float>(char jobz, long N, float *h_A, float *eigenvals) {
  TEST_USE_MAGMA(
    magma_int_t N_ = N; return carma_syevd_cpu_gen<float,
                                                   lapackf77_ssyevd>(magma_vec_const(jobz), N_, N_,
                                                                     h_A, eigenvals));
}
...
int carma_syevd_cpu<double>(char jobz, long N, double *h_A, double *eigenvals) {
  TEST_USE_MAGMA(
    magma_int_t N_ = N; return carma_syevd_cpu_gen<double,
                                                   lapackf77_dsyevd>(magma_vec_const(jobz), N_, N_,
                                                                     h_A, eigenvals));
}
...
template<>
int carma_magma_spmv<float>(float                    alpha,
                            carma_sparse_obj<float> *dA,
                            carma_obj<float>        *dx,
                            float                    beta,
                            carma_obj<float>        *dy) {
  if (dA->format != "ELL") {
...
template<>
int carma_magma_spmv<double>(double                    alpha,
                             carma_sparse_obj<double> *dA,
                             carma_obj<double>        *dx,
                             double                    beta,
                             carma_obj<double>        *dy) {
  if (dA->format != "ELL") {
