...
template<class T>
int carma_syevd(char jobz, carma_obj<T> *mat, T *eigenvals,
  ...
  //fprintf(stderr, "%s@%d : I'm here\n", __FILE__, __LINE__);
  ptr_syevd_gpu(jobz, 'L', N, *mat, ldda, eigenvals, h_R, lda, h_work, lwork,
      iwork, liwork, &info);
...
template<>
int carma_syevd<float>(char jobz, caObjS *mat, float *eigenvals) {
  return carma_syevd<float>(jobz, mat, eigenvals, magma_ssyevd_gpu);
}
template<>
int carma_syevd<double>(char jobz, caObjD *mat, double *eigenvals) {
  return carma_syevd<double>(jobz, mat, eigenvals, magma_dsyevd_gpu);
}

template<class T>
int carma_syevd(char jobz, carma_obj<T> *mat, T *eigenvals, carma_obj<T> *U) {
  cerr << "carma_syevd: this method not implemented !\n";
  return EXIT_FAILURE;
}
template<>
int carma_syevd<float>(char jobz, caObjS *mat, float *eigenvals, caObjS *U) {
  U->copy(mat, 1, 1);
  return carma_syevd<float>(jobz, U, eigenvals, magma_ssyevd_gpu);
}
template<>
int carma_syevd<double>(char jobz, caObjD *mat, double *eigenvals, caObjD *U) {
  U->copy(mat, 1, 1);
...
template<>
int carma_syevd<float, 1>(char jobz, caObjS *mat, float *eigenvals, caObjS *U) {
  return carma_syevd<float>(jobz, mat, eigenvals, U);
}
template<>
int carma_syevd<double, 1>(char jobz, caObjD *mat, double *eigenvals,
    caObjD *U) {
...
template<class T>
int carma_syevd_m(magma_int_t ngpu, char jobz, T *mat, T *eigenvals,
    magma_int_t N,
    magma_int_t (*ptr_syevd_m)(magma_int_t nrgpu, magma_vec_t jobz, char uplo,
        magma_int_t n, T *a, magma_int_t lda, T *w, T *work, magma_int_t lwork,
...
template<class T>
int carma_syevd_m(long ngpu, char jobz, T *mat, T *eigenvals, long N) {
  cerr << "carma_syevd_m not implemented this this type! \n";
  return EXIT_FAILURE;
}
template<>
int carma_syevd_m<float>(long ngpu, char jobz, float *mat, float *eigenvals,
    long N) {
  return carma_syevd_m<float>(ngpu, jobz, mat, eigenvals, N, magma_ssyevd_m);
}
template<>
int carma_syevd_m<double>(long ngpu, char jobz, double *mat, double *eigenvals,
    long N) {
  return carma_syevd_m<double>(ngpu, jobz, mat, eigenvals, N, magma_dsyevd_m);
}

template<class T>
int carma_syevd_m(long ngpu, char jobz, T *mat, T *eigenvals, T *U, long N) {
  cerr << "carma_syevd_m not implemented this this type! \n";
  return EXIT_FAILURE;
}
template<>
...
int carma_potri(carma_obj<T> *d_iA,
    magma_int_t (*ptr_potrf)(char uplo, magma_int_t n, T *d_A, magma_int_t ldda,
        magma_int_t *info),
  ...
  magma_int_t info;
  ptr_potrf('L', n, *d_iA, n, &info);
  ptr_potri('L', n, *d_iA, n, &info);

  fill_sym_matrix<T>('U', *d_iA, n, n * m);
  return EXIT_SUCCESS;
}
...
template<class T>
int carma_potri(long num_gpus, T *h_A, carma_obj<T> *d_iA,
    magma_int_t (*ptr_potrf)(magma_int_t num_gpus, char uplo, magma_int_t n,
        T **d_A, magma_int_t ldda, magma_int_t *info),
    magma_int_t (*ptr_potri)(char uplo, magma_int_t n, T *d_A, magma_int_t ldda,
        magma_int_t *info), magma_int_t (*ptr_get_potrf_nb)(magma_int_t m),
...
int carma_getri(carma_obj<T> *d_iA,
  ...
  ptr_getrf(n, n, *d_iA, ldda, ipiv, &info);
  ptr_getri(n, *d_iA, ldda, ipiv, dwork, ldwork, &info);
...
template<class T>
int carma_getri(T* h_A, carma_obj<T> *d_iA) {
...
int carma_getri<float>(float* h_A, carma_obj<float> *d_iA) {
  ...
  magma_int_t lda = N;
  magma_int_t ldda = ((N + 31) / 32) * 32;
  magma_ssetmatrix(N, N, h_A, lda, *d_iA, ldda);
...
int carma_getri<double>(double* h_A, carma_obj<double> *d_iA) {
  ...
  magma_int_t lda = N;
  magma_int_t ldda = ((N + 31) / 32) * 32;
  magma_dsetmatrix(N, N, h_A, lda, *d_iA, ldda);
