...
#define MAGMA_TRACE(fmt, args ...) fprintf(stderr,                \
                                           "%s:%d Warning: " fmt, \
                                           __FILE__,              \
                                           __LINE__,              \
                                           ## args)
...
T* create_padded_data(magma_int_t N, const magma_int_t size, magma_int_t& ldda,
	...
	//  MAGMA_TRACE(
	//      "the dimension of the matrix (N=%ld) is not a multiple of %ld -> padding leading dimension
	// to %ld\n",
...
int carma_syevd_gen(Fn const &ptr_syevd_gpu, magma_vec_t jobz, magma_int_t N,
		magma_int_t ldda, T *d_mat, T *h_eigenvals) {
	magma_int_t *iwork;
	...
	CHECK_MAGMA(
			ptr_syevd_gpu(jobz, magma_uplo_const('L'), N, d_mat, ldda,
					h_eigenvals, h_R, lda, h_work, lwork, iwork, liwork, &info),
			info, info = 0);
...
template<class T, typename Fn>
int carma_syevd_m_gen(Fn const &ptr_syevd_m, magma_int_t ngpu, magma_vec_t jobz,
		magma_int_t N, T *mat, T *eigenvals) {
	magma_int_t *iwork;
	magma_int_t info = 0, lwork, liwork, lda = N, aux_iwork[1];
	T aux_work[1];
	T *h_work;
	...
	CHECK_MAGMA(
			ptr_syevd_m(ngpu, jobz, magma_uplo_const('L'), N, mat, lda,
					eigenvals, h_work, lwork, iwork, liwork, &info), info,
			info = 0);
...
template<class T, typename Fn>
int carma_svd_cpu_gen(Fn const &ptr_gesvd, carma_host_obj<T> *mat, carma_host_obj<T> *eigenvals,
	...
	CHECK_MAGMA(
			ptr_gesvd(magma_vec_const('A'), magma_vec_const('A'), m, n, *tmp, m,
					*eigenvals, *mes2mod, m, *mod2act, n, *h_work, lwork,
					&info), info, delete h_work; delete tmp; delete[] dims_data);
...
template<class T, typename Fn>
int carma_potri_gen(Fn const &ptr_potrf, Fn const &ptr_potri, magma_int_t N, magma_int_t ldda, T *d_iA,
		carma_device *device) {
	magma_int_t info = 0;
...
template<class T, typename Fn>
int carma_syevd_cpu_gen(Fn const &ptr_syevd_cpu, magma_vec_t jobz, magma_int_t N, magma_int_t lda,
	...
	CHECK_MAGMA(
			...
			cudaFreeHost(h_R); magma_free_cpu(iwork););
...
int carma_getri_gen(Fnf const &ptr_getrf, Fni const &ptr_getri, Fni_nb const &ptr_get_getri_nb, long N, T *d_iA) {
	magma_int_t *ipiv;
	...
	CHECK_MAGMA(ptr_getrf(N, N, d_iA, ldda, ipiv, &info), info,
			cudaFree(dwork); magma_free_cpu(ipiv));
...
template<class T, typename Fnf, typename Fni, typename Fni_nb>
int carma_getri_cpu_gen(Fnf const &ptr_getrf, Fni const &ptr_getri, Fni_nb const &ptr_get_getri_nb, magma_int_t N, T *h_A) {
	magma_int_t *ipiv;
	...
	CHECK_MAGMA(ptr_getrf(&N, &N, h_A, &lda, ipiv, &info), info,
			magma_free_cpu(work); magma_free_cpu(ipiv));
...
template<class T, typename Fnf, typename Fni, typename Fnf_nb, typename FnSetMat, typename FnGetMat>
int carma_potri_m_gen(Fnf const &ptr_potrf, Fni const &ptr_potri, Fnf_nb const &ptr_get_potrf_nb,
		FnSetMat const &ptr_setmatrix_1D_col_bcyclic, FnGetMat const &ptr_getmatrix_1D_col_bcyclic,
	...
	CHECK_MAGMA(
			ptr_getmatrix_1D_col_bcyclic(N, N, d_lA, ldda, h_A, lda, num_gpus,
					nb), info,
...
int carma_potri_cpu_gen(Fn const &ptr_potrf, Fn const &ptr_potri, Fn_copy const &ptr_copy, magma_int_t N, T *h_A) {
	magma_int_t info = 0;
	char uplo = 'L';
	magma_int_t one = 1;
...
# ifdef USE_MAGMA_SPARSE
template<class T, class var, typename Fn_mtransfert, typename Fn_mconver, typename Fn_mfree>
var carma_csr2ell_gen(Fn_mtransfert const &mtransfer, Fn_mconver const &mconvert, Fn_mfree const &mfree, carma_sparse_obj<T> *dA) {
	var A;
...
int carma_spmv_gen(Fn spmv, T alpha, varM dA, carma_obj<T> *dx, T beta, carma_obj<T> *dy) {
	varV x, y;
...
int carma_syevd<float>(char jobz, long N, float *mat, float *eigenvals) {
	TEST_USE_MAGMA(
			return carma_syevd_gen(magma_ssyevd_gpu, magma_vec_const(jobz), N, N, mat, eigenvals));
}

template<>
int carma_syevd<double>(char jobz, long N, double *mat, double *eigenvals) {
	TEST_USE_MAGMA(
			return carma_syevd_gen(magma_dsyevd_gpu, magma_vec_const(jobz), N, N, mat, eigenvals));
}
...
int carma_syevd<float, 1>(char jobz, caObjS *mat,
	...
	TEST_USE_MAGMA(
			return carma_syevd_gen(magma_ssyevd_gpu, magma_vec_const(jobz), N, N, mat->getData(), eigenvals->getData()));
}
...
int carma_syevd<double, 1>(char jobz, caObjD *mat,
	...
	TEST_USE_MAGMA(
			return carma_syevd_gen(magma_dsyevd_gpu, magma_vec_const(jobz), N, N, mat->getData(), eigenvals->getData()));
}
...
int carma_syevd_m<float>(long ngpu, char jobz, long N, float *mat,
		float *eigenvals) {
	TEST_USE_MAGMA(
			magma_int_t N_ = N; return carma_syevd_m_gen(magma_ssyevd_m, ngpu, magma_vec_const(jobz), N_, mat, eigenvals));
}
...
int carma_syevd_m<double>(long ngpu, char jobz, long N, double *mat,
		double *eigenvals) {
	TEST_USE_MAGMA(
			magma_int_t N_ = N; return carma_syevd_m_gen(magma_dsyevd_m, ngpu, magma_vec_const(jobz), N_, mat, eigenvals));
}
...
int carma_syevd_m<float>(long ngpu, char jobz, carma_host_obj<float> *mat,
	...
	TEST_USE_MAGMA(
			return carma_syevd_m_gen(magma_ssyevd_m, ngpu, magma_vec_const(jobz), N, mat->getData(), eigenvals->getData()));
}
...
int carma_syevd_m<double>(long ngpu, char jobz, carma_host_obj<double> *mat,
	...
	TEST_USE_MAGMA(
			return carma_syevd_m_gen(magma_dsyevd_m, magma_int_t(ngpu), magma_vec_const(jobz), N, mat->getData(), eigenvals->getData()));
}
...
int carma_syevd_m<float>(long ngpu, char jobz, carma_host_obj<float> *mat,
	...
	TEST_USE_MAGMA(
			...
			if (N != mat->getDims( 2)) { std::cerr << "Matrix must be symmetric" << std::endl; return EXIT_FAILURE; }

			magma_int_t lda = N; lapackf77_slacpy(MagmaUpperLowerStr, &N, &N, *mat, &lda, *U, &lda); return carma_syevd_m_gen<float>(magma_ssyevd_m, ngpu, magma_vec_const(jobz), N, *U, *eigenvals));
}
...
int carma_syevd_m<double>(long ngpu, char jobz, carma_host_obj<double> *mat,
	...
	TEST_USE_MAGMA(
			...
			if (N != mat->getDims( 2)) { std::cerr << "Matrix must be symmetric" << std::endl; return EXIT_FAILURE; } magma_int_t lda = N; lapackf77_dlacpy(MagmaUpperLowerStr, &N, &N, *mat, &lda, *U, &lda); return carma_syevd_m_gen(magma_dsyevd_m, ngpu, magma_vec_const(jobz), N, U->getData(), eigenvals->getData()));
}
...
int carma_svd_cpu<float>(carma_host_obj<float> *mat,
	...
	TEST_USE_MAGMA(return carma_svd_cpu_gen<float>(magma_sgesvd, mat, eigenvals, mod2act, mes2mod));
}
...
int carma_svd_cpu<double>(carma_host_obj<double> *mat,
	...
	TEST_USE_MAGMA(return carma_svd_cpu_gen<double>(magma_dgesvd, mat, eigenvals, mod2act, mes2mod));
}
...
int carma_potri<float>(carma_obj<float> *d_iA) {
	...
	TEST_USE_MAGMA(return carma_potri_gen<float>(magma_spotrf_gpu, magma_spotri_gpu, N, N, *d_iA, d_iA->getContext()->get_device(d_iA->getDevice())));
}
...
int carma_potri<double>(carma_obj<double> *d_iA) {
	...
	TEST_USE_MAGMA(return carma_potri_gen<double>(magma_dpotrf_gpu, magma_dpotri_gpu, N, N, *d_iA, d_iA->getContext()->get_device(d_iA->getDevice())));
}
...
int carma_potri_m<float>(long num_gpus, carma_host_obj<float> *h_A,
	...
	TEST_USE_MAGMA(return carma_potri_m_gen<float>(magma_spotrf_mgpu,
			magma_spotri_gpu, magma_get_spotrf_nb,
			magma_ssetmatrix_1D_col_bcyclic,
			magma_sgetmatrix_1D_col_bcyclic,
			num_gpus, *h_A, *d_iA, N, d_iA->getContext()->get_device(d_iA-> getDevice())));
}
...
int carma_potri_m<double>(long num_gpus, carma_host_obj<double> *h_A,
	...
	TEST_USE_MAGMA(return carma_potri_m_gen<double>(
			magma_dpotrf_mgpu,
			magma_dpotri_gpu, magma_get_dpotrf_nb,
...
int carma_syevd_cpu<float>(char jobz, carma_host_obj<float> *h_A,
	...
	TEST_USE_MAGMA(return carma_syevd_cpu_gen<float>(lapackf77_ssyevd, magma_vec_const(jobz), N, N, *h_A, *eigenvals));
}
...
int carma_syevd_cpu<double>(char jobz, carma_host_obj<double> *h_A,
	...
	TEST_USE_MAGMA(return carma_syevd_cpu_gen<double>(lapackf77_dsyevd, magma_vec_const(jobz), N, N, *h_A, *eigenvals));
}

template<>
int carma_syevd_cpu<float>(char jobz, long N, float *h_A, float *eigenvals) {
	TEST_USE_MAGMA(magma_int_t N_ = N; return carma_syevd_cpu_gen<float>(lapackf77_ssyevd, magma_vec_const(jobz), N_, N_, h_A, eigenvals));
}

template<>
int carma_syevd_cpu<double>(char jobz, long N, double *h_A, double *eigenvals) {
	TEST_USE_MAGMA(magma_int_t N_ = N; return carma_syevd_cpu_gen<double>(lapackf77_dsyevd, magma_vec_const(jobz), N_, N_, h_A, eigenvals));
}
...
int carma_axpy_cpu<float>(long N, float alpha, float *h_X, long incX,
		float *h_Y, long incY) {
	TEST_USE_MAGMA(
			magma_int_t tmp_N = N; magma_int_t tmp_incX = incX; magma_int_t tmp_incY = incY; blasf77_saxpy(&tmp_N, &alpha, h_X, &tmp_incX, h_Y, &tmp_incY));
	return EXIT_SUCCESS;
}
...
int carma_axpy_cpu<double>(long N, double alpha, double *h_X, long incX,
	...
	TEST_USE_MAGMA(
			magma_int_t tmp_N = N; magma_int_t tmp_incX = incX; magma_int_t tmp_incY = incY; blasf77_daxpy(&tmp_N, &alpha, h_X, &tmp_incX, h_Y, &tmp_incY));
...
int carma_gemm_cpu<float>(char transa, char transb, long m, long n, long k,
		float alpha, float *A, long lda, float *B, long ldb, float beta,
		float *C, long ldc) {
	TEST_USE_MAGMA(
			magma_int_t tmp_m = m; magma_int_t tmp_n = n; magma_int_t tmp_k = k; magma_int_t tmp_lda = lda; magma_int_t tmp_ldb = ldb; magma_int_t tmp_ldc = ldc; blasf77_sgemm(&transa, &transb, &tmp_m, &tmp_n, &tmp_k, &alpha, A, &tmp_lda, B, &tmp_ldb, &beta, C, &tmp_ldc));
	return EXIT_SUCCESS;
}
...
int carma_gemm_cpu<double>(char transa, char transb, long m, long n, long k,
		...
		double *C, long ldc) {
	TEST_USE_MAGMA(
			magma_int_t tmp_m = m; magma_int_t tmp_n = n; magma_int_t tmp_k = k; magma_int_t tmp_lda = lda; magma_int_t tmp_ldb = ldb; magma_int_t tmp_ldc = ldc; blasf77_dgemm(&transa, &transb, &tmp_m, &tmp_n, &tmp_k, &alpha, A, &tmp_lda, B, &tmp_ldb, &beta, C, &tmp_ldc));
	return EXIT_SUCCESS;
}

template<>
int carma_magma_csr2ell<float>(carma_sparse_obj<float> *dA) {
	TEST_USE_SMAGMA(dA->s_spMat = carma_csr2ell_gen<float, magma_s_sparse_matrix>(magma_s_mtransfer, magma_s_mconvert,
					magma_s_mfree, dA);
...
int carma_magma_csr2ell<double>(carma_sparse_obj<double> *dA) {
	TEST_USE_SMAGMA(dA->d_spMat = carma_csr2ell_gen<double, magma_d_sparse_matrix>(magma_d_mtransfer, magma_d_mconvert,
			magma_d_mfree, dA); return EXIT_SUCCESS;
...
int carma_magma_spmv<float>(float alpha, carma_sparse_obj<float> *dA,
		carma_obj<float> *dx, float beta, carma_obj<float> *dy) {
	if (dA->format != "ELL") {
		DEBUG_TRACE(
				"carma_fill_magma_sparse_matrix needs a ELL matrix as input");
		return EXIT_FAILURE;
	}
	TEST_USE_SMAGMA(return carma_spmv_gen<float, magma_s_sparse_matrix,
			magma_s_vector>(magma_s_spmv, alpha, dA->s_spMat, dx, beta, dy);
...
int carma_magma_spmv<double>(double alpha, carma_sparse_obj<double> *dA,
		carma_obj<double> *dx, double beta, carma_obj<double> *dy) {
	if (dA->format != "ELL") {
		DEBUG_TRACE(
				"carma_fill_magma_sparse_matrix needs a ELL matrix as input");
		return EXIT_FAILURE;
	}
	TEST_USE_SMAGMA(return carma_spmv_gen<double, magma_d_sparse_matrix,
			magma_d_vector>(magma_d_spmv, alpha, dA->d_spMat, dx, beta, dy);
...
int carma_sparse_magma_free<float>(carma_sparse_obj<float> *dA) {
	...
	TEST_USE_SMAGMA(
			magma_queue_t queue; magma_queue_create(/*devices[ opts->device ],*/&queue); int ret = magma_s_mfree(&(dA->s_spMat), queue); magma_queue_destroy(queue);

			dA->format = "NONE"; dA->nz_elem = dA->s_spMat.nnz; dA->d_data = dA->s_spMat.dval; dA->d_rowind = dA->s_spMat.drow; dA->d_colind = dA->s_spMat.dcol; return ret;
...
int carma_sparse_magma_free<double>(carma_sparse_obj<double> *dA) {
	...
	TEST_USE_SMAGMA(
			magma_queue_t queue; magma_queue_create(/*devices[ opts->device ],*/&queue); int ret = magma_d_mfree(&(dA->d_spMat), queue); magma_queue_destroy(queue);

			dA->format = "NONE"; dA->nz_elem = dA->d_spMat.nnz; dA->d_data = dA->d_spMat.dval; dA->d_rowind = dA->d_spMat.drow; dA->d_colind = dA->d_spMat.dcol; return ret;
