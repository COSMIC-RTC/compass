...
cublasStatus_t carma_rot<double>(cublasHandle_t cublas_handle, int n,
		double *vectx, int incx, double *vecty, int incy, double sc,
		double ss) {
...
cublasStatus_t carma_gemv<double>(cublasHandle_t cublas_handle, char trans,
		int m, int n, double alpha, double *matA, int lda, double *vectx,
		int incx, double beta, double *vecty, int incy) {
	cublasOperation_t trans2 = carma_char2cublasOperation(trans);
...
cublasStatus_t carma_syrk<float>(cublasHandle_t cublas_handle,
		cublasFillMode_t uplo, char transa, int n, int k, float alpha,
		float *matA, int lda, float beta, float *matC, int ldc) {
	cublasOperation_t transa2 = carma_char2cublasOperation(transa);
...
cublasStatus_t carma_geam<float>(cublasHandle_t cublas_handle, char transa,
		char transb, int m, int n, float alpha, float *matA, int lda,
		float beta, float *matB, int ldb, float *matC, int ldc) {
	cublasOperation_t transa2 = carma_char2cublasOperation(transa);
	cublasOperation_t transb2 = carma_char2cublasOperation(transb);
...
cublasStatus_t carma_geam<double>(cublasHandle_t cublas_handle, char transa,
		char transb, int m, int n, double alpha, double *matA, int lda,
		double beta, double *matB, int ldb, double *matC, int ldc) {
	cublasOperation_t transa2 = carma_char2cublasOperation(transa);
	cublasOperation_t transb2 = carma_char2cublasOperation(transb);
...
cublasStatus_t carma_dgmm<double>(cublasHandle_t cublas_handle,
		cublasSideMode_t side, int m, int n, double *matA, int lda,
		double *vectx, int incx, double *matC, int ldc) {
...
int carma_obj<T_data>::host2deviceVect(T_data *data, int incx, int incy) {
	...
	carma_checkCublasStatus(
			cublasSetVector(this->nb_elem, sizeof(T_data), data, incx,
					this->d_data, incy));
	return EXIT_SUCCESS;
}
template int
...
int carma_obj<T_data>::device2hostVect(T_data *data, int incx, int incy) {
	...
	carma_checkCublasStatus(
			cublasGetVector(this->nb_elem, sizeof(T_data), this->d_data, incx,
					data, incy));
	return EXIT_SUCCESS;
}
...
int carma_obj<T_data>::host2deviceMat(T_data *data, int lda, int ldb) {
	 ...
	 */
	carma_checkCublasStatus(
			cublasSetMatrix(this->dims_data[1], this->dims_data[2],
					sizeof(T_data), data, lda, this->d_data, ldb));
	return EXIT_SUCCESS;
}
...
int carma_obj<T_data>::device2hostMat(T_data *data, int lda, int ldb) {
	 ...
	 */
	carma_checkCublasStatus(
			cublasGetMatrix(this->dims_data[1], this->dims_data[2],
					sizeof(T_data), this->d_data, lda, data, ldb));
	return EXIT_SUCCESS;
}
