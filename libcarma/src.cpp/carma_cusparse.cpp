...
cusparseStatus_t carma_checkCusparseStatus_v2(cusparseStatus_t status, int line,
  ...
  case CUSPARSE_STATUS_ALLOC_FAILED:
    cerr
        << "Cusparse error : Resource allocation failed inside the CUSPARSE library. !!!!!\n";
    break;
  case CUSPARSE_STATUS_INVALID_VALUE:
    cerr
        << "Cusparse error : An unsupported value or parameter was passed to the function.\n";
    break;
  case CUSPARSE_STATUS_ARCH_MISMATCH:
    cerr
        << "Cusparse error : The function requires a feature absent from the device architecture.\n";
    break;
  case CUSPARSE_STATUS_MAPPING_ERROR:
    cerr
        << "Cusparse error : An access to GPU memory space failed, which is usually caused by a failure to bind a texture.\n";
    break;
  ...
  case CUSPARSE_STATUS_MATRIX_TYPE_NOT_SUPPORTED:
    cerr
        << "Cusparse error : The matrix type is not supported by this function.\n";
    break;
// Define not supported status for pre-6.0 compatibility.
#if CUDA_VERSION >= 6000
  case CUSPARSE_STATUS_ZERO_PIVOT:
    cerr
        << "Cusparse error : An entry of the matrix is either structural zero or numerical zero (singular block)\n";
    break;
#endif
  }
...
cusparseStatus_t carma_csr2dense_gen(carma_sparse_obj<T_data>* A, T_data *B) {
  ...
  if (status != CUSPARSE_STATUS_SUCCESS) {
    cerr << "Error | carma_csr2dense (sparse) | csr2dense failed" << endl;
