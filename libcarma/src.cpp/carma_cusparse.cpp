...
cusparseStatus_t carma_checkCusparseStatus_v2(cusparseStatus_t status, int line,
  ...
  case CUSPARSE_STATUS_NOT_INITIALIZED:
    std::cerr << "Cusparse error : The CUSPARSE library was not initialized." << std::endl;
    break;
  case CUSPARSE_STATUS_ALLOC_FAILED:
    std::cerr
        << "Cusparse error : Resource allocation failed inside the CUSPARSE library. !!!!!" << std::endl;
    break;
  case CUSPARSE_STATUS_INVALID_VALUE:
    std::cerr
        << "Cusparse error : An unsupported value or parameter was passed to the function." << std::endl;
    break;
  case CUSPARSE_STATUS_ARCH_MISMATCH:
    std::cerr
        << "Cusparse error : The function requires a feature absent from the device architecture." << std::endl;
    break;
  case CUSPARSE_STATUS_MAPPING_ERROR:
    std::cerr
        << "Cusparse error : An access to GPU memory space failed, which is usually caused by a failure to bind a texture." << std::endl;
    break;
  ...
  case CUSPARSE_STATUS_INTERNAL_ERROR:
    std::cerr << "Cusparse error : An internal CUSPARSE operation failed." << std::endl;
    break;
  case CUSPARSE_STATUS_MATRIX_TYPE_NOT_SUPPORTED:
    std::cerr
        << "Cusparse error : The matrix type is not supported by this function." << std::endl;
    break;
// Define not supported status for pre-6.0 compatibility.
#if CUDA_VERSION >= 6000
  case CUSPARSE_STATUS_ZERO_PIVOT:
    std::cerr
        << "Cusparse error : An entry of the matrix is either structural zero or numerical zero (singular block)" << std::endl;
    break;
#endif
  }
