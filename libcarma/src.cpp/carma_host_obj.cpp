...
template<class T_data>
  void
  carma_host_obj<T_data>::init(const long *dims_data, T_data *data,
      MemAlloc mallocType, int nb_streams) {
...
template<class T_data>
  void
  carma_host_obj<T_data>::get_devpntr(void **pntr_dev) {
...
template<class T_data>
  int
  carma_host_obj<T_data>::get_nbStreams() {
...
template<class T_data>
  int
  carma_host_obj<T_data>::add_stream() {
...
template<class T_data>
  int
  carma_host_obj<T_data>::add_stream(int nb) {
    if (this->mallocType != MA_MALLOC)
...
template<class T_data>
  cudaStream_t
  carma_host_obj<T_data>::get_cudaStream_t(int stream) {
    return this->streams->get_stream(stream);
  }

template<class T_data>
  int
  carma_host_obj<T_data>::del_stream() {
...
template<class T_data>
  int
  carma_host_obj<T_data>::del_stream(int nb) {
...
template<class T_data>
  int
  carma_host_obj<T_data>::wait_stream(int stream) {
...
template<class T_data>
  int
  carma_host_obj<T_data>::wait_all_streams() {
...
template<class T_data>
  int
  carma_host_obj<T_data>::fill_from(T_data *data) {
    if (get_nbStreams() > 1) {
...
template<class T_data>
  int
  carma_host_obj<T_data>::fill_into(T_data *data) {
    if (get_nbStreams() > 1) {
      int nstreams = get_nbStreams();
      for (int i = 0; i < nstreams; i++) {
        cutilSafeCall(
...
template<class T_data>
  int
  carma_host_obj<T_data>::cpy_obj(carma_obj<T_data> *caObj,
      cudaMemcpyKind flag) {
...
template<class T_data>
  int
  carma_host_obj<T_data>::cpy_obj(carma_obj<T_data> *caObj, cudaMemcpyKind flag,
      unsigned int stream) {
