...
template carma_host_obj<double>::carma_host_obj(const long *dims_data,
		double *data, MemAlloc mallocType);
template carma_host_obj<int>::carma_host_obj(const long *dims_data, int *data,
		MemAlloc mallocType);
template carma_host_obj<unsigned int>::carma_host_obj(const long *dims_data,
...
template carma_host_obj<float>::carma_host_obj(const long *dims_data,
		float *data, MemAlloc mallocType, int nb_streams);
template carma_host_obj<double>::carma_host_obj(const long *dims_data,
		double *data, MemAlloc mallocType, int nb_streams);
template carma_host_obj<int>::carma_host_obj(const long *dims_data, int *data,
		MemAlloc mallocType, int nb_streams);
template carma_host_obj<unsigned int>::carma_host_obj(const long *dims_data,
...
void carma_host_obj<T_data>::get_devpntr(void **pntr_dev) {
	...
	cutilSafeCall(
			cudaHostGetDevicePointer(pntr_dev, (void * )(this->h_data), 0));
}
...
int carma_host_obj<T_data>::cpy_obj(carma_obj<T_data> *caObj,
	...
	if (get_nbStreams() > 1) {
		...
		cutilSafeCall(
				cudaMemcpyAsync(data_dst, data_src, sizeof(T_data) * nb_elem,
