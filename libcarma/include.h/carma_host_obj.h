...
template<class T_data>
  ...
  protected:
    ...
    void
    init(const long *dims_data, T_data *data, MemAlloc mallocType,
        int nb_streams);
  ...
  public:
    ...
    int
    cpy_obj(carma_obj<T_data>* caObj, cudaMemcpyKind flag);
    int
    cpy_obj(carma_obj<T_data>* caObj, cudaMemcpyKind flag, unsigned int stream);
    ...
    /**< Memory transfer */
    int
    fill_from(T_data *data);
    int
    fill_into(T_data *data);
...
template<class T_data>
  int
  carma_svd_cpu(carma_host_obj<T_data> *imat, carma_host_obj<T_data> *eigenvals,
...
template<class T>
  int
  carma_svd_cpu(long N, long M, T *imat, T *eigenvals, T *mod2act, T *mes2mod);
template<class T>
  int
...
template<class T>
  int
  carma_syevd_cpu(char jobz, long N, T *h_A, T *eigenvals);
template<class T>
  int
