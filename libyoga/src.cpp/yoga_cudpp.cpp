...
template<class T> int yoga_obj<T>::sort_init(bool keysOnly)
{
...
template int yObjS::sort_init(bool keysOnly);
template int yObjI::sort_init(bool keysOnly);

template<class T> int yoga_obj<T>::host2deviceInd(unsigned int *ind){
  cutilSafeCall(cudaMemcpy(this->values,ind,this->nb_elem * sizeof(unsigned int),cudaMemcpyHostToDevice));
  return EXIT_SUCCESS;
}

template int yObjS::host2deviceInd(unsigned int *ind);

template<class T> int yoga_obj<T>::device2hostInd(unsigned int *ind){
  cutilSafeCall(cudaMemcpy(ind,this->values,this->nb_elem * sizeof(unsigned int),cudaMemcpyDeviceToHost));
  return EXIT_SUCCESS;
}

template int yObjS::device2hostInd(unsigned int *ind);

template<class T> int yoga_obj<T>::device2deviceInd(unsigned int *src){
...
template int yObjS::device2deviceInd(unsigned int *src);
...
template int yObjS::sort();
template int yObjI::sort();

template<class T> int yoga_obj<T>::config_scan(char *type, int dir, int incl)
{
...
template int yObjS::config_scan(char *type, int dir, int incl);
template int yObjI::config_scan(char *type, int dir, int incl);
...
template float yObjS::scan();
template int yObjI::scan();


template<class T> int yoga_obj<T>::compact_init()
{
  if (this->getValues() == NULL){
    cudaMalloc((void **)&(this->values),this->nb_elem * sizeof(unsigned int));
  }
...
template int yObjS::compact_init();
template int yObjI::compact_init();
...
template int yObjS::compact(yObjS *dest);
template int yObjI::compact(yObjI *dest);
...
template int yObjS::compact(yObjS *dest,unsigned int *values);
template int yObjI::compact(yObjI *dest,unsigned int *values);
