...
template <class T> __device__ T yoga_sin(T data);
template <> __device__ float yoga_sin(float data )
{
  return  sinf(data);
}

template <> __device__ double yoga_sin(double data )
{
return sin(data);
}
...
template<class T> __global__ void generic1d(T *odata, T *idata, int N)
{
  ...
  while (tid < N) {
    odata[tid] = yoga_sin(2.0f * idata[tid]);
...
template<class T> __global__ void generic2d(T *odata, T *idata, int N)
{
  ...
  cache[BLOCK_SZ-1-threadIdx.x][BLOCK_SZ-1-threadIdx.y] =  yoga_sin(2.0f * idata[tid]);
...
template<class T> int yoga_plus(T *d_odata,T alpha,int N)
{
...
template int yoga_plus<float>(float *d_odata,float alpha,int N);

template int yoga_plus<double>(double *d_odata,double alpha,int N);
...
template int yoga_plusai<float>(float *d_odata,float *d_idata, int i,int sgn, int N);

template int yoga_plusai<double>(double *d_odata,double *d_idata, int i,int sgn, int N);
