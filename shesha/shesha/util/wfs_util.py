...
def write_wfs_custom_fits(
    ...
    primary_hdu.header["SUBAPD"] = (
        subap_diam,
        "subaperture diameter (pix):side of the subap",
    )
...
def export_custom_wfs(file_name: str, p_wfs, p_geom, *, p_tel=None):
    ...
    if p_wfs.get_validsubsx().max() * scale + p_wfs.get_subapd() - diam != 0:
        ...
        print(
            "          the wfs settings span a diameter of {}m (procided : {})".format(
    ...
    wfs_custom = write_wfs_custom_fits(
        file_name,
        p_wfs.type,
        xpos,
        ypos,
