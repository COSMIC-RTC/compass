...
def make_pupil(dim, pupd, tel, xc=-1, yc=-1, real=0, halfSpider=False):
    ...
    if tel.type_ap == ApertureType.EELT_NOMINAL:
        ...
        return make_pupil_generic(
            dim, pupd, tel.t_spiders, tel.spiders_type, xc, yc, real, tel.cobs
        )
    else:
...
def make_pupil_generic(
    ...
    if real == 1:
        ...
        pup = (pup < (pupd + 1.0) / 2).astype(np.float32)

    if cobs > 0:
        if real == 1:
            pup -= np.exp(-((util.dist(dim, xc, yc) / (pupd * cobs * 0.5)) ** 60.0)) ** 0.69314
        else:
            pup -= (util.dist(dim, xc, yc) < (pupd * cobs + 1.0) * 0.5).astype(np.float32)
            ...
            if spiders_type == "four":
                ...
                spiders_map = (
                    (X.T > (X + t_spiders / s4_2) * t4) + (X.T < (X - t_spiders / s4_2) * t4)
                ).astype(np.float32)
                spiders_map *= (
                    (X.T > (-X + t_spiders / s4_2) * t4) + (X.T < (-X - t_spiders / s4_2) * t4)
                ).astype(np.float32)
...
def make_VLT(dim, pupd, tel):
    ...
    if tel.t_spiders == -1:
        print("force t_spider =%5.3f" % (0.09 / 18.0))
        tel.set_t_spiders(0.09 / 18.0)
    angle = 50.5 * np.pi / 180.0  # --> 50.5 degre *2 d'angle entre les spiders
    ...
    if tel.t_spiders == -1:
        ...
        spiders_map = (
            (
                (X.T > (X - tel.cobs / 2 + tel.t_spiders / np.sin(angle)) * np.tan(angle))
    ...
    if (tel.type_ap == ApertureType.GENERIC) or (tel.type_ap == ApertureType.VLT):
        ...
        if os.path.isfile(ab_file):
            ...
            if np.sum(phase_tt) != 0:
                phase_tt *= std_tt / np.sqrt(1.0 / N_EELT * np.sum(phase_tt[np.where(pup)] ** 2))
...
def generateEeltPupilMask(
    ...
    if cobs > 0:
        obstru = (
            util.dist(pup.shape[0], pup.shape[0] // 2 + 0.5, pup.shape[0] // 2 + 0.5)
            >= (pup.shape[0] * cobs + 1.0) * 0.5
        ).astype(np.float32)
        pup = pup * obstru
    return pup
...
def compute1Spider(nspider, N, dspider, i0, j0, scale, rot):
    ...
    w = 2 * np.pi / 6
    i = nspider
    nn = abs(X * np.cos(i * w - rot) + Y * np.sin(i * w - rot)) < dspider / 2.0
    a[nn] = False
    return a
...
def fillHalfSpider(N, nspider, dspider, i0, j0, scale, rot):
    ...
    for i in range(nspider):
        right = (X * np.cos(i * w - rot) + Y * np.sin(i * w - rot) < dspider / 2) * (
            X * np.cos(i * w - rot) + Y * np.sin(i * w - rot) > 0.0
        )
...
def createHexaPattern(pitch, supportSize):
    ...
    x = x.flatten()
    y = y.flatten()
    peak_axis = np.append(x, x + pitch / 2.0)  # axe dirige selon sommet
    flat_axis = np.append(y, y + pitch * V3 / 2.0)  # axe dirige selon plat
    return flat_axis, peak_axis


def generateCoordSegments(
    D,
    rot,
    pitch=1.244683637214,
    nseg=33,
...
def generateSegmentProperties(
    ...
    if n != 3:
        ...
        if softGap != 0:
            ...
            for i in range(nseg):
                indx, indy, distedge = fillPolygon(
                    hx[:, i],
                    hy[:, i],
                    i0 - ix0[i],
                    j0 - iy0[i],
                ...
                pupil[indx + ix0[i], indy + iy0[i]] = attribute[i] * (
                    1.0 - (gap / scale / np.pi) / (1 + (distedge / scale) ** 2)
                )
        else:
            # Hard gaps
            for i in range(nseg):
