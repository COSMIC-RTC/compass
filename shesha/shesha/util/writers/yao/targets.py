...
def write_targets(file_name, tars, *, sub_system=1):
    ...
    f.write(
        "\ntarget.lambda       = &("
        + np.array2string(
            np.array([t.Lambda for t in tars]),
            separator=",",
            max_line_width=300,
        )
        + ");"
    ...
    f.write(
        "\ntarget.xposition    = &("
        + np.array2string(np.array([t.xpos for t in tars]), separator=",", max_line_width=300)
        + ");"
    )  # &mmsepos_asec1;
    f.write(
        "\ntarget.yposition    = &("
        + np.array2string(np.array([t.ypos for t in tars]), separator=",", max_line_width=300)
        + ");"
    )  # &mmsepos_asec2;
    dispzoom = np.ones((len(tars)))
    f.write(
        "\ntarget.dispzoom     = &("
        + np.array2string(dispzoom, separator=",", max_line_width=300)
        + ") ; // not set by compass"
    )
