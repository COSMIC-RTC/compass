...
def write_atm(file_name, atm, screen_file, zenithangle):
    ...
    for i in range(2, atm.nscreens + 1):
        indexList += ',"' + str(i) + '"'
    f.write("\natm.screen = &(" + screen_file + "+[" + indexList + ']+".fits")')
    f.write(
        "\natm.layerspeed  = &("
        + np.array2string(
            atm.windspeed / np.cos(np.pi * zenithangle / 180),
            separator=",",
            max_line_width=300,
        )
        + ");"
    ...
    f.write(
        "\natm.layeralt    = &("
        + np.array2string(
            atm.alt * np.cos(np.pi * zenithangle / 180),
            separator=",",
            max_line_width=300,
        )
        + ");"
    ...
    f.write(
        "\natm.layerfrac   = &("
        + np.array2string(atm.frac, separator=",", max_line_width=300)
        + ");"
    )
    f.write(
        "\natm.winddir     = &("
        + np.array2string(-(atm.winddir + 90) % 360, separator=",", max_line_width=300)
        + ");"
    )
    f.close()
