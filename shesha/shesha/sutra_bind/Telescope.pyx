...
cdef class Telescope:
    ...
    def __cinit__(self, naga_context context, long pup_size, long num_eleme_pup, np.ndarray[ndim=2, dtype=np.float32_t] pupil,
        ...
        cdef np.ndarray[ndim= 2, dtype = np.float32_t] pupil_F = pupil.T.copy()
        cdef np.ndarray[ndim = 2, dtype = np.float32_t] phase_ab_M1_F = phase_ab_M1.T.copy()
        cdef np.ndarray[ndim = 2, dtype = np.float32_t] pupil_m_F = pupil_m.T.copy()
        cdef np.ndarray[ndim = 2, dtype = np.float32_t] phase_ab_m1_m_F = phase_ab_m1_m.T.copy()
    ...
    def set_phase_ab_M1(self, np.ndarray[ndim=2, dtype=np.float32_t] phase_ab):
        ...
        cdef np.ndarray[ndim = 2, dtype = np.float32_t] phase_ab_F = phase_ab.T.copy()
