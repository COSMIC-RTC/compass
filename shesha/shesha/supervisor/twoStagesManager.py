...
class TwoStagesManager(object):
    ...
    def __init__(
        self,
        first_stage: StageSupervisor,
        second_stage: StageSupervisor,
        frequency_ratio: int,
    ):
        """
    ...
    def next(self, *, do_control: bool = True) -> None:
        ...
        if do_control:
            ...
            self.first_stage.next(
                do_control=first_stage_control,
                apply_control=True,
        ...
        self.second_stage_input[
            self.mpup_offset : -self.mpup_offset,
            self.mpup_offset : -self.mpup_offset,
            :,
        ] = first_stage_residual[:, :, None]
        self.second_stage.tel.set_input_phase(
            self.second_stage_input
        )  # 1st stage residuals sent to seconds stage simulation.
        ...
        if number_of_iter == -1:  # Infinite loop
            while True:
                ...
                if (self.iterations + 1) % monitoring_freq == 0:
                    self.second_stage._print_strehl(
                        monitoring_freq, time.time() - t1, number_of_iter
                    )
                    t1 = time.time()
