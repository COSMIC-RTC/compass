...
class Calibration(object):
    ...
    def apply_volts_and_get_slopes(
        self,
    ...
    def do_imat_modal(
        ...
        *,
        ...
        modal_imat = np.zeros(
            (
                self._config.p_controllers[controller_index].nslope,
                modal_basis.shape[1],
            )
        )
        ...
        for m in track(range(nmodes_max)):
            ...
            if (push_pull is True) or (
                with_turbu is True
            ):  # with turbulence/aberrations => push/pull
                self._rtc.set_perturbation_voltage(
                    controller_index, "imat_modal", v_old + v
                )  # Adding Perturbation voltage on current iteration
                devpos = self.apply_volts_and_get_slopes(
                    controller_index, turbu=with_turbu, noise=noise
                )
                self._rtc.set_perturbation_voltage(controller_index, "imat_modal", v_old - v)
                devmin = self.apply_volts_and_get_slopes(
                    controller_index, turbu=with_turbu, noise=noise
                )
                modal_imat[:, m] = (devpos - devmin) / (2.0 * ampli[m])
                ...
                modal_imat[:, m] = (
                    self.apply_volts_and_get_slopes(controller_index, noise=noise) / ampli[m]
                )
    ...
    def do_imat_phase(
        ...
        *,
        ...
        self._wfs.set_ncpa_wfs(wfs_index, cube_phase[nphase, :, :] * 0.0)  # Remove the Phase on WFS
        _ = self.apply_volts_and_get_slopes(controller_index, turbu=with_turbu, noise=noise)

        return imat_phase

    def compute_modal_residuals(
        self,
        projection_matrix: np.ndarray,
        *,
        selected_actus: np.ndarray = None,
    ) -> np.ndarray:
        ...
        try:
            ...
            return [0]
        v = self._rtc.get_command(
            1
        )  # We compute here the residual phase on the DM modes. Gives the Equivalent volts to apply/
        if selected_actus is None:
            ai = projection_matrix.dot(v) * 1000.0  # np rms units
        else:  # Slaving actus case
            v2 = v[:-2][list(selected_actus)]  # If actus are slaved then we select them.
            v3 = v[-2:]
            ai = projection_matrix.dot(np.concatenate((v2, v3))) * 1000.0
        return ai
