...
class AtlasSupervisor(BenchSupervisor):
    ...
    def computePh2Modes(self):
        oldnoise = self.config.p_wfs0.noise
        self.setNoise(-1)
        ...
        for i in trange(nbmode):
            ...
            self.dms.set_full_comm((self.modalBasis[:, i]).astype(np.float32).copy())
    ...
    def returnkl2V(self):
        ...
        if (self.KL2V is None):
            ...
            KL2V = ao.compute_KL2V(self.config.p_controllers[0], self.dms,
    ...
    def computeMerged(self, nbpairs=None):
        ...
        dmposx = self.config.p_dm0._xpos - 0.5
        dmposy = self.config.p_dm0._ypos - 0.5
        dmposMat = np.c_[dmposx, dmposy].T  # one actu per column

        pitch = self.config.p_dm0._pitch
    ...
    def computeActusPetals(self):
        import shesha.util.make_pupil as mkP
        import shesha.util.utilities as util
        N = self.config.p_geom.pupdiam
        i0 = j0 = self.config.p_geom.pupdiam / 2. - 0.5
        pixscale = self.config.p_tel.diam / self.config.p_geom.pupdiam
        dspider = self.config.p_tel.t_spiders
        #spup = self.getSpupil()
        oldspidersize = self.config.p_tel.t_spiders
        self.config.p_tel.set_t_spiders = 0.
        spup = mkP.make_pupil(self.config.p_geom.pupdiam, self.config.p_geom.pupdiam,
                              self.config.p_tel, i0, j0)
        self.config.p_tel.set_t_spiders = oldspidersize
        petalsPups = mkP.compute6Segments(spup, N, pixscale, 0.51 / 2, i0, j0)
        ipup = util.pad_array(spup, self.config.p_geom.ssize).astype(np.float32)
        dmpup = np.zeros_like(ipup)
        dmposx = self.config.p_dm0._xpos
        dmposy = self.config.p_dm0._ypos
    ...
    def applyVoltGetSlopes(self, noise=False, turbu=False, reset=1):
        ...
        for w in range(len(self.wfs.d_wfs)):
            ...
            if (turbu):
                self.raytraceWfs(w, "all")
                # w.d_gs.raytrace(self.atm)
                # w.d_gs.raytrace(self.tel)
                # w.d_gs.raytrace(self.dms)
                # w.d_gs.raytrace()
            else:
                self.raytraceWfs(w, ["dm", "ncpa"], rst=reset)
                # w.d_gs.raytrace(self.dms, rst=reset)
                # w.d_gs.raytrace()

            self.compWfsImage(w, noise=noise)
    ...
    def writeConfigOnFile(self,
        ...
        for i in range(aodict["nbWfs"]):
            new_hduwfsl.append(
                    pfits.ImageHDU(self.config.p_wfss[i]._isvalid))  # Valid subap array
...
if __name__ == '__main__':
    ...
    try:
        ...
        server = PyroServer()
        server.add_device(supervisor, "ATLAS_SUPERVISOR" + user)
        server.start()
    except:
    ...
    if USEPYRO:
        ...
        srv = PyroServer(bindTo=('wsmiao2.obspm.fr',
