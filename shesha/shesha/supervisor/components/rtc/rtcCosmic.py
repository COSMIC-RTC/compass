...
class RtcCosmic:
    ...
    def __init__(self, config, wfs, dms):
        ...
        if self.config.p_hrtc.frame_shm is None:
            ...
            self.frame = ts.MudpiFrame(
                np.zeros((self.framesize, self.framesize), dtype=np.uint16),
                config.p_hrtc.wfs_payload_size,
            )
            self.com = ts.MudpiFrame(
                np.zeros(com_shape, dtype=np.float32),
                config.p_hrtc.com_payload_size,
            )
    ...
    def do_control(self):
        ...
        wfs_cropped = self._wfs.get_wfs_image(0)[
            self.crop : self.crop + self.framesize,
            self.crop : self.crop + self.framesize,
        ]
        if self.publisher is not None:
            self.frame = ts.MudpiFrame(
                wfs_cropped.astype(np.uint16),
                self.config.p_hrtc.wfs_payload_size,
            )
