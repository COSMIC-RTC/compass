...
class RtcAbstract(ABC):
    ...
    def set_perturbation_voltage(
        self, controller_index: int, name: str, command: np.ndarray
    ) -> None:
    ...
    def get_perturbation_voltage(
        self, controller_index: int, *, name: str = None
    ) -> Union[dict, tuple]:
    ...
    def set_A_matrix(self, controller_index: int, matrix_index: int, a_matrix: np.ndarray) -> None:
    ...
    def set_L_matrix(self, controller_index: int, matrix_index: int, l_matrix: np.ndarray) -> None:
    ...
    def set_iir_a_vector(
        self, controller_index: int, vector_index: int, iir_a_vector: np.ndarray
    ) -> None:
    ...
    def set_iir_b_vector(
        self, controller_index: int, vector_index: int, iir_b_vector: np.ndarray
    ) -> None:
    ...
    def do_control(
        ...
        *,
        sources: SourceCompass = None,
        source_index: int = 0,
        is_wfs_phase: bool = False,
    ) -> None:
        ...
        if self._rtc.d_control[controller_index].type == scons.ControllerType.GEO:
            if sources is not None:
                self._rtc.d_control[controller_index].comp_dphi(sources[source_index], is_wfs_phase)
    ...
    def get_image_calibrated(self, centroider_index: int) -> np.ndarray:
        ...
        if (
            self._config.p_centroiders[centroider_index].type == scons.CentroiderType.MASKEDPIX
        ):  # Full pixel case
