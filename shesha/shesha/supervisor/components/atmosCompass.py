...
class AtmosCompass(object):
    ...
    Attributes:
        ...
        _atmos : (sutra.Atmos) : Sutra atmos instance
    ...
    def __init__(self, context, config):
        ...
        self._atmos = atmos_init(
            self._context,
            self._config.p_atmos,
            self._config.p_tel,
            self._config.p_geom,
            self._config.p_loop.ittime,
            p_wfss=self._config.p_wfss,
    ...
    def set_wind(
        ...
        *,
        ...
        lin_delta = (
            self._config.p_geom.pupdiam
            / self._config.p_tel.diam
            * self._config.p_atmos.windspeed[screen_index]
            * np.cos(CONST.DEG2RAD * self._config.p_geom.zenithangle)
            * self._config.p_loop.ittime
        )
        oldx = self._config.p_atmos._deltax[screen_index]
        oldy = self._config.p_atmos._deltay[screen_index]
        self._config.p_atmos._deltax[screen_index] = lin_delta * np.sin(
            CONST.DEG2RAD * self._config.p_atmos.winddir[screen_index] + np.pi
        )
        self._config.p_atmos._deltay[screen_index] = lin_delta * np.cos(
            CONST.DEG2RAD * self._config.p_atmos.winddir[screen_index] + np.pi
        )
        ...
        if (
            oldx * self._config.p_atmos._deltax[screen_index] < 0
        ):  # Sign has changed, must change the stencil
        ...
        if (
            oldy * self._config.p_atmos._deltay[screen_index] < 0
        ):  # Sign has changed, must change the stencil
