...
def init_focal_plane_mask(p_corono: conf.ParamCoronagraph):
    ...
    if p_corono._focal_plane_mask_name == scons.FpmType.CLASSICAL_LYOT:
        classical_lyot = True
    elif p_corono._focal_plane_mask_name in (
        scons.FpmType.SPHERE_APLC_fpm_ALC1,
    ...
    if classical_lyot:
        ...
        if p_corono._fpm_sampling is None:
            p_corono.set_fpm_sampling(20.0)
        ...
        fpm = util.classical_lyot_fpm(
            p_corono._lyot_fpm_radius,
            p_corono._dim_fpm,
...
def init_mft(p_corono: conf.ParamCoronagraph, pupdiam, planes, center_on_pixel=False):
    ...
    if planes == "apod_to_fpm":
        ...
        for w, wavelength in enumerate(wav_vec):
            ...
            AA_apod_to_fpm[:, :, w], BB_apod_to_fpm[:, :, w], norm0[w] = mft_matrices(
                pupdiam, dim_fpm, nbres
            )
        ...
        for w, wavelength in enumerate(wav_vec):
            ...
            AA_fpm_to_lyot[:, :, w], BB_fpm_to_lyot[:, :, w], norm0[w] = mft_matrices(
                dim_fpm, pupdiam, nbres, inverse=True
            )
        ...
        for w, wavelength in enumerate(wav_vec):
            ...
            if center_on_pixel:
                ...
                (
                    ...
                    BB_lyot_to_image[:, :, w],
                    norm0[w],
                ) = mft_matrices(pupdiam, dim_image, nbres)
...
def mft_matrices(
    ...
    if np.isscalar(real_dim_input):
        ...
        if all(isinstance(dims, (int, np.int32)) for dims in real_dim_input) & (
            len(real_dim_input) == 2
        ):
    ...
    if np.isscalar(nbres):
        ...
        if all(isinstance(nbresi, (float, np.float32, int, np.int32)) for nbresi in nbres) & (
            len(nbres) == 2
        ):
    ...
    if not inverse:
        if norm == "backward":
            ...
            norm0 = np.sqrt(
                nbresx * nbresy / dim_input_x / dim_input_y / dim_output_x / dim_output_y
            )
        sign_exponential = -1
