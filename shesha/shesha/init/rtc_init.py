        ...
        if p_wfss is not None and p_dms is not None:
            for i in range(ncontrol):
                if "dm" not in dataBase:
                    imat = imats.imat_geom(wfs, dms, p_wfss, p_dms, p_controllers[i], meth=0)
                else:
                    imat = None
            ...
            if roket_flag:
                ...
                rtc.add_controller(
                    ...
                    p_controller.nslope,
                    p_controller.nactu,
                    p_controller.nslope_buffer,
                    p_controller.nstates,
                    p_controller.nstate_buffer,
                    nmodes,
                    p_controller.n_iir_in,
                    p_controller.n_iir_out,
                    p_controller.polc,
                    p_controller.modal,
                    dms,
                    p_controller.ndm,
                    p_controller.ndm.size,
                    p_controller.nwfs,
                    p_controller.nwfs.size,
                    Nphi,
                    True,
                )
                init_controller_geo(ncontrol, rtc, dms, p_geom, p_controller, p_dms, roket=True)
...
def init_centroider(
    ...
    nwfs: int,
    p_wfs: conf.ParamWfs,
    p_centroider: conf.ParamCentroider,
    p_tel: conf.ParamTel,
    p_atmos: conf.ParamAtmos,
    wfs: Sensors,
    rtc: Rtc,
    ...
    if p_wfs.type == scons.WFSType.SH:
        ...
        s_offset = 0.0
        s_scale = (p_wfs.Lambda * 1e-6 / p_tel.diam) * p_wfs.pyr_ampl * CONST.RAD2ARCSEC
    ...
    rtc.add_centroider(
        ...
        s_offset,
        s_scale,
        p_centroider.filter_TT,
        context.active_device,
        p_centroider.type,
        wfs.d_wfs[nwfs],
    )
    rtc.d_centro[-1].load_validpos(
        p_wfs._validsubsx, p_wfs._validsubsy, p_wfs._nvalid * p_wfs.nPupils
    )
    ...
    if p_centroider.type == scons.CentroiderType.PYR:
        ...
        if (
            p_centroider.type == scons.CentroiderType.TCOG
            or p_centroider.type == scons.CentroiderType.WCOG
        ):
        ...
        if (
            ...
            or p_centroider.type == scons.CentroiderType.CORR
        ):
            r0 = p_atmos.r0 * (p_wfs.Lambda / 0.5) ** (6 / 5.0)
            seeing = CONST.RAD2ARCSEC * (p_wfs.Lambda * 1.0e-6) / r0
            ...
            if p_centroider.type == scons.CentroiderType.WCOG:
                ...
                rtc.d_centro[nwfs].load_weights(p_centroider.weights, p_centroider.weights.ndim)
            else:
                ...
                rtc.d_centro[nwfs].init_corr(
                    p_centroider.sizex,
                    p_centroider.sizey,
                    p_centroider.interpmat,
                )
                rtc.d_centro[nwfs].load_corr(
                    p_centroider.weights, corrnorm, p_centroider.weights.ndim
                )
...
def comp_weights(p_centroider: conf.ParamCentroider, p_wfs: conf.ParamWfs, npix: int):
    ...
    if p_centroider.type_fct == scons.CentroiderFctType.MODEL:
        if p_wfs.gsalt > 0:
            tmp = p_wfs._lgskern
            tmp2 = utilities.makegaussian(tmp.shape[1], npix * p_wfs._nrebin).astype(np.float32)
            tmp3 = np.zeros((tmp.shape[1], tmp.shape[1], p_wfs._nvalid), dtype=np.float32)
            ...
            tmp = np.zeros(
                (j - offset + 1, j - offset + 1, tmp3.shape[2]),
                dtype=np.float32,
            )
    ...
    if p_centroider.type_fct == scons.CentroiderFctType.GAUSS:
        ...
        if p_wfs.npix % 2 == 1:
            p_centroider.weights = utilities.makegaussian(
                p_wfs.npix, p_centroider.width, p_wfs.npix // 2, p_wfs.npix // 2
            ).astype(np.float32)
...
def init_controller(
    ...
    tel: Telescope,
    atmos: Atmos,
    p_centroiders: List[conf.ParamCentroider],
    do_refslp=False,
    dataBase={},
    use_DB=False,
):
    ...
    if p_controller.type == scons.ControllerType.GEO:
        ...
        try:
            p_controller._imat = imats.imat_geom(wfs, dms, p_wfss, p_dms, p_controller, meth=0)
...
def init_controller_geo(
    ...
    unitpervolt = np.array([p_dms[j].unitpervolt for j in range(len(p_dms))], dtype=np.float32)

    rtc.d_control[i].init_proj_sparse(dms, indx_dm, unitpervolt, indx_pup, indx_mpup, roket=roket)
...
def init_controller_ls(
    ...
    if p_controller.do_kl_imat:
        ...
        print(
            "Filtering ",
            p_controller.nModesFilt,
            " modes based on mode ordering",
        )
        M2V = M2V[
            :,
            list(range(M2V.shape[1] - 2 - p_controller.nModesFilt)) + [-2, -1],
        ]
    ...
    if p_controller.modopti:
        ...
        rtc.d_control[i].init_modalOpti(
            p_controller.nmodes,
            p_controller.nrec,
            M2V,
            p_controller.gmin,
        ...
        ol_slopes = modopti.open_loopSlp(tel, atmos, wfs, rtc, p_controller.nrec, i, p_wfss)
        ...
        mgain = np.ones(
            sum([p_dms[j]._ntotact for j in range(len(p_dms))]),
            dtype=np.float32,
        )
        cc = 0
        for ndm in p_dms:
...
def init_controller_generic(i: int, p_controller: conf.ParamController, p_dms: list, rtc: Rtc):
