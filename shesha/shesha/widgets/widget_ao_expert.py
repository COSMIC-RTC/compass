...
class WidgetAOExpert(ExpertClassTemplate):
    def __init__(self) -> None:
        ...
        self.uiExpert.wao_dmActuPushArcSecNumWFS.currentIndexChanged.connect(self.updateDMrange)
    ...
    def updateAmpliCompGUI(self) -> None:
        diffract = (
            self.sim.config.p_wfss[0].Lambda * 1e-6 / self.sim.config.p_tel.diam * CONST.RAD2ARCSEC
        )
    ...
    def updateAmpliComp(self) -> None:
        ...
        self.uiExpert.wao_pyr_ampl_arcsec.setValue(self.sim.config.p_wfss[0].pyr_ampl * diffract)
    ...
    def updateDMrange(self, push4imat: float = None, unitpervolt: float = None) -> None:
        ...
        if (numdm != "") and (numwfs != "") and (push4imat != 0) and (unitpervolt != 0):
    ...
    def updateDmPanel(self) -> None:
        ...
        self.uiExpert.wao_dmTypeSelector.setCurrentIndex(
            self.uiExpert.wao_dmTypeSelector.findText(str(self.sim.config.p_dms[ndm].type))
        )
    ...
    def updateWfsPanel(self) -> None:
        ...
        if (
            self.sim.config.p_wfss[nwfs].type == "pyrhr"
            or self.sim.config.p_wfss[nwfs].type == "pyr"
        ):
    ...
    def updateAtmosPanel(self) -> None:
        ...
        width = (self.sim.config.p_atmos.alt.max() / 20.0 + 0.1) / 1000.0
        self.uiExpert.wao_atmosWindow.canvas.axes.barh(
            self.sim.config.p_atmos.alt / 1000.0 - width / 2.0,
    ...
    def updateRtcPanel(self) -> None:
        ...
        if type == scons.CentroiderType.BPCOG:
            self.uiExpert.wao_centroNbrightest.setValue(self.sim.config.p_centroiders[ncentro].nmax)
        ...
        if type in [scons.CentroiderType.CORR, scons.CentroiderType.WCOG]:
            ...
            self.uiExpert.wao_centroWidth.setValue(self.sim.config.p_centroiders[ncentro].width)

        # Controller panel
        type_contro = self.sim.config.p_controllers[0].type
        if type_contro == scons.ControllerType.LS and self.sim.config.p_controllers[0].modopti == 0:
        ...
        elif type_contro == scons.ControllerType.LS and self.sim.config.p_controllers[0].modopti:
    ...
    def setRtcParams(self) -> None:
        ...
        self.sim.config.p_centroiders[ncentro].set_thresh(self.uiExpert.wao_centroThresh.value())
        self.sim.config.p_centroiders[ncentro].set_nmax(self.uiExpert.wao_centroNbrightest.value())
        self.sim.config.p_centroiders[ncentro].set_thresh(self.uiExpert.wao_centroThresh.value())
    ...
    def setDmParams(self) -> None:
        ...
        if ndm < 0:
            ndm = 0
        self.sim.config.p_dms[ndm].set_type(str(self.uiExpert.wao_dmTypeSelector.currentText()))
    ...
    def BttCommand(self) -> None:
        if self.sim.rtc:
            ...
            ao.command_on_Btt(
                self.sim.rtc,
                self.sim.dms,
                self.sim.config.p_dms,
                self.sim.config.p_geom,
                nfilt,
            )
    ...
    def KLCommand(self) -> None:
        if self.sim.rtc:
            ...
            cmat = ao.command_on_KL(
                self.sim.rtc,
                self.sim.dms,
                self.sim.config.p_controllers[0],
                self.sim.config.p_dms,
                self.sim.config.p_geom,
    ...
    def updatePlotWfs(self) -> None:
        ...
        if (
            (
                self.sim.config.p_wfss[n].type == scons.WFSType.PYRHR
                or self.sim.config.p_wfss[n].type == scons.WFSType.PYRLR
            )
            and typeText == "Pyramid mod. pts"
            and self.sim.is_init
        ):
            scale_fact = (
                2
                * np.pi
                / self.sim.config.p_wfss[n]._Nfft
                * self.sim.config.p_wfss[n].Lambda
                * 1e-6
                / self.sim.config.p_tel.diam
                / self.sim.config.p_wfss[n]._qpixsize
                * CONST.RAD2ARCSEC
            )
    ...
    def displayRtcMatrix(self) -> None:
        ...
        if self.sim.rtc:
            ...
            if (
                type_matrix == "imat"
                and self.sim.config.p_controllers[0].type != scons.ControllerType.GENERIC
                and self.sim.config.p_controllers[0].type != scons.ControllerType.GEO
            ):
                ...
                if (
                    self.sim.config.p_controllers[0].type == scons.ControllerType.LS
                    or self.sim.config.p_controllers[0].type == scons.ControllerType.MV
                ):
                    data = self.sim.rtc.getEigenvals(0)
            elif (
                type_matrix == "Cmm"
                and self.sim.config.p_controllers[0].type == scons.ControllerType.MV
            ):
            ...
            elif type_matrix == "Cmm inverse" and self.sim.config.p_controllers[0].type == "mv":
                data = self.sim.rtc.get_cmm(0)
            elif type_matrix == "Cmm eigen" and self.sim.config.p_controllers[0].type == "mv":
                data = self.sim.rtc.getCmmEigenvals(0)
            elif type_matrix == "Cphim" and self.sim.config.p_controllers[0].type == "mv":
    ...
    def computeDMrange(
        ...
        if self.sim.config.p_dms[i].type is not scons.DmType.TT:
            ...
            dist = self.sim.config.p_tel.diam / self.sim.config.p_wfss[numwfs].nxsub
