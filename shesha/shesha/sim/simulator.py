...
class Simulator:
    ...
    def __init__(self, filepath: str = None, use_DB: bool = False) -> None:
        ...
        self.context = None  # type: carmaWrap_context
    ...
    def clear_init(self) -> None:
        ...
        if self.loaded and self.is_init:
            ...
            # del self.context  # What is this supposed to do ... ?
            # self.context = None
    ...
    def next(self, *, move_atmos: bool = True, see_atmos: bool = True, nControl: int = 0,
        ...
        if (
            ...
            if tar_trace is not None:
                for t in tar_trace:
                    if see_atmos:
                        self.raytrace_tar(t, ["atmos", "tel"])
                    else:
                        self.raytrace_tar(t, "tel")

                    if self.rtc is not None:
                        self.do_control(nControl)
                        self.raytrace_tar(t, ["dm", "ncpa"], rst=False)
                        self.apply_control(nControl)
        else:
            if tar_trace is not None:
                for t in tar_trace:
                    if see_atmos:
                        self.raytrace_tar(t, "all")
                    else:
                        self.raytrace_tar(t, ["tel", "dm", "ncpa"])
            if wfs_trace is not None:
                for w in wfs_trace:
                    if see_atmos:
                        self.raytrace_wfs(w, ["atmos", "tel", "ncpa"])
                    else:
                        self.raytrace_wfs(w, ["tel", "ncpa"])

                    if not self.config.p_wfss[w].open_loop and self.dms is not None:
                        self.raytrace_wfs(w, "dm", rst=False)
                    self.compute_wfs_image(w)
                ...
                for ncontrol in range(len(self.rtc.d_control)):
                    if self.rtc.d_control[ncontrol].type != scons.ControllerType.GEO:
                        if self.rtc.d_control[
                            ...
                            self.do_calibrate_img(ncontrol)
                        self.do_centroids(ncontrol)
                        self.do_control(ncontrol)
                        self.do_clipping(ncontrol)
        ...
        if compute_tar_psf:
            for tar_index in tar_trace:
                self.comp_tar_image(tar_index)
                self.comp_strehl(tar_index)

        self.iter += 1

    def print_strehl(self, monitoring_freq: int, t1: float, nCur: int = 0, nTot: int = 0,
                     tar_index: int = 0):
        """ Print the Strehl ratio on the terminal
        """
        framerate = monitoring_freq / t1
        strehl = self.get_strehl(tar_index)
...
#
    ...
    def raytrace_tar(self, tarNum, layers: list, rst: bool = True):
    ...
    def raytrace_wfs(self, wfsNum, layers: list, rst: bool = True):
        """
    ...
    def compute_wfs_image(self, wfsNum: int = 0, noise: bool = True):
    ...
    def comp_tar_image(self, tarNum: int = 0, puponly: int = 0, compLE: bool = True):
        """ Computes the PSF
    ...
    def comp_strehl(self, tarNum: int = 0, do_fit: bool = True):
    ...
    def do_control(self, nControl: int, n: int = 0, wfs_direction: bool = False):
    ...
    def do_calibrate_img(self, nControl: int):
    ...
    def do_centroids(self, nControl: int):
    ...
    def do_centroids_geom(self, nControl: int):
    ...
    def apply_control(self, nControl: int, compVoltage: bool = True):
    ...
    def do_clipping(self, nControl: int):
