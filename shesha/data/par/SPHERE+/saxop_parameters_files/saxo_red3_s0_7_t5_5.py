...
# import atmosphere
import sys
sys.path.append(os.path.abspath("saxoplus-reference-cases/scripts"))
atmosphere = __import__("atmosphere")
# from saxoPlusUtils import saxo_compute_delay
# import dictionary_handler as simu       # for dictionary of simulations

# ---- Directories around this script with the DM data are needed
# _my_cases_dir, _my_param_name = os.path.split(os.path.abspath(__file__))
...
freq = 50. # create your own pair here.
saxo_frequency_delay_pair = [freq, 0.042] # your custom pair

# loop
p_loop = conf.ParamLoop()
p_loop.set_niter(6376)                               # number of loops [?]
p_loop.set_ittime(0.0008)   # first stage frequency [?]
...
# FIXME: remove r0 (set in run_script)? [?]
# r0 = (atm.r0[atm.profile_conditions.index(profileCondition)]).value # /?\ take ESO r0 or..
#r0 = 0.14                                                          # /?\ set your r0 value
r0 = 0.157
n_layers = len(atm.height.value)
p_atmos.set_nscreens(n_layers)                         # /!\ set Nb of layers
...
# coherenceTime = 5.5
# windSpeed = np.copy(p_atmos.windspeed) # [m/s]
# equivSpeed = np.sum(p_atmos.frac*(np.abs(windSpeed))**(5./3))**(3./5)
# tau0Seeing = 0.31 * r0 / equivSpeed 
# speedFactor = coherenceTime * 1e-3 / tau0Seeing
# p_atmos.set_windspeed(atm.wind_velocity.value/speedFactor)   
...
p_wfs0.set_pixsize(0.44)      # /!\ Shannon at 700nm. No exact reference found
p_wfs0.set_fracsub(0.5)       # /!\ Select 1240 subapertures.
p_wfs0.set_xpos(0.)           # /!\ On axis
p_wfs0.set_ypos(0.)           # /!\ On axis
p_wfs0.set_Lambda(0.69)      # /!\ SAXO SH bandwidth : [475, 900] nm
...
p_wfs0.set_gsmag(18.18522)          # Not used
...
p_wfs0.set_noise(0.02)         # EMCCD with < 0.1e- RON
...
p_wfs0.set_fssize(0.89)     # 1.25*lambda/dSubap, called MEDIUM in SAXO
#p_wfs0.set_fssize(1.0674)     # 1.5*lambda/dSubap, called LARGE in SAXO
...
dm_data = '/data/pfontanillas/workspace/compass/sl_and_rl_for_exao/saxoplus-reference-cases/data/'
p_dm0.set_file_influ_fits(dm_data + 'HODM_gauss_fitSPARTA.fits') # /!\ to use a custom SAXO HO DM
...
p_controller0.set_delay(1.05) # /!\ see at the head of the file. [?]
p_controller0.set_gain(0.05)
