...
def depouillePerf(filename, version=None, mode="profile"):
    ...
    try:
        ...
        print(
                "No results for git version : " + version + ", taking " +
                list(store.keys())[-1] + " version")
    ...
    times = [
            ...
            "sensor_trace_atmos", "sensor_trace_dm", "comp_img", "docentroids",
            "docontrol", "applycontrol"]

    plt.figure()

    colors = [
            "blue", "green", "red", "yellow", "orange", "cyan", "purple",
            "magenta", "darkcyan"]
    ...
    for indx in df.index:
        ...
        if (mode == "full"):
            ...
            plt.barh(
                    cc,
                    df.loc[indx, times[0]] / tottime * 100,
                    width,
                    color=colors[ccc])
            ...
            for i in times[1:]:
                plt.barh(
                        cc,
                        df.loc[indx, i] / tottime * 100,
                        width,
                        color=colors[ccc],
                        left=timeb)
                if (df.loc[indx, i] / tottime * 100 > 10.):
                    plt.text(
                            timeb + df.loc[indx, i] / tottime * 100 / 2,
                            cc + width / 2.,
                            '%d' % int(df.loc[indx, i] / tottime * 100) + " %")
                timeb += df.loc[indx, i] / tottime * 100
                ccc += 1
        elif (mode == "framerate"):
            plt.barh(
                    cc,
                    1. / df.loc[indx, "iter_time"] * 1000.,
                    width,
                    color=colors[ind % len(colors)])
            plt.text(
                    1. / df.loc[indx, "iter_time"] * 1000. / 2.,
                    cc + width / 2.,
                    '%.1f' % float(1. / df.loc[indx, "iter_time"] * 1000.))
            ccc += 1
...
doc.preamble.append(
        Command(
                'title', 'COMPASS Performance Report on %s' % str(
                        df["device"].values[0])))
...
with doc.create(Section('Environment')):
    ...
    with doc.create(Tabular('|l|c|')) as table:
        ...
        table.add_row((
                "Processors", str(
                        str(df["ncpu"].values[0]) + "x " +
                        df["processor"].values[0])))
        table.add_hline()
        table.add_row((
                "GPU devices", str(
                        str(df["ndevices"].values[0]) + "x " +
                        str(df["device"].values[0]))))
        table.add_hline()
    ...
    for indx in df.index:
        with doc.create(Subsection(df.loc[indx, "simulname"])):
            with doc.create(Tabular('|l|c|')) as table:
                ...
                table.add_hline()
                table.add_row(
                        ("Number of pixels/subap", str(df.loc[indx, "npix"])))
                table.add_hline()
                table.add_row(
