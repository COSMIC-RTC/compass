...
def depouillePerf(filename,version=None,mode="profile"):
    ...
    for indx in df.index:
        ...
        if(mode=="full"):
            ...
            ccc+=1
            for i in times[1:]:
                plt.barh(cc,df.loc[indx,i],width,color=colors[ccc],left=timeb)
                timeb+=df.loc[indx,i]
                ccc+=1
        elif(mode == "profile"):
            tottime = 0
            ...
            plt.barh(cc,df.loc[indx,times[0]]/tottime*100,width,color=colors[ccc])
            timeb=df.loc[indx,times[0]]/tottime*100
            ccc+=1
            for i in times[1:]:
                plt.barh(cc,df.loc[indx,i]/tottime*100,width,color=colors[ccc],left=timeb)
                if(df.loc[indx,i]/tottime*100 > 10.):
                    plt.text(timeb+df.loc[indx,i]/tottime*100/2,cc+width/2.,'%d' % int(df.loc[indx,i]/tottime*100) + " %")
                timeb+=df.loc[indx,i]/tottime*100
                ccc+=1
        elif(mode == "framerate"):
            plt.barh(cc,1./df.loc[indx,"iter_time"]*1000.,width,color=colors[ind])
            plt.text(1./df.loc[indx,"iter_time"]*1000./2.,cc+width/2.,'%.1f' % float(1./df.loc[indx,"iter_time"]*1000.))
            ccc+=1
...
doc.preamble.append(Command('title', 'COMPASS Performance Report on %s'%str(df["device"].values[0])))
...
with doc.create(Section('Environment')):
    doc.append('Simulations have been performed within the following environment:\n ')
    with doc.create(Tabular('|l|c|')) as table:
        table.add_hline()
        table.add_row(("Platform",str(df["platform"].values[0])))
        table.add_hline()
        table.add_row(("Processors", str(str(df["ncpu"].values[0])+"x "+df["processor"].values[0])))
        table.add_hline()
        table.add_row(("GPU devices", str(str(df["ndevices"].values[0])+"x "+df["device"].values[0])))
        table.add_hline()
...
with doc.create(Section('Simulation parameters')):
    ...
    for indx in df.index:
        with doc.create(Subsection(df.loc[indx,"simulname"])):
            with doc.create(Tabular('|l|c|')) as table:
                table.add_hline()
                table.add_row(("Simulation case",str(df.loc[indx,"simulname"])))
                table.add_hline()
                table.add_row(("Telescope diameter [m]",str(df.loc[indx,"tel.diam"])))
                table.add_hline()
                table.add_row(("WFS type",str(df.loc[indx,"sensor_type"])))
                table.add_hline()
                table.add_row(("Number of subaps",str(df.loc[indx,"nxsub"])))
                table.add_hline()
                table.add_row(("Number of pixels/subap",str(df.loc[indx,"npix"])))
                table.add_hline()
                table.add_row(("Controller type",str(df.loc[indx,"controller"])))
                table.add_hline()
                table.add_row(("Centroider type",str(df.loc[indx,"centroider"])))
                table.add_hline()
