...
cdef class Dms:
    ...
    def add_dm(self, bytes type_dm, float alt, long dim, long ninflu, long influsize, long ninflupos, long npts, float push4imat, long nord=0, int device=-1):
    ...
    def load_kl(self, float alt,
        ...
        cdef np.ndarray[ndim = 2, dtype = np.float32_t] cp_azbas = azbas.copy()
        cdef np.ndarray[ndim = 1, dtype = np.int32_t] cp_ords = ords.copy()
