...
def gamora_init( bytes type, int nactus, int niter,
    ...
    """
    if Btt is None:
        return Gamora(type,nactus, niter, IFvalue, IFrowind, IFcolind, TT, spupil, scale)
    else:
        return Gamora(type,nactus, niter, IFvalue, IFrowind, IFcolind, TT, spupil, scale, nmodes, Btt, covmodes)
