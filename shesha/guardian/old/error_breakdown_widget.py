...
class html_display:
    ...
    def __init__(self):
        ...
        self.plus_select = CheckboxButtonGroup(labels=self.coms_list + ["fitting"],
                                               active=[0, 1, 2, 4, 5, 6, 8])
        ...
        self.basis_select3 = Select(title="Basis", value=self.basis[0],
                                    options=self.basis)
        self.modes_select = Slider(title="Mode #", value=0, start=0,
                                   end=self.modes.shape[1], step=1)
        ...
        self.colors = {
                "H_com": "green", "bp_com": "orange", "com": "black", "noise_com": "red",
                "tomo_com": "purple", "trunc_com": "cyan", "wf_com": "magenta",
                "alias_wfs_com": "blue"
        }
        ...
        self.tab1 = Panel(child=HBox(self.inputs, VBox(self.p, self.psum), self.plog),
                          title="Commands")
        self.tab2 = Panel(child=HBox(
                VBox(
                        HBox(self.inputs2, self.p2, self.p3), self.cov_table,
                        self.cor_table)), title="Covmats")
...
#            data2 = np.dot(self.P,data2)
    ...
    def cov_cor(self):
        cov = np.zeros((6, 6))
        bufdict = {
                "0": self.f["noise_com"][:], "1": self.f["trunc_com"][:],
                "2": self.f["alias_wfs_com"][:], "3": self.f["H_com"][:],
                "4": self.f["bp_com"][:], "5": self.f["tomo_com"][:]
        }
