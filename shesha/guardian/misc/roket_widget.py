...
class html_display:
    ...
    def __init__(self):
        ...
        self.pcov_source = ColumnDataSource(data=dict(image=[], x=[], y=[], dw=[],
                                                      dh=[]))
        self.pcor_source = ColumnDataSource(data=dict(image=[], x=[], y=[], dw=[],
                                                      dh=[]))
        ...
        self.psf_display_select = Select(title="PSF display", value="COMPASS", options=[
        ...
        self.inputs3 = HBox(
                VBox(self.DB_select, self.DB_button, self.basis_select3,
                     VBox(
                             VBox(
                                     HBox(self.modes_select,
        ...
        self.tab1 = Panel(child=HBox(self.inputs, VBox(self.plog, self.psum)),
                          title="Breakdown")
        self.tab2 = Panel(child=HBox(
                VBox(
                        HBox(self.inputs2, self.p2, self.p3),
                        HBox(self.cov_table, self.pcov), HBox(
                                self.cor_table, self.pcor))), title="Cov/cor")
        self.tab3 = Panel(child=HBox(self.inputs3, self.pmodes), title="Basis")
        self.tab4 = Panel(child=HBox(self.inputs4, VBox(self.ppsf, self.pcutpsf)),
                          title="PSF")
        self.tab5 = Panel(
    ...
    def update_psf(self):
        ...
        if (image is not None):
            ...
            self.ppsf.image_url(url=dict(value=self.url + time + ".png"), x=0, y=0,
                                w=image.shape[0], h=image.shape[0])
    ...
    def rescale_matrix(self):
        ...
        self.p2.image_url(url=dict(value=self.url + time + ".png"), x=0, y=0,
    ...
    def update_mode(self):
        ...
        self.pmodes.image_url(url=dict(value=self.url + time + ".png"), x=0, y=0,
                              w=self.pup.shape[0], h=self.pup.shape[0])
    ...
    def plot_sum(self):
        ...
        for i in plus:
            ...
            if (self.plus_select.labels[i] != "CORRECT"):
                ...
                r0 = self.f.attrs["r0"] * (self.f.attrs["target.Lambda"][0] / self.f.
                                           attrs["wfs.Lambda"][0])**(6. / 5.)
                RASC = 180 / np.pi * 3600.
                Dtomo = 0
                Dbp = 0
                Dcov = 0
...
#            data2 = np.dot(self.P,data2)
        ...
        if (list(self.f.keys()).count("SR2")):
            self.source3.data = dict(x=[x, x, x, x], y=[
                    data,
            ...
            if (list(self.f.keys()).count("SR")):
                self.source3.data = dict(x=[x, x, x], y=[
                        data, np.ones(len(x)) * self.f["SR"].value, data2
    ...
    def cov_cor(self):
        cov = np.zeros((6, 6))
        bufdict = {
                "0": self.f["noise"][:], "1": self.f["non linearity"][:],
                "2": self.f["aliasing"][:], "3": self.f["filtered modes"][:],
                "4": self.f["bandwidth"][:], "5": self.f["tomography"][:]
        }
