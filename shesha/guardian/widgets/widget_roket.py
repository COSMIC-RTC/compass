...
class Bokeh_roket:
    ...
    def __init__(self):
        ...
        self.source_breakdown = ColumnDataSource(data=dict(n=[], a=[], b=[], t=[], nl=[],
                                                           f=[], fm=[]))
        self.source_cov = ColumnDataSource(data=dict(Type=[], Noise=[], Trunc=[],
                                                     Aliasing=[], FilteredModes=[],
                                                     Bandwidth=[], Tomography=[]))
        self.source_cor = ColumnDataSource(data=dict(Type=[], Noise=[], Trunc=[],
                                                     Aliasing=[], FilteredModes=[],
                                                     Bandwidth=[], Tomography=[]))
    ...
    def update_covcor(self):
        ...
        self.source_cov.data = dict(Type=[
                "Noise", "Trunc", "Alias.", "Filt.", "Band.", "Tomo"
        ], Noise=["%.2E" % v for v in self.cov[:, 0]], Trunc=[
                "%.2E" % v for v in self.cov[:, 1]
        ...
        self.source_cor.data = dict(Type=[
                "Noise", "Trunc", "Alias.", "Filt.", "Band.", "Tomo"
        ], Noise=["%.2f" % v for v in self.cor[:, 0]], Trunc=[
                "%.2f" % v for v in self.cor[:, 1]
    ...
    def update(self):
        ...
        for c in self.contributors:
            ...
            self.source_variances[c].data = dict(x=np.arange(len(data)).tolist(),
                                                 y=np.var(data, axis=1).tolist())
