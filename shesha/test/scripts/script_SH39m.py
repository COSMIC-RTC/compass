...
def initSimu(config,c):
    ...
    print "->rtc"
    #   rtc
    rtc = ao.rtc_init(tel, wfs, config.p_wfss, dms, config.p_dms,config.p_geom, config.p_rtc, config.p_atmos, atm, config.p_tel, config.p_loop, do_refslp=False, clean=clean, simul_name=simul_name, load=matricesToLoad)
...
#fig.show()
Nsimutot = len(gainslist) * len(magnitudes) * len(bps) * len(RONS)* len(pixsizes) * len(npixs) * len(freqs)
NCurrSim = 0
for freq in freqs:
    ...
    for npix in npixs:
        ...
        for pixsize in pixsizes:
            pxsize = pixsize*config.p_wfs0.Lambda/(config.p_tel.diam/config.p_wfs0.nxsub)*0.206265
