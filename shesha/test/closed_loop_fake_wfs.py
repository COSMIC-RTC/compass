...
def create_P(bin_factor, size):
    return np.repeat(np.identity(size/bin_factor, dtype=np.float32),bin_factor,axis=0)
...
def calib_pyr(centers,wfs_numbers,bin_factor=1,crop_factor=0 ):
    ...
    for i in range(4):
        ...
        center_compass = [((np.max(subx)-np.min(subx))/2.)+np.min(subx),((np.max(suby)-np.min(suby))/2.)+np.min(suby)]
        # calcul des offsets        
        offset[:,i] = [np.int32((centers[j[i]][0]-crop_factor/2.)/bin_factor)-center_compass[0],np.int32((centers[j[i]][1]-crop_factor/2.)/bin_factor)-center_compass[1]]
...
def get_slope_pyrhr(npup, valid_pixel):
    ...
    slope = np.append(gx,gy)*((config.p_wfss[0].pyr_ampl*config.p_wfss[0].Lambda*1e-6)/config.p_tel.diam)*(180/np.pi)*3600
...
def crop_im(im,taille_sortie):
    ...
    size = im.shape[0]
    im_crop = im[np.int32((size/2.)-(taille_sortie/2.)):np.int32((size/2.)+(taille_sortie/2)),np.int32((size/2.)-(taille_sortie/2.)):np.int32((size/2.)+(taille_sortie/2.))] 
                    ...
                    if (config.p_wfss[w].type_wfs == 'pyrhr' ): # verif type_wfs = pyrhr
                        if (bin_factor[fake_it] >1): # verif bining
                            if (cube_im == []): # verif bincube not here
                                ...
                                pyr_im_crop = crop_im(pyr_im,pyr_im.shape[0]-crop_factor[w])# crop image
                                
                            else:
                                pyr_im_crop = crop_im(cube_im[i],cube_im[i].shape[0]-2).astype(np.float32)# crop image

                            d_imhr = ch.naga_obj_Float2D(ch.naga_context(), data=pyr_im_crop/(bin_factor[fake_it]**2)) # inject pyr_image in GPU 
                            d_imlr = d_P[fake_it].gemm(d_imhr, 't', 'n').gemm(d_P[fake_it]) # bining GPU
                        else:
                            if (cube_im == []):
                                pyr_im = pyr_aquisition(i)# aquistion image
                                d_imlr = ch.naga_obj_Float2D(ch.naga_context(), data=pyr_im)# inject pyr_image in GPU 
                            else:
                                d_imlr = ch.naga_obj_Float2D(ch.naga_context(), data=cube_im[i])# inject pyr_image in GPU 
                        # valable seulmement pour wf0 :       
                        wfs.copy_pyrimg(w, d_imlr, d_valid_pix[fake_it][0], d_valid_pix[fake_it][1]) # envoie de l image pyramide
...
for f in range(sum(bool_fake_wfs)):
    ...
    if (config.p_wfss[fake_pos[f]].type_wfs == 'pyrhr' ):
        offset[:,:,f] = calib_pyr(centers_fake_wfs[f],fake_pos[f],bin_factor=bin_factor[f],crop_factor = crop_factor[f]) # calcul offset for wfs
        d_P.append(ch.naga_obj_Float2D(ch.naga_context(), data=create_P(bin_factor[f], size_c[f]))) # add wfs offset on GPU
    else:
        d_P.append([])
...
if (bool_fake_wfs[w]==1):# verif fake_wfs
    if (config.p_wfss[w].type_wfs == 'pyrhr' ):# verif fake_pyrhr
        ...
        d_P.append(ch.naga_obj_Float2D(ch.naga_context(), data=create_P(bin_factor[w], size_c[w])))
        valid_pix[0,:] = np.int32(config.p_wfss[w]._validsubsx+offset[0,:,w].repeat(config.p_wfss[w]._nvalid)) # cacul new X  new validsubx
        valid_pix[1,:] = np.int32(config.p_wfss[w]._validsubsy+offset[1,:,w].repeat(config.p_wfss[w]._nvalid)) # cacul new Y  new validsuby
        
        #d_valid_pix =  ch.naga_obj_Float2D(ch.naga_context(), data=valid_pix)
        d_valid_pix.append([ch.naga_obj_Int1D(ch.naga_context(), data=valid_pix[0,:]),ch.naga_obj_Int1D(ch.naga_context(), data=valid_pix[1,:])]) # add valid subpix coord in GPU
        loop(100,d_valid_pix,d_P,offset=offset,bool_fake_wfs=bool_fake_wfs,cube_im=pyr_im_cube,bin_factor=bin_factor,crop_factor=crop_factor) # Run loop 
