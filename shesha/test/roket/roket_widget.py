...
class html_display:
    ...
    def __init__(self):
        ...
        self.coms_list = ["noise", "aliasing","tomography","filtered modes","bandwidth","non linearity"]
...
#        self.modes = self.modes[:,self.swap]
    ...
    def update_psf(self):
        ...
        if(image is not None):
            ...
            self.ppsf.image_url(url=dict(value=self.url+time+".png"), x=0, y=0, w=image.shape[0], h=image.shape[0])
...
#            data2 = np.dot(self.P,data2)
    ...
    def createDataTables(self):
        ...
        tmp = [TableColumn(field="Parameter",title="Parameter"),TableColumn(field="Value",title="Value")]
