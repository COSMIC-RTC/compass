...
def macdo(x):
    ...
    Ga = [0, 12.067619015983075, 5.17183672113560444,
...
def update(attrs,old,new):
    ...
    if(yname == b"Covar"):
        ...
        for k in range(rho[ind].shape[0]):
            Drho[k] = dphi_lowpass(rho[ind][k],0.2,L0,tabx,taby) * (1/r0)**(5./3.)
        #Drho = 6.88 * (rho[ind]/r0)**(5./3.)
        for k in range(Dt.shape[0]):
            Dt[k] = dphi_lowpass(Htheta,0.2,L0,tabx,taby) * (1/r0)**(5./3.)# * xmap["Gain"][ind][k]**2
    ...
    source.data = dict(x=x[ind],y=ydata[ind],speed=xmap["Windspeed"][ind],theta=xmap["Winddir"][ind],gain=xmap["Gain"][ind])
    source_model.data = dict(x=x[ind],y=y_model,speed=xmap["Windspeed"][ind],theta=xmap["Winddir"][ind],gain=xmap["Gain"][ind])
...
yaxis_select = Select(title="Y-axis",value="Covar",options=["Covar","Var(t+bp)","Var(t)","Var(bp)","Var(t)+Var(bp)"])

speed_select = Select(title="Windspeeds",value="All",options=["All"]+[str(s) for s in np.unique(speeds)])
dir_select = Select(title="Winddirs",value="All",options=["All"]+[str(s) for s in np.unique(theta)])
gain_select = Select(title="Gain",value="All",options=["All"]+[str(s)[:3] for s in np.unique(gain)])
...
ymap = {"Covar":np.sum(covar,axis=0),
...
curdoc().add_root(HBox(VBox(xaxis_select,yaxis_select,speed_select,dir_select,gain_select),p))
