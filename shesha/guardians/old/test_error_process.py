...
def Init(config, device=0):
    ...
    rtc = ao.rtc_init(tel, wfs, config.p_wfss, dms, config.p_dms, config.p_geom,
                      ...
                      tar, config.p_target, clean=clean, simul_name=simul_name,
                      load=matricesToLoad)
...
def check_param(config, niter, save=False, device=5):
    ...
    for i in range(len(param)):
        bv, nv, dspm, fm, dspKmed, fKmed = process_err(config, niter, param[i],
                                                       save=save, linear=True,
                                                       device=device)
        if (save):
            store = pandas.HDFStore("/home/fferreira/Data/resErrorProcessMicado2.h5")
            df = store["results"]
...
def loop(n, full_com, noise_com, noise_mes, atm, wfs, dms, tar, rtc, tel, config):
    ...
    for i in range(n):
        ...
        rtc.docontrol(0)
        noise_com[:, i] = noise_com[:, i - 1] * (
                1 - config.p_controllers[0].gain) + rtc.getCom(
                        0)  #rtc.getErr(0)*config.p_controllers[0].gain
...
##########################################################################
param, brvar, nvar, dsp_med, f_med, dsp_Kmed, f_Kmed = check_param(
        config, 50000, save=True)
