...
class html_display:
    ...
    def __init__(self):
        ...
        self.inputs = HBox(
                VBox(self.comsTags, self.coms, self.methTags, self.meth,
                     self.plot_select, self.iter_select), width=650)
        self.inputs2 = HBox(
                VBox(self.imat, self.A, self.B, self.power, self.draw, self.cmax,
                     self.cmin, self.cut))  #, width=350)
    ...
    def cut_matrix(self):
        ...
        self.p2.image_url(url=dict(value=self.url + time + ".png"), x=0,
                          y=self.covmat.shape[0], w=self.covmat.shape[0],
                          h=self.covmat.shape[0], retry_attempts=2, retry_timeout=1000)
        self.dialog.visible = False
