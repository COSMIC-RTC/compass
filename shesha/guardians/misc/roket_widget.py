...
class html_display:
    ...
    def __init__(self):
        ...
        self.plot_select = Select(
            title="Plot type", value=self.plot_type[1], options=self.plot_type
        )
        ...
        self.power = Slider(title="Abs(covmat)**X", start=0.1, end=1.0, step=0.1, value=1.0)
        ...
        self.modes_select = Slider(title="Mode #", value=0, start=0, end=self.nmodes, step=1)
        ...
        self.psf_rec_methods_select = Select(
            title="Reconstruction method", value="Vii", options=["Vii", "ROKET"]
        )
        self.gamora_tag = Paragraph(text="PSF reconstruction :", height=25)
        self.psf_display_tag = Paragraph(text="PSF display :", height=25)
        self.error_select = CheckboxButtonGroup(
            labels=self.coms_list + ["fitting"], active=[0, 1, 2, 3, 4, 5, 6]
        )
            ...
            VBox(
                ...
                VBox(
                    VBox(
                        HBox(
                            self.modes_select,
                            HBox(self.desinc_mode, self.inc_mode, height=40),
                        )
                    ),
                    self.draw_mode,
                ),
        ...
        self.inputs4 = HBox(
            VBox(
                HBox(self.DB_select, self.DB_button),
                self.gamora_tag,
                self.psf_rec_methods_select,
                self.error_select,
                self.independence,
                self.gamora_comp,
                self.psf_display_tag,
                self.psf_display_select,
                self.psf_display,
            ),
            width=350,
        )
        self.tab1 = Panel(
            child=HBox(self.inputs, VBox(self.plog, self.psum)),
            title="Breakdown",
        )
        self.tab2 = Panel(
            child=HBox(
        ...
        if psf_type == b"Vii":
            ...
            if independence:
                self.otftel, self.otf2, self.psf, self.gamora = gamora.psf_rec_Vii(
                    self.datapath + str(self.DB_select.value),
                    fitting=fiterr,
                    covmodes=covmodes,
                )
            else:
    ...
    def update_psf(self):
        ...
        if image is not None:
            ...
            self.ppsf.image_url(
                url=dict(value=self.url + time + ".png"),
                x=0,
                y=0,
                w=image.shape[0],
                h=image.shape[0],
    ...
    def rescale_matrix(self):
        ...
        self.p2.image_url(
            ...
            x=0,
            y=0,
            w=self.covmat.shape[0],
            h=self.covmat.shape[0],
        )
        self.dialog.visible = False
    ...
    def update_matrix2(self):
        ...
        self.cmin.step = (self.cmin.end - self.cmin.start) / 100.0
    ...
    def plot_sum(self):
        ...
        for i in plus:
            ...
            if self.plus_select.labels[i] != "CORRECT":
                ...
                theta -= self.f.attrs["winddir"][0] * np.pi / 180.0
                r0 = self.f.attrs["r0"] * (
                    self.f.attrs["target.Lambda"][0] / self.f.attrs["wfs.Lambda"][0]
                ) ** (6.0 / 5.0)
                ...
                for k in range(self.f.attrs["nscreens"]):
                    ...
                    htheta = (
                        np.sqrt(self.f.attrs["wfs.xpos"][0] ** 2 + self.f.attrs["wfs.ypos"][0] ** 2)
                        / RASC
                        * H
                    )
                    Dtomo += 6.88 * (htheta / r0) ** (5.0 / 3.0)
                    Dbp += 6.88 * (v * dt / g / r0) ** (5.0 / 3.0)
                    rho = np.sqrt(
                        htheta**2
                        + (v * dt / g) ** 2
                        - 2 * htheta * v * dt / g * np.cos(np.pi - theta)
                    )
                    Dcov += 6.88 * (rho / r0) ** (5.0 / 3.0)
        ...
        if list(self.f.keys()).count("SR2"):
            self.source3.data = dict(
                ...
                y=[
                    ...
                    np.ones(len(x)) * self.f["SR2"].value,
                    data2,
                ],
            ...
            if list(self.f.keys()).count("SR"):
                self.source3.data = dict(
                    x=[x, x, x],
                    y=[data, np.ones(len(x)) * self.f["SR"].value, data2],
    ...
    def createDataTables(self):
        ...
        param_table = DataTable(source=self.table_param_source, columns=tmp, width=700, height=500)

        return cov_table, cor_table, param_table

    def updateDataTables(self):
        self.table_cov_source.data = dict(
            Type=self.DataTableItems[1:],
            Noise=self.cov[:, 0],
        ...
        self.pcov_source.data = dict(
            image=[self.cov], x=[0], y=[0], dw=[6], dh=[6], palette="Spectral11"
        )
        self.pcor_source.data = dict(
            image=[self.cor], x=[0], y=[0], dw=[6], dh=[6], palette="Spectral11"
        )
