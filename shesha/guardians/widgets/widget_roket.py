...
class Bokeh_roket:
    ...
    def __init__(self):
        ...
        self.table_breakdown = DataTable(
            source=self.source_breakdown, columns=columns, width=400, height=75
        )
        ...
        self.table_cov = DataTable(source=self.source_cov, columns=columns2, width=400, height=200)
        columns2[0] = TableColumn(field="Type", title="Cor.")
        self.table_cor = DataTable(source=self.source_cor, columns=columns2, width=400, height=250)
        # self.update_covcor()
        ...
        self.table_params = DataTable(source=self.source_params, columns=tmp, width=600, height=500)
        # self.update_params()
        self.source_variances = {}
        for c in self.contributors:
            self.source_variances[c] = ColumnDataSource(data=dict(x=[], y=[]))
        self.contributorsVariancePlot = figure(
            plot_height=600,
            plot_width=800,
            y_axis_type="log",
            y_range=[1e-9, 1],
            title="Contibutors variance",
        )
        for c in self.contributors:
            self.contributorsVariancePlot.line(
                x="x",
                y="y",
                legend=c,
                color=self.colors[c],
                muted_color=self.colors[c],
                muted_alpha=0.1,
                source=self.source_variances[c],
            )

        self.contributorsVariancePlot.legend.click_policy = "mute"
    ...
    def update_covcor(self):
        ...
        self.source_cov.data = dict(
            Type=["Noise", "Trunc", "Alias.", "Filt.", "Band.", "Tomo"],
            Noise=["%.2E" % v for v in self.cov[:, 0]],
        ...
        self.source_cor.data = dict(
            Type=["Noise", "Trunc", "Alias.", "Filt.", "Band.", "Tomo"],
            Noise=["%.2f" % v for v in self.cor[:, 0]],
    ...
    def update(self):
        ...
        for c in self.contributors:
            ...
            data = self.f[c][:]
            self.contributorsVariancePlot.xaxis.axis_label = "Actuators"
            if basis_active:
                data = self.P.dot(data)
                self.contributorsVariancePlot.xaxis.axis_label = "Modes"
