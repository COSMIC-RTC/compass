...
class Bokeh_groot:
    def __init__(self):
        ...
        self.url = "http://" + os.uname()[1] + ".obspm.fr/~" + os.getlogin() + "/roket_display"
        self.old = None
        ...
        self.radioButton_basis = RadioButtonGroup(labels=["Actus", "Btt", "Slopes"], active=0)
        ...
        self.image_roket = figure(
            x_range=self.xdr,
            y_range=self.ydr,
            x_axis_location="above",
            title="PSF ROKET",
        )
        self.image_groot = figure(
            x_range=self.image_roket.x_range,
            y_range=self.image_roket.y_range,
            x_axis_location="above",
            title="PSF GROOT",
        )
        self.im_covmat_roket = figure(
            x_range=self.xdr2,
            y_range=self.ydr2,
            x_axis_location="above",
            title="Covmat ROKET",
        )
        self.im_covmat_groot = figure(
            x_range=self.im_covmat_roket.x_range,
            y_range=self.im_covmat_roket.y_range,
            x_axis_location="above",
            title="Covmat GROOT",
        )
        self.plot_psf_cuts = figure(
            plot_height=600,
            plot_width=800,
            y_range=[1e-9, 1],
            x_range=self.image_roket.x_range,
        ...
        self.plot_psf_cuts.line(
            x="x",
            y="y",
            legend="ROKET",
            color="blue",
            muted_alpha=0.1,
            source=self.source_psf_roket,
        )
        self.plot_psf_cuts.line(
            x="x",
            y="y",
            ...
            color="red",
            muted_alpha=0.1,
            source=self.source_psf_compass,
        )
        self.plot_psf_cuts.line(
            x="x",
            y="y",
            ...
            color="green",
            muted_alpha=0.1,
            source=self.source_psf_groot,
        )
        self.plot_psf_cuts.legend.click_policy = "mute"

        # Callback functions
        self.select_datapath.on_change("value", lambda attr, old, new: self.update_files())
        ...
        self.tab = Panel(
            child=layout(
                [
                    [
                        self.control_box,
                        self.im_covmat_roket,
                        self.im_covmat_groot,
                    ],
                    [self.image_roket, self.image_groot],
                    [self.plot_psf_cuts],
                ]
            ),
            title="GROOT",
    ...
    def update_files(self):
        ...
        self.files = self.files = [f.split("/")[-1] for f in glob(self.datapath + "roket_*.h5")]
        if self.files == []:
            self.files = ["No hdf5 files"]
    ...
    def update_psf(self):
        ...
        if self.psf_groot is not None:
            ...
            self.image_groot.image_url(
                ...
                x=0,
                y=0,
                w=self.psf_groot.shape[0],
                h=self.psf_groot.shape[0],
            )
    ...
    def update_covmats(self):
        ...
        if self.covmat_groot is not None:
            ...
            self.im_covmat_groot.image_url(
                ...
                x=0,
                y=0,
                w=self.covmat_groot.shape[0],
                h=self.covmat_groot.shape[0],
            )
    ...
    def comp_covmats(self):
        ...
        contrib = [self.contributors[c] for c in self.checkboxButtonGroup_contributors.active]
        if contrib == []:
        ...
        if modal != 2:
            ...
            err = drax.get_err_contributors(self.datapath + str(self.select_files.value), contrib)
    ...
    def comp_psf(self):
        ...
        otftel, otf2, self.psf_groot, _ = gamora.psf_rec_Vii(
            self.datapath + str(self.select_files.value),
            cov=self.covmat_groot.astype(np.float32),
            fitting=False,
        )
        if fit:
        ...
        _, _, self.psf_roket, _ = gamora.psf_rec_Vii(
            self.datapath + str(self.select_files.value),
            cov=self.covmat_roket.astype(np.float32),
            fitting=fit,
        )
