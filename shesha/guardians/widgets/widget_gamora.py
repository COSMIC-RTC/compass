...
class Bokeh_gamora:
    def __init__(self):
        ...
        self.image_compass = figure(
            x_range=self.xdr,
            y_range=self.ydr,
            x_axis_location="above",
            title="PSF COMPASS",
        )
        self.image_Vii = figure(
    ...
    def update_cuts(self):
        ...
        if self.psf_Vii is not None:
            self.source_psf_Vii.data = dict(x=x, y=self.psf_Vii[:, self.psf_Vii.shape[0] // 2])
    ...
    def comp_psf(self):
        ...
        _, _, self.psf_Vii, _ = gamora.psf_rec_Vii(self.datapath + str(self.select_files.value))
        self.psf_compass = self.f["psf"][:]
        self.update_psf()
        self.pretext.text = """ """
