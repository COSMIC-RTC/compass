...
def test_set_ncpa_wfs():
    sup.wfs.set_ncpa_wfs(0, sup.wfs.get_wfs_phase(0) * 0.0)
    assert True
...
def test_set_flat():
    sup.rtc.set_flat(0, sup.wfs.get_wfs_image(0) * 0 + 1.0)
    assert True
...
@pytest.mark.skipif(
    "petal" not in [dm.influ_type for dm in sup.config.p_dms],
    reason="Petal dm only",
)
...
def test_do_imat_phase():
    sup.calibration.do_imat_phase(
        0,
        np.zeros(
            (
                5,
                sup.wfs.get_ncpa_wfs(0).shape[0],
                sup.wfs.get_ncpa_wfs(0).shape[0],
            )
        ),
    )
...
@pytest.mark.skipif(
    sup.config.p_controllers[1].type != "generic_linear",
