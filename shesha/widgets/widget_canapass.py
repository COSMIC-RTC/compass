...
class widgetAOWindow(TemplateBaseClass):
    ...
    def __init__(self):
        ...
        self.mainLoop = [
            self.tel, self.atm, self.wfs, self.rtc, self.tar, self.dms]
        self.brama_flag = 1
        self.see_atmos = 0
        ...
        # Create Loop thread
        self.aoLoopThread = aoLoopThread(
            self.mainLoop, self.config, self.img, self.ui.wao_strehlSE, self.ui.wao_strehlLE, 1, self.hist, self.RTDisplay, self.RTDFreq)
        self.connect(self.aoLoopThread, QtCore.SIGNAL(
            "currentLoopFrequency(float)"), self.updateCurrentLoopFrequency)
        self.connect(self.aoLoopThread, QtCore.SIGNAL(
            "currentSRSE(QString)"), self.updateSRSE)
        self.connect(self.aoLoopThread, QtCore.SIGNAL(
            "currentSRLE(QString)"), self.updateSRLE)

        self.connect(
            self.aoLoopThread, QtCore.SIGNAL("finished()"), self.aoLoopFinished)
    ...
    def InitConfig(self):
        ...
        self.rtc = ao.rtc_init(self.tel, self.wfs, self.config.p_wfss, self.dms, self.config.p_dms,
                               ...
                               self.atm, self.config.p_tel, self.config.p_loop, clean=clean, simul_name=simul_name, load=matricesToLoad, brama=self.brama_flag)
...
class aoLoopThread(QtCore.QThread):
    ...
    def mainLoop(self, itnum):
        ...
        if(self.RTDisplay):
            # "%1.2f"%SR[0])#str(SR[0]))
            self.emit(QtCore.SIGNAL('currentSRSE(QString)'), signal_se)
            # "%1.2f"%SR[1])#str(SR[1]))
            self.emit(QtCore.SIGNAL('currentSRLE(QString)'), signal_le)
            self.emit(
                QtCore.SIGNAL('currentLoopFrequency(float)'), CurrentFreq)
        ...
        # print CurrentFreq
        self.printInPlace("iter #%d SR: (L.E, S.E.)= %s, %srunning at %4.1fHz" % (
            itnum, signal_le, signal_se, CurrentFreq))
        # sys.stdout.flush()
