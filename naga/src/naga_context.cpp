...
void declare_naga_context(py::module &mod) {
  ...
  .def_static("get_instance", &carma_context::instance, py::return_value_policy::reference)
  .def_static("get_instance_1gpu", &carma_context::instance_1gpu, py::return_value_policy::reference)
 
  // .def(py::init([](py::buffer data, ::CORBA::Boolean copy) {
  //   py::buffer_info info = data.request();
  //   if(info.format != py::format_descriptor<::CORBA::ULong>::format())
  //     throw invalid_argument("Buffer given has an incorrect format, expected: uint32");
  //   ::CORBA::ULong *ptr;
  //   if (copy == 0) {
  ...
  //   }
  //   return unique_ptr<B::Dims>(new B::Dims(info.size, info.size, ptr, copy));
  // }))
 
  .def_property_readonly("ndevice", &carma_context::get_ndevice)
  .def_property_readonly("activeDevice", &carma_context::get_activeDevice)
  .def_property_readonly("activeRealDevice", &carma_context::get_activeRealDevice)
  .def_property_readonly("cudaRuntimeGetVersion", &carma_context::get_cudaRuntimeGetVersion)
  .def_property_readonly("driverVersion", &carma_context::get_cudaDriverGetVersion)
  .def("get_device", &carma_context::get_device)
  // .def("set_activeDevice", &carma_context::set_activeDevice)
  // .def("set_activeDeviceForce", &carma_context::set_activeDeviceForce)
