...
sutra_wfs_mpi::sutra_wfs_mpi(carma_context *context, const char* type, long nxsub,
...
sutra_wfs_mpi::sutra_wfs_mpi(carma_context *context, long nxsub, long nvalid,
    long nphase, long npup, float pdiam, int device, bool is_master_ =true) {
  this->d_camplipup = 0L;
  this->d_camplifoc = 0L;
  this->d_fttotim = 0L;
  this->d_ftkernel = 0L;
...
int sutra_wfs_mpi::allocate_worker_sh() {
  long *dims_data1 = new long[2];
  dims_data1[0] = 1;
  long *dims_data2 = new long[3];
  dims_data2[0] = 2;
  ...
  this->d_binimg = new carma_obj<float>(this->current_context, dims_data2);
  ...
  dims_data3[1] = nfft;
  dims_data3[2] = nfft;
  dims_data3[3] = nvalid / nWorker;

  this->d_hrimg = new carma_obj<float>(this->current_context, dims_data3);
  this->d_submask = new carma_obj<float>(this->current_context, dims_data3);
  this->d_camplipup = new carma_obj<cuFloatComplex>(this->current_context, dims_data3);
  this->d_camplifoc = new carma_obj<cuFloatComplex>(this->current_context, dims_data3);
  int mdims[2];
  ...
  if(rank==0)
    ...
    dims_data3[3] = nvalid / nWorker;
  this->d_bincube = new carma_obj<float>(this->current_context, dims_data3);

  dims_data1[1] = 2 * nvalid;
  this->d_slopes = new carma_obj<float>(this->current_context, dims_data1);

  if (this->ntot != this->nfft) {
    dims_data3[1] = ntot;
    dims_data3[2] = ntot;
    dims_data3[3] = nvalid / nWorker;
    this->d_fttotim = new carma_obj<cuFloatComplex>(this->current_context, dims_data3);
    mdims[0] = (int) dims_data3[1];
    mdims[1] = (int) dims_data3[2];
    ...
    this->d_hrmap = new carma_obj<int>(this->current_context, dims_data1);

  }

  dims_data2[1] = ntot;
  dims_data2[2] = ntot;
  this->d_ftkernel = new carma_obj<cuFloatComplex>(this->current_context, dims_data2);
  ...
  dims_data2[1] = nphase;
  dims_data2[2] = nphase;
  this->d_offsets = new carma_obj<float>(this->current_context, dims_data2);

  dims_data1[1] = nxsub;
  this->d_istart = new carma_obj<int>(this->current_context, dims_data1);
  this->d_jstart = new carma_obj<int>(this->current_context, dims_data1);

  dims_data2[1] = nrebin * nrebin;
  dims_data2[2] = npix * npix;
  this->d_binmap = new carma_obj<int>(this->current_context, dims_data2);

  dims_data1[1] = nvalid;
  this->d_subsum = new carma_obj<float>(this->current_context, dims_data1);

  this->d_fluxPerSub = new carma_obj<float>(this->current_context, dims_data1);
  this->d_validsubsx = new carma_obj<int>(this->current_context, dims_data1);
  this->d_validsubsy = new carma_obj<int>(this->current_context, dims_data1);

  dims_data2[1] = nxsub;
  dims_data2[2] = nxsub;
  this->d_isvalid = new carma_obj<int>(this->current_context, dims_data2);
...
int sutra_wfs_mpi::allocate_worker_pyr() {
  ...
  if ((this->type == "pyr") || (this->type == "pyr3")) {
    dims_data2[1] = ntot;
    dims_data2[2] = ntot;
    this->d_submask = new carma_obj<float>(this->current_context, dims_data2);
  ...
  if (this->type == "pyr") {
    ...
    dims_data2[1] = nfft;
    dims_data2[2] = nfft;
    this->d_poffsets = new carma_obj<cuFloatComplex>(this->current_context, dims_data2);
    dims_data2[1] = ntot;
    dims_data2[2] = ntot;
    this->d_phalfxy = new carma_obj<cuFloatComplex>(this->current_context, dims_data2);
    dims_data2[1] = nfft;
    dims_data2[2] = nfft;
    this->d_sincar = new carma_obj<float>(this->current_context, dims_data2);
  }
  ...
  if (this->type == "pyr")
    this->d_psum = new carma_obj<float>(this->current_context, dims_data1);
...
int sutra_wfs_mpi::wfs_initgs(float xpos, float ypos, float lambda, float mag,
...
int sutra_wfs_mpi::wfs_initarrays(int *phasemap, int *hrmap, int *binmap,
  ...
  MPI_Bcast(d_fluxPerSub->getData(), d_fluxPerSub->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast(d_offsets->getData(), d_offsets->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast(d_pupil->getData(), d_pupil->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast(d_binmap->getData(), d_binmap->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast(d_validsubsx->getData(), d_validsubsx->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast(d_validsubsy->getData(), d_validsubsy->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast(d_istart->getData(), d_istart->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast(d_jstart->getData(), d_jstart->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  if (this->ntot != this->nfft) {
    MPI_Bcast(d_hrmap->getData(), d_hrmap->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  }
  MPI_Bcast(d_ftkernel->getData(), d_ftkernel->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
...
int sutra_wfs_mpi::wfs_initarrays(int *phasemap, float *offsets, float *pupil,
...
int sutra_wfs_mpi::wfs_initarrays(cuFloatComplex *halfxy, cuFloatComplex *offsets,
...
int sutra_wfs_mpi::load_kernels(float *lgskern) {
  if (this->lgs)
...
int sutra_wfs_mpi::sensor_trace(sutra_atmos *yatmos) {
...
int sutra_wfs_mpi::sensor_trace(sutra_dms *ydm, int rst) {
  //do raytracing to get the phase
  int rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  int nWorkers;
...
//  cudaDeviceSynchronize();
  MPI_Bcast(d_gs->d_phase->d_screen->getData(), d_gs->d_phase->d_screen->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
#else
  float *h_tmp = (float*)malloc( d_gs->d_phase->d_screen->getNbElem() * sizeof(float));
  if(rank==0)
    d_gs->d_phase->d_screen->device2host(h_tmp);
  MPI_Bcast(h_tmp, d_gs->d_phase->d_screen->getNbElem(), MPI_FLOAT, 0, MPI_COMM_WORLD);
  if(rank>0)
    d_gs->d_phase->d_screen->host2device(h_tmp);
#endif

  return EXIT_SUCCESS;
}

int sutra_wfs_mpi::comp_sh_generic() {
  current_context->set_activeDevice(device);
  int rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  int nWorkers;
  ...
  fillcamplipup(this->d_camplipup->getData(),
      ...
      this->d_jstart->getData(), this->d_validsubsx->getData(offset),
      this->d_validsubsy->getData(offset), this->nphase,
      this->d_gs->d_phase->d_screen->getDims(1), this->nfft,
      this->nphase * this->nphase * this->nvalid/nWorkers, this->device,
      offset * this->nphase * this->nphase);
...
//      }
      ...
      indexfill(this->d_fttotim->getData(), this->d_camplifoc->getData(),
      ...
      */
      ...
      if (this->nstreams > 1)
        fillbincube_async(this->streams, this->d_bincube->getData(offset*npix*npix),
            this->d_fttotim->getData(), this->d_binmap->getData(),
            this->ntot * this->ntot, this->npix * this->npix,
            this->nrebin * this->nrebin, this->nvalid/nWorkers, this->device);
      else
        fillbincube(this->d_bincube->getData(offset*npix*npix), this->d_fttotim->getData(),
            this->d_binmap->getData(), this->ntot * this->ntot,
            this->npix * this->npix, this->nrebin * this->nrebin, this->nvalid/nWorkers,
            this->device);
      //fprintf(stderr, "[%s@%d]: I'm here!\n", __FILE__, __LINE__);
    }
  } else {
...
//            this->device);
  ...
  if (this->nstreams > 1)
    subap_reduce_async(this->npix * this->npix, this->nvalid/nWorkers, this->streams,
        this->d_bincube->getData(), this->d_subsum->getData(offset));
  else
    subap_reduce(this->d_bincube->getNbElem(), this->npix * this->npix,
        this->nvalid/nWorkers, this->d_bincube->getData(), this->d_subsum->getData(offset));

  //fprintf(stderr, "[%s@%d]: I'm here!\n", __FILE__, __LINE__);

  if (this->nstreams > 1)
    subap_norm_async(this->d_bincube->getData(), this->d_bincube->getData(),
        this->d_fluxPerSub->getData(), this->d_subsum->getData(offset), this->nphot,
...
int sutra_wfs_mpi::comp_pyr_generic() {
...
int sutra_wfs_mpi::comp_image_tele() {
  int result;
  if (this->type == "sh") {
    result = comp_sh_generic();
  } else {
...
int sutra_wfs_mpi::slopes_geom(int type, float *slopes) {
  /*
   normalization notes :
...
int sutra_wfs_mpi::slopes_geom(int type) {
  this->slopes_geom(type, this->d_slopes->getData());

  return EXIT_SUCCESS;
}

sutra_sensors_mpi::sutra_sensors_mpi(carma_context *context, const char* type, int nwfs,
...
sutra_sensors_mpi::sutra_sensors_mpi(carma_context *context, int nwfs, long *nxsub,
...
sutra_sensors_mpi::~sutra_sensors_mpi() {
...
int sutra_sensors_mpi::sensors_initgs(float *xpos, float *ypos, float *lambda,
