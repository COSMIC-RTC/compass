...
//    status=read_shm<char>(shm_key, dims_type,SVIPC_CHAR, id_type, type);
    ...
    status = read_shm<long>(shm_key, dims_nvalid, SVIPC_LONG, id_nvalid,
        nvalid);
    ...
    status = read_shm<long>(shm_key, dims_nphase, SVIPC_LONG, id_nphase,
        nphase);

    char id_nrebin[] = "nrebin";
    int dims_nrebin[] = { 1, nwfs };
    status = read_shm<long>(shm_key, dims_nrebin, SVIPC_LONG, id_nrebin,
        nrebin);
...
//  }
  ...
  if (my_rank == 0) {
    ...
    status = read_shm<float>(shm_key, dims_lambda, SVIPC_FLOAT, id_lambda,
        lambda);
    ...
    status = read_shm<long>(shm_key, dims_gssize, SVIPC_LONG, id_gssize,
        gssize);
  ...
  if (my_rank == 0) {
    ...
    status = read_shm<int>(shm_key, dims_phasemap, SVIPC_INT, id_phasemap,
        phasemap);
    ...
    status = read_shm<float>(shm_key, dims_offsets, SVIPC_FLOAT, id_offsets,
        offsets);
    ...
    status = read_shm<float>(shm_key, dims_fluxPerSub, SVIPC_FLOAT,
        id_fluxPerSub, fluxPerSub);

    char id_isvalid[] = "isvalid";
    int dims_isvalid[] = { 1, sensors.d_wfs[0]->d_isvalid->getNbElem() };
    status = read_shm<int>(shm_key, dims_isvalid, SVIPC_INT, id_isvalid,
        isvalid);

    char id_validsubsx[] = "validsubsx";
    int dims_validsubsx[] = { 1, sensors.d_wfs[0]->d_validsubsx->getNbElem() };
    status = read_shm<int>(shm_key, dims_validsubsx, SVIPC_INT, id_validsubsx,
        validsubsx);

    char id_validsubsy[] = "validsubsy";
    int dims_validsubsy[] = { 1, sensors.d_wfs[0]->d_validsubsy->getNbElem() };
    status = read_shm<int>(shm_key, dims_validsubsy, SVIPC_INT, id_validsubsy,
        validsubsy);
    ...
    status = read_shm<float>(shm_key, dims_ftkernel, SVIPC_FLOAT, id_ftkernel,
        ftkernel);
  }

  MPI_Bcast(phasemap, sensors.d_wfs[0]->d_phasemap->getNbElem(), MPI_INT, 0,
      MPI_COMM_WORLD );
  MPI_Bcast(hrmap, hrmap_size, MPI_INT, 0, MPI_COMM_WORLD );
  MPI_Bcast(binmap, sensors.d_wfs[0]->d_binmap->getNbElem(), MPI_INT, 0,
      MPI_COMM_WORLD );
  MPI_Bcast(offsets, sensors.d_wfs[0]->d_offsets->getNbElem(), MPI_FLOAT, 0,
      MPI_COMM_WORLD );
  MPI_Bcast(pupil, sensors.d_wfs[0]->d_pupil->getNbElem(), MPI_FLOAT, 0,
      MPI_COMM_WORLD );
  MPI_Bcast(fluxPerSub, sensors.d_wfs[0]->d_fluxPerSub->getNbElem(), MPI_FLOAT,
      0, MPI_COMM_WORLD );
  MPI_Bcast(isvalid, sensors.d_wfs[0]->d_isvalid->getNbElem(), MPI_INT, 0,
      MPI_COMM_WORLD );
  MPI_Bcast(validsubsx, sensors.d_wfs[0]->d_validsubsx->getNbElem(), MPI_INT, 0,
      MPI_COMM_WORLD );
  MPI_Bcast(validsubsy, sensors.d_wfs[0]->d_validsubsy->getNbElem(), MPI_INT, 0,
      MPI_COMM_WORLD );
  MPI_Bcast(istart, sensors.d_wfs[0]->d_istart->getNbElem(), MPI_INT, 0,
      MPI_COMM_WORLD );
  MPI_Bcast(jstart, sensors.d_wfs[0]->d_jstart->getNbElem(), MPI_INT, 0,
      MPI_COMM_WORLD );
  MPI_Bcast(ftkernel, 2 * sensors.d_wfs[0]->d_ftkernel->getNbElem(), MPI_FLOAT,
      0, MPI_COMM_WORLD );

  sensors.d_wfs[0]->wfs_initarrays(phasemap, hrmap, binmap, offsets, pupil,
      fluxPerSub, isvalid, validsubsx, validsubsy, istart, jstart,
      (cuFloatComplex *) ftkernel);
  ...
  if (1) {
    ...
    float h_data[sensors.d_wfs[0]->d_gs->d_phase->d_screen->getNbElem()];

    int hdutype, bitpix, bytepix, naxis, nkeys, datatype = 0, anynul;
    long naxes[3];
    long totpix = 1;
    double nulval = 0.;
    if (!fits_open_file(&fptr, "tilty.fits", READONLY, &status)) {
      ...
      if (hdutype == IMAGE_HDU) {
        ...
        fits_get_img_param(fptr, 3, &bitpix, &naxis, naxes,
            &status);
        //fprintf(stderr,"naxis[index]: %d\n", naxis[index]);

        totpix = 1; //naxes[0] * naxes[1] * naxes[2]; // axis1, axis2, ..., nbImg
        int ii;
        for (ii = 0; ii < naxis; ii++) {
          totpix *= naxes[ii];
          //fprintf(stderr,"%d ", naxes[index][ii]);
        }
        //fprintf(stderr,"-> %d\n", totpix);
        ...
        /* read all or part of image then write it back to the inputdata buffer */
        fits_read_img(fptr, datatype, 1, totpix, &nulval, h_data,
            &anynul, &status);

        for(int ii=0; ii<sensors.d_wfs[0]->d_gs->d_phase->d_screen->getNbElem(); ii++) h_data[ii]*=33;
        sensors.d_wfs[0]->d_gs->d_phase->d_screen->host2device(h_data);
      }
  ...
  int npix_tot = sensors.d_wfs[0]->nxsub * sensors.d_wfs[0]->nxsub
      * sensors.d_wfs[0]->nvalid;
  int npix_per_node = npix_tot / nb_rank;

  fprintf(stderr,
      "my_rankb %d, npix_tot %d, npix_per_node %d,nvalid %d, getNbElem %d\n",
      my_rank, npix_tot, npix_per_node, sensors.d_wfs[0]->nvalid,
      sensors.d_wfs[0]->d_bincube->getNbElem());
if(my_rank==0)
  MPI_Gather(MPI_IN_PLACE, npix_per_node,
      MPI_FLOAT, h_bincube, npix_per_node, MPI_FLOAT, 0, MPI_COMM_WORLD );
else
  MPI_Gather(h_bincube, npix_per_node, MPI_FLOAT, h_bincube, npix_per_node,
      MPI_FLOAT, 0, MPI_COMM_WORLD );

  //MPI_Barrier(MPI_COMM_WORLD);

  if (my_rank == 0) {
//    sensors.d_wfs[0]->d_bincube->host2device(h_bincube);
    fitsfile *fptr; /* FITS file pointer, defined in fitsio.h */
    char filename[256];
    sprintf(filename, "!image_rank%d.fits", my_rank);
