#ifndef _sutra_wfs_mpi_H_
#define _sutra_wfs_mpi_H_
...
public:
  sutra_wfs_mpi(carma_context *context, const char* type, long nxsub, long nvalid,
      long npix, long nphase, long nrebin, long nfft, long ntot, long npup,
      float pdiam, float nphotons, int lgs, int device, bool master);
  sutra_wfs_mpi(carma_context *context, long nxsub, long nvalid, long nphase,
      long npup, float pdiam, int device, bool master);
  sutra_wfs_mpi(const sutra_wfs_mpi& wfs);
  ~sutra_wfs_mpi();

  int allocate_worker_sh();
  int allocate_worker_pyr();
...
class sutra_sensors_mpi {
public:
  size_t nsensors() {
    return d_wfs.size();
  }
  vector<sutra_wfs_mpi *> d_wfs;

public:
  sutra_sensors_mpi(carma_context *context, const char* type, int nwfs, long *nxsub,
      long *nvalid, long *npix, long *nphase, long *nrebin, long *nfft,
      long *ntot, long npup, float *pdiam, float *nphot, int *lgs, int device);
  sutra_sensors_mpi(carma_context *context, int nwfs, long *nxsub, long *nvalid,
...
fillcamplipup(cuFloatComplex *amplipup, float *phase, float *offset,
    ...
    int *jvalid, int nphase, int npup, int Nfft, int Ntot, int device, int offset_phase);
int
