...
extern "C" {
  ...
  Y_yoga_magma_getri(int argc)
  {
	...
	/* Factor the matrix */
	magma_dsetmatrix( N, N, (float *)h_A, lda, d_A->getData(), ldda );
	magma_dgetrf_gpu( N, N, d_A->getData(), ldda, ipiv, &info );
	magma_dgetmatrix( N, N, d_A->getData(), ldda, (float *)h_A, lda );

	magma_dgetri_gpu( N,    d_A->getData(), ldda, ipiv, (float *)dwork, ldwork, &info );
      }
