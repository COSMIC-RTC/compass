  ...
  if (!yarg_subroutine()) {
    ...
    if (d_R_handler->type == Y_DOUBLE) {
      ...
      /* Query for workspace sizes */magma_dsyevd_gpu_t(jobz, uplo, N,
          (double *) d_R->getData(), ldda, (double *) w1, (double *) h_R, lda,
      ...
      if (yType == Y_OPAQUE) {
        ...
        magma_dsetmatrix(N, N, (double * )h_A, lda, (double * )d_R->getData(),
            ldda);
      }
      magma_dsyevd_gpu_t(jobz, uplo, N, (double *) d_R->getData(), ldda,
          (double *) w1, (double *) h_R, lda, (double *) h_work, lwork, iwork,
          liwork, &info);
    }
    if (d_R_handler->type == Y_FLOAT) {
      ...
      /* Query for workspace sizes */magma_ssyevd_gpu_t(jobz, uplo, N,
          (float *) d_R->getData(), ldda, (float *) w1, (float *) h_R, lda,
          aux_work, -1, aux_iwork, -1, &info);
      ...
      if (yType == Y_OPAQUE) {
        ...
        magma_ssetmatrix(N, N, (float * )h_A, lda, (float * )d_R->getData(),
            ldda);
      }

      magma_ssyevd_gpu_t(jobz, uplo, N, (float *) d_R->getData(), ldda,
          (float *) w1, (float *) h_R, lda, (float *) h_work, lwork, iwork,
          liwork, &info);
    }
...
void Y_yoga_magma_getri(int argc) {
  ...
  if (yarg_subroutine()) {
    ...
    if (yType == Y_DOUBLE) {
      ...
      magma_dgetri_gpu(N, d_A->getData(), ldda, ipiv, (double *) dwork, ldwork,
          &info);
    ...
    if (yType == Y_FLOAT) {
      ...
      magma_sgetri_gpu(N, d_A->getData(), ldda, ipiv, (float *) dwork, ldwork,
          &info);
    }
