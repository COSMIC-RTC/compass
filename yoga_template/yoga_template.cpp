...
extern "C" {
  ...
  Y_yoga_magma_evd(int argc)
  {
    ...
    if (!yarg_subroutine()) {
      ...
      if (N * ldda != d_R->getNbElem()) y_error("Please provide arrays of proper size\n");
      ...
      w1 = ypush_d(dims_eigen);

      /* Initialize the matrix */
      //fprintf(stderr, "%s: %s@%d\n", __FILE__, __FUNCTION__, __LINE__);
      magma_dsetmatrix( N, N, (double *)h_A, lda, d_R2, ldda );
