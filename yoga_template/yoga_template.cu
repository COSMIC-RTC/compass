...
template<class T> __global__ void multim_krnl(T *odata, T* idata, int N)
{
...
template int multim<float>(float *d_odata,float *d_idata, int N);
template int multim<double>(double *d_odata,double *d_idata, int N);
