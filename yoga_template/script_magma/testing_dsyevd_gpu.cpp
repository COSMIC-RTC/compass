...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < opts.ntest; ++i) {
		for (int iter = 0; iter < opts.niter; ++iter) {
			...
			/* Query for workspace sizes */magma_dsyevd_gpu_t(opts.jobz,
					opts.uplo, N, d_R, ldda, w1, h_R, lda, aux_work, -1,
					aux_iwork, -1, &info);
			...
			/* warm up run */
			if (opts.warmup) {
				magma_dsyevd_gpu_t(opts.jobz, opts.uplo, N, d_R, ldda, w1, h_R,
						lda, h_work, lwork, iwork, liwork, &info);
				if (info != 0)
			...
			if (opts.check) {
				...
				magma_dsetmatrix(N, N, h_A, lda, d_R, ldda);
				magma_dsyevd_gpu_t(MagmaNoVec, opts.uplo, N, d_R, ldda, w2, h_R,
						lda, h_work, lwork, iwork, liwork, &info);
				if (info != 0)
					printf("magma_dsyevd_gpu returned error %d: %s.\n",
							(int) info, magma_strerror(info));
			...
			if (opts.check) {
				...
				printf("(1)    | A - U S U' | / (|A| N)     = %8.2e%s\n",
						result[0] * eps,
						(result[0] * eps < tol ? "" : "  failed"));
				printf("(2)    | I -   U'U  | /  N          = %8.2e%s\n",
						result[1] * eps,
						(result[1] * eps < tol ? "" : "  failed"));
