...
void kp_kalman_core_full_CPU::calculate_gain(float bruit_pix, float k_W,
                                             const kp_matrix<double>& SigmaV,
                                             const kp_vector<double>& atur_,
                                             const kp_vector<double>& btur_) {
  if (atur_.size() != nb_az) {
    cerr
        << "Error | kp_kalman_core_full_CPU::calculate_gain | size problem atur"
        << endl;
    exit(EXIT_FAILURE);
  }
  if (btur_.size() != nb_az) {
    cerr
        << "Error | kp_kalman_core_full_CPU::calculate_gain | size problem btur"
        << endl;
    exit(EXIT_FAILURE);
  }
  if ((SigmaV.getDim1() != SigmaV.getDim2()) || (SigmaV.getDim1() != nb_az)) {
    cerr << "Error | kp_kalman_core_full_CPU::calculate_gain | size problem "
            "SigmaV"
         << endl;
    exit(EXIT_FAILURE);
  }
  ...
  if (!gainComputed) H_inf.resize(0, 0);
  ...
  if (ordreAR == 1)
    alpha_kp1.resize(nb_az, nb_az);
  else
    alpha_kp1.resize(nb_n, nb_n);
  ...
  if (ordreAR == 1)
    beta_kp1.resize(nb_az, nb_az);
  else
    beta_kp1.resize(nb_n, nb_n);
  beta_kp1.zeros();
  ...
  if (ordreAR == 1)
    ...
    ms.push_back(T_kp1sk_tmp);
    ms.push_back(zeros_nbaz_nbaz);
    kp_matrix<double>* beta_kp1_tmp = new kp_matrix<double>(nb_n, nb_az);
    kp_vertcat(*beta_kp1_tmp, ms);
    delete zeros_nbaz_nbaz;
    zeros_nbaz_nbaz = NULL;
    delete T_kp1sk_tmp;
    T_kp1sk_tmp = NULL;
    ms.clear();
    ...
    ms.push_back(beta_kp1_tmp);
    ms.push_back(zeros_nbn_nbaz);
    kp_horizcat(T_kp1sk, ms);
    delete beta_kp1_tmp;
    beta_kp1_tmp = NULL;
    delete zeros_nbn_nbaz;
    zeros_nbn_nbaz = NULL;
    ms.clear();
  }
  ...
  if ((expression == 1) && (ordreAR == 1)) {
    ...
    *Sinf_0_0 += SigmaV;
    ...
    ms.push_back(Sinf_0_0);
    ms.push_back(Atur_Tkp1sk);
    ...
    ms.push_back(Sinf_1_0);
    ms.push_back(&T_kp1sk);
    ...
    if (ordreAR == 1)
      Tkp1sk_C1t->resize(nb_az, nb_p);
    else
      Tkp1sk_C1t->resize(nb_n, nb_p);
    kp_gemm('N', 'T', 1, T_kp1sk, C1, 1, *Tkp1sk_C1t);

    // Calcul de C1 * T_kp1sk * C1' + SigmaW
    kp_matrix<double>* Sigma_tot =
        new kp_matrix<double>(C1.getDim1(), Tkp1sk_C1t->getDim2());
    ...
    if (ordreAR == 1) {
      kp_matrix<double>* H_opt =
          new kp_matrix<double>(Tkp1sk_C1t->getDim1(), inv_Sigmatot->getDim2());
...
void kp_kalman_core_full_CPU::next_step(const kp_vector<KFPP>& Y_k,
                                        kp_vector<KFPP>& U_k) {
  ...
  if (!gainComputed) {
    cerr << "Error | kp_kalman_core_full_CPU::next_step | gain has not been "
            "initialized"
         << endl;
    exit(EXIT_FAILURE);
  }
