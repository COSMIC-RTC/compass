...
kp_kalman_core_sparse_GPU::kp_kalman_core_sparse_GPU(
    const kp_smatrix<KFPP>& D_Mo_, const kp_smatrix<KFPP>& N_Act_,
    const kp_smatrix<KFPP>& PROJ_, bool isZonal_, cublasHandle_t cublasHandle_,
    cusparseHandle_t cusparseHandle_)
    : kp_kalman_core_sparse(D_Mo_, N_Act_, PROJ_, isZonal_),
...
void kp_kalman_core_sparse_GPU::calculate_gain(float bruit_pix, float k_W,
  ...
  if (cu_atur_.size() != nb_az) {
    cerr << "Error | kp_kalman_core_sparse_GPU::kp_kalman_core_sparse_GPU | "
            "size problem atur"
         << endl;
    exit(EXIT_FAILURE);
  }
  ...
  if ((SigmaV.getDim1() != SigmaV.getDim2()) || (SigmaV.getDim1() != nb_az)) {
    cerr << "Error | kp_kalman_core_sparse_GPU::calculate_gain | size problem "
            "SigmaV"
         << endl;
    exit(EXIT_FAILURE);
  }
  ...
  if (!gainComputed) cu_H_inf.resize(0, 0);
  ...
  if (ordreAR == 1)
    cu_alpha_kp1.resize(nb_az, nb_az);
  else
    cu_alpha_kp1.resize(nb_n, nb_n);
  cu_alpha_kp1.zeros();

  // pour ordreAR 1 et 2
  kernel_memcpy_diag(cu_alpha_kp1.getData(), cu_atur_.getData(), 0, 0, nb_az,
                     cu_alpha_kp1.getDim1());
  // pour ordreAR 2 uniquement
  if (ordreAR == 2) {
    kernel_memset_diag(cu_alpha_kp1.getData(), 1.0, 0, nb_az, nb_az,
                       cu_alpha_kp1.getDim1());
    kernel_memcpy_diag(cu_alpha_kp1.getData(), cu_btur_.getData(), nb_az, 0,
                       nb_az, cu_alpha_kp1.getDim1());
  }
  kp_cu_matrix<double> cu_beta_kp1(0, 0);
  if (ordreAR == 1)
    cu_beta_kp1.resize(nb_az, nb_az);
  else
    cu_beta_kp1.resize(nb_n, nb_n);
  cu_beta_kp1.zeros();
  ...
  if (ordreAR == 1) {
    ...
    if (expression != 1) cu_D_Mo_double.resize(0, 0, 0);
  }
  ...
  if (ordreAR == 1)
    ...
    cu_ms.push_back(&cu_T_kp1sk_tmp);
    cu_ms.push_back(&cu_zeros_nbaz_nbaz);
    ...
    cu_ms.push_back(&cu_beta_kp1_tmp);
    cu_ms.push_back(&cu_zeros_nbn_nbaz);
  ...
  kp_cu_matrix<double> cu_alpha_k(cu_alpha_kp1.getDim1(),
                                  cu_alpha_kp1.getDim2());
  ...
  kp_cu_matrix<double> cu_alphak_IBG1_betak(cu_alpha_k.getDim1(),
                                            cu_beta_k.getDim2());
  kp_cu_matrix<double> cu_Tk_IBG1_alphak(cu_T_k.getDim1(),
                                         cu_alpha_k.getDim2());
  ...
  while ((ecart > seuil) && (boucle < boucle_max)) {
    ...
    cu_alphak_IBG1_betak.gemm(cublasHandle, 'N', 'N', 1, cu_alphak_IBG1,
                              cu_beta_k, 0);
    // beta_kp1 = alphak_IBG1_betak * (alpha_k)T (= alpha_k * IBG1 * beta_k *
    // (alpkha_k)T)
    cu_beta_kp1.gemm(cublasHandle, 'N', 'T', 1, cu_alphak_IBG1_betak,
                     cu_alpha_k, 1);
    // temps_betak.pause();
    ...
    cu_Tk_IBG1_alphak.gemm(cublasHandle, 'N', 'N', 1, cu_Tk_IBG1, cu_alpha_k,
                           0);
    // T_kp1sk = (alpha_k)T * Tk_IBG1_alphak (= (alpha_k)T * T_k * IBG1 *
    // alpha_k)
    cu_T_kp1sk.gemm(cublasHandle, 'T', 'N', 1, cu_alpha_k, cu_Tk_IBG1_alphak,
                    1);
    // temps_Tk.pause();

    kernel_get_diag(cu_diag_cu_Tkp1sk.getData(), cu_T_kp1sk.getData(),
                    cu_T_kp1sk.getDim1());
    kernel_get_diag(cu_diag_cu_Tk.getData(), cu_T_k.getData(),
                    cu_T_k.getDim1());
  ...
  if ((expression == 1) && (ordreAR == 1)) {
    ...
    kernel_diag_mult(cu_Atur_Tkp1sk.getData(), cu_T_kp1sk.getData(),
                     cu_atur_.getData(), cu_atur_.size(),
                     cu_T_kp1sk.getDim1() * cu_T_kp1sk.getDim2());
    ...
    kernel_diag_mult(cu_Sinf_0_0_t.getData(), cu_Atur_Tkp1sk_t.getData(),
                     cu_atur_.getData(), cu_atur_.size(),
                     cu_Atur_Tkp1sk_t.getDim1() * cu_Atur_Tkp1sk_t.getDim2());
    ...
    cu_ms.push_back(&cu_Sinf_0_0);
    cu_ms.push_back(&cu_Atur_Tkp1sk);
    ...
    kernel_diag_mult(cu_Sinf_1_0_t.getData(), cu_T_kp1sk_t.getData(),
                     cu_atur_.getData(), cu_atur_.size(),
                     cu_T_kp1sk_t.getDim1() * cu_T_kp1sk_t.getDim2());
    ...
    cu_ms.push_back(&cu_Sinf_1_0);
    cu_ms.push_back(&cu_T_kp1sk);
    ...
    cu_ms.push_back(&cu_Sinf_0);
    cu_ms.push_back(&cu_Sinf_1);
    kp_cu_vertcat(cu_S_inf, cu_ms);
    cu_ms.clear();
    cu_Sinf_0.resize(0, 0);
    cu_Sinf_1.resize(0, 0);

    kp_cu_smatrix<double> cu_zeros_Dmo;
    cu_zeros_Dmo.resize(0, 0, 0);
    cu_zeros_Dmo.resize(cu_D_Mo_double.nnz, cu_D_Mo_double.getDim1(), nb_n);
    kernel_memcpy(cu_zeros_Dmo.rowind_cu, cu_D_Mo_double.rowind_cu,
                  cu_zeros_Dmo.nnz);
    kernel_memcpy(cu_zeros_Dmo.colind_cu, cu_D_Mo_double.colind_cu,
                  cu_zeros_Dmo.nnz);
    kernel_memcpy(cu_zeros_Dmo.values_cu, cu_D_Mo_double.values_cu,
                  cu_zeros_Dmo.nnz);
    ...
    // Sinft = (S_inf)T
    kp_cu_matrix<double> cu_Sinft(0, 0);
    cu_Sinft.init_from_transpose(cublasHandle, cu_S_inf);

    cu_S_inf.resize(0, 0);

    // Sinf_zerosDmot_tmp = zeros_Dmo * Sinft  ( = [0 D_Mo] * (Sinf)T )
    kp_cu_gemm(cusparseHandle, 'N', 1, cu_zeros_Dmo, cu_Sinft, 0,
               cu_Sinf_zerosDmot_tmp);
    ...
    // Sigma_tot = zeros_Dmo * Sinf_zerosDmot  ( = [0 D_Mo] * Sinf * [0 ;
    // (D_Mo)T] )
    kp_cu_gemm(cusparseHandle, 'N', 1, cu_zeros_Dmo, cu_Sinf_zerosDmot, 0,
               cu_inv_Sigmatot);
    // Sigma_tot = Sigma_tot + SigmaW*Id (avec SigmaW reel)  (= [0 D_Mo] * Sinf
    // * [0 ; (D_Mo)T] + SigmaW*Id)
    kernel_add_diag_const(cu_inv_Sigmatot.getData(), SigmaW,
                          cu_inv_Sigmatot.getDim1());
    cu_zeros_Dmo.resize(0, 0, 0);
    ...
    // H_inf = Sinf_zerosDmot * inv_Sigmatot  ( = Sinf * [0 (D_Mo)T] *
    // inv(Sigma_tot))
    cu_H_inf2.gemm(cublasHandle, 'N', 'N', 1, cu_Sinf_zerosDmot,
                   cu_inv_Sigmatot, 0);
    ...
    cu_T_kp1sk.resize(0, 0);
    kp_cu_matrix<double> cu_Tkp1sk_C1t_tmp(cu_C1.getDim1(),
                                           cu_Tkp1skt.getDim2());
    ...
    kp_cu_matrix<double> cu_inv_Sigmatot(cu_C1.getDim1(),
                                         cu_Tkp1sk_C1t.getDim2());
    // Sigma_tot = SigmaW * Id (avec SigmaW reel)
    cu_inv_Sigmatot.zeros();
    // for(int i = 0 ; i < cu_inv_Sigmatot.getDim1() ; i++)
    // cu_inv_Sigmatot.getData()[i * cu_inv_Sigmatot.getDim1() + i] += SigmaW;
    kernel_add_diag_const(cu_inv_Sigmatot.getData(), SigmaW,
                          cu_inv_Sigmatot.getDim1());

    // Sigma_tot = Sigma_tot + C1 * Tkp1sk_C1t  ( = (C1 * T_kp1sk * (C1)T) +
    // SigmaW*Id )
    kp_cu_gemm(cusparseHandle, 'N', 1, cu_C1, cu_Tkp1sk_C1t, 1,
               cu_inv_Sigmatot);
    // kp_cu_gemm (cusparseHandle,cublasHandle, 'N', 'N',1 , cu_C1,
    // cu_Tkp1sk_C1t , 1, cu_inv_Sigmatot);
    cu_C1.resize(0, 0, 0);
    ...
    if (ordreAR == 1) {
      kp_cu_matrix<double> cu_H_opt(cu_Tkp1sk_C1t.getDim1(),
                                    cu_inv_Sigmatot.getDim2());
      // H_opt = Tkp1sk_C1t * inv_Sigmatot
      cu_H_opt.gemm(cublasHandle, 'N', 'N', 1, cu_Tkp1sk_C1t, cu_inv_Sigmatot,
                    0);
      ...
      kernel_diag_mult(cu_Atur_Hopt.getData(), cu_H_opt.getData(),
                       cu_atur_.getData(), cu_atur_.size(),
                       cu_H_opt.getDim1() * cu_H_opt.getDim2());

      // cu_H_inf.resize(nb_n,nb_p);
      cu_ms.push_back(&cu_Atur_Hopt);
      cu_ms.push_back(&cu_H_opt);
      ...
      cu_H_inf2.resize(nb_n, nb_p);
      cu_H_inf2.gemm(cublasHandle, 'N', 'N', 1, cu_Tkp1sk_C1t, cu_inv_Sigmatot,
                     0);
      cu_Tkp1sk_C1t.resize(0, 0);
      cu_inv_Sigmatot.resize(0, 0);
    }
...
void kp_kalman_core_sparse_GPU::next_step(const kp_vector<KFPP>& Y_k,
                                          kp_vector<KFPP>& U_k) {
  // ofstream fichier2;
  ...
  if (!gainComputed) {
    cerr << "Error | kp_kalman_core_sparse_GPU::next_step | gain has not been "
            "initialized"
         << endl;
    exit(EXIT_FAILURE);
  }
  ...
  // kp_cu_cudaMemcpy(tmp_vec1->d, cu_X_kskm1->d_cu+nb_az, nb_az*sizeof(KFPP),
  ...
  // X_kskm1_tmp = X_kskm1_tmp + H_inf * innovation (= X_kskm1 + H_inf * (Y_k -
  // Y_kskm1))
  kernel_memset(cu_X_kp1sk.getData(), (KFPP)0.0, nb_az);
  kernel_memcpy(cu_A1_00_Xkdebut.getData(), cu_X_kskm1_tmp.getData(), nb_az);
  kernel_memcpy(cu_A1_01_Xkfin.getData(), cu_X_kskm1_tmp.getData() + nb_az,
                nb_az);
