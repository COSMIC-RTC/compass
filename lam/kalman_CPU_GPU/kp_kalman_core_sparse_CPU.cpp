...
kp_kalman_core_sparse_CPU::kp_kalman_core_sparse_CPU(
...
void kp_kalman_core_sparse_CPU::calculate_gain(float bruit_pix, float k_W,
                                               const kp_matrix<double>& SigmaV,
                                               const kp_vector<double>& atur_,
                                               const kp_vector<double>& btur_) {
  if (atur_.size() != nb_az) {
    cerr << "Error | kp_kalman_core_sparse_CPU::calculate_gain | size problem "
            "atur"
         << endl;
    exit(EXIT_FAILURE);
  }
  ...
  if ((expression == 1) && (ordreAR == 1)) {
    ...
    // Sinft = (S_inf)T
    kp_matrix<double> Sinft;
    Sinft.init_from_transpose(S_inf);
    S_inf.resize(0, 0);
    ...
    // for(int i = 0 ; i < Sigma_tot->dim1 ; i++) Sigma_tot->d[i *
    // Sigma_tot->dim1 + i] += SigmaW.d[i];
...
void kp_kalman_core_sparse_CPU::next_step(const kp_vector<KFPP>& Y_k,
                                          kp_vector<KFPP>& U_k) {
#ifdef __TEMPS_BOUCLE_KALMAN__
  temps_boucle.start();
#endif
  if (!gainComputed) {
    cerr << "Error | kp_kalman_core_sparse_CPU::next_step | gain has not been "
            "initialized"
         << endl;
    exit(EXIT_FAILURE);
  }
