...
kp_kalman_core_sparse::kp_kalman_core_sparse(const kp_smatrix<KFPP>& D_Mo_,
  ...
  if ((N_Act_.getDim1() != D_Mo_.getDim2()) ||
      N_Act_.getDim1() != PROJ_.getDim2() || N_Act_.getDim1() != nb_az) {
    cerr << "Error | kp_kalman_core_sparse::kp_kalman_core_sparse | dimension "
            "problem N_Act.getDim1() or D_Mo.getDim2() or PROJ.getDim2()"
         << endl;
    exit(EXIT_FAILURE);
  }
  ...
  if ((N_Act_.getDim2() != PROJ_.getDim1()) || N_Act_.getDim2() != nb_act) {
    cerr << "Error | kp_kalman_core_sparse::kp_kalman_core_sparse | dimension "
            "problem N_Act.getDim2() or PROJ.getDim1()"
         << endl;
    exit(EXIT_FAILURE);
  }
}
