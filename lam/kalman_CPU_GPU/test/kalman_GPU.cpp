...
int main() {
  ...
  if (cublasStat != CUBLAS_STATUS_SUCCESS) {
    cerr << "Error | kp_kalman_core_full_GPU::kp_kalman_core_full_GPU | CUBLAS "
  ...
  kp_kalman_core_sparse_GPU kalman_GPU(*D_Mo_s, *N_Act_s, *PROJ_s, isZonal,
                                       cublasHandle, cusparseHandle);
  // kp_kalman_core_full_GPU kalman_GPU(*D_Mo_f, *N_Act_f, *PROJ_f, isZonal,
  // cublasHandle);
  ...
  CPC_kp1.zeros();
  kp_vector<KFPP> X_kskm1(NB_N);
  X_kskm1.zeros();
  kp_vector<KFPP> X_kp1sk(NB_N);
  X_kp1sk.zeros();
  ...
  while (mf_phasetur.set_next_phase(CPT_km1) &&
    ...
    /*if (comparaison_matlab)
    {
            ...
            if(fichier2)
            {
                    fichier2 << boucle << " " <<  __SP U_k.d[212] << " " << __SP
    CPR_km1.d[boucle] \
                            << " " << __SP CPC_k.d[boucle] << " " << __SP
    Y_kskm1.d[212] << " " <<\
                            __SP X_kskm1.d[212] << " "<< __SP
    Var_PhRes.d[boucle]<< endl; fichier2.close();
            }
    }*/
  ...
  gsl_rng_free(rng);
  // cout<<"ratio="<<kalman_GPU.temps_init_1boucle.rez()<<"
  // "<<kalman_GPU.temps_1boucle.rez()<<"
  // "<<kalman_GPU.temps_init_1boucle.rez()/kalman_GPU.temps_1boucle.rez()<<endl;
  if (comparaison_matlab) Mat_Close(mat);

  cout << "---FIN EXECUTION KALMAN---" << endl;

  cout << "temps kalman core : " << kalman_GPU.temps_boucle.rez() << " s."
       << endl;
  if (afficher_temps) {
    temps_boucle.pause();
    temps_total.pause();
