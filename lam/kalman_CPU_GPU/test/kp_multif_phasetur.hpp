...
template<typename T> 
bool kp_multif_phasetur<T>::set_next_phase(kp_vector<T>& CPT_km1)
{
   if (boucle_local >= PHAS_TUR.getDim2())
     {
	if (!read_next())
	  return false;
     }
...
template<typename T> 
void kp_multif_phasetur<T>::init(const vector<string>& files_)
{
   files     = files_;
   if (files.size() == 0)
     {
...
//                                                                                            
template<typename T> 
bool kp_multif_phasetur<T>::read_next()
{
