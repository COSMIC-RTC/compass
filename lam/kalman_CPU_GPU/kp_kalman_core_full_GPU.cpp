...
kp_kalman_core_full_GPU::kp_kalman_core_full_GPU(const kp_matrix<KFPP>& D_Mo_,
    ...
    : kp_kalman_core_full(D_Mo_, N_Act_, PROJ_, isZonal_),
...
void kp_kalman_core_full_GPU::calculate_gain(float bruit_pix, float k_W,
  ...
  if (cu_atur_.size() != nb_az) {
    cerr << "Error | kp_kalman_core_full_GPU::kp_kalman_core_full_GPU | size "
            "problem atur"
         << endl;
    exit(EXIT_FAILURE);
  }
  ...
  if ((expression == 1) && (ordreAR == 1)) {
    ...
    kernel_diag_mult3(cu_Atur_Tkp1sk.getData(), cu_Sinf_0_0.getData(),
                      cu_T_kp1sk.getData(), cu_atur_.getData(), cu_atur_.size(),
    ...
    kernel_diag_mult(cu_Sinf_1_0.getData(), cu_T_kp1sk.getData(),
    ...
    cu_Sinf_zerosDmot.gemm(cublasHandle, 'N', 'T', 1, cu_S_inf, cu_zeros_Dmo,
                           0);
    ...
    // (D_Mo)T] )
    cu_inv_Sigmatot.gemm(cublasHandle, 'N', 'N', 1, cu_zeros_Dmo,
                         cu_Sinf_zerosDmot, 0);
    ...
    if (ordreAR == 1)
      cu_Tkp1sk_C1t.resize(nb_az, nb_p);
    else
      cu_Tkp1sk_C1t.resize(nb_n, nb_p);
...
void kp_kalman_core_full_GPU::next_step(const kp_vector<KFPP>& Y_k,
                                        kp_vector<KFPP>& U_k) {
  ...
  if (!gainComputed) {
    cerr << "Error | kp_kalman_core_full_GPU::next_step | gain has not been "
            "initialized"
         << endl;
    exit(EXIT_FAILURE);
  }
