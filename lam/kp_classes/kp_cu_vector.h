...
class kp_cu_vector
{
	...
	template<typename T> friend class kp_cu_vector;

#ifndef KP_WITH_CARMA
 public:
	kp_cu_vector(int s_);

	template <typename T> kp_cu_vector(const kp_vector<T>&);
...
// each method exists twice : one if KP_WITH_CARMA is not defined ; another one if KP_WITH_CARMA is defined
	kp_cu_vector();
	template<typename T>  kp_cu_vector(const kp_cu_vector<T>&);
	kp_cu_vector(const kp_cu_vector<real>&);
	template<typename T>  void operator=(const kp_cu_vector<T>& v);
	void operator=(const kp_cu_vector<real>& v);
	template<typename T>  void operator=(const kp_vector<T>& v);
	...
	// if KP_WITH_CARMA is defined, handle (1st argument) is not used. This is the handle from carma_object which is used
	void gemv(cublasHandle_t handle, char op_A, real alpha, const kp_cu_matrix<real>& A, const kp_cu_vector<real>& x, real beta);
...
// same methods (do not depend on whether KP_WITH_CARMA is defined or not)	
	...
	void operator/=(real val);
   	...
   	void set_from_subvector(const kp_cu_vector<real>& subv, const vector<int>& idx);
