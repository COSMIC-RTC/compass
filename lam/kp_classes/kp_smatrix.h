...
// we save matrix in coordinate format from MKL (see Sparse Matrix Storage
// Formats in MKL) we use one-based indexing (because one-based indexing would
// need row first packing for dens arrays in kp_gemm)
...
class kp_smatrix {
  template <typename T>
  friend class kp_smatrix;

 public:
  kp_smatrix();

  template <typename T>
  kp_smatrix(const kp_smatrix<T>& sm);
  ...
  template <typename T>
  void operator=(const kp_smatrix<T>& M);
...
// y := alpha*A*x + beta * y
template <typename real>
void kp_gemv(real alpha, kp_smatrix<real>& A, kp_vector<real>& x, real beta,
             kp_vector<real>& y);
template <typename real>
inline void kp_gemv(int alpha, kp_smatrix<real>& A, kp_vector<real>& x,
                    int beta, kp_vector<real>& y) {
...
template <typename real>
void kp_gemm(char op_A, real alpha, kp_smatrix<real>& A, kp_matrix<real>& B,
             real beta, kp_matrix<real>& C);
template <typename real>
inline void kp_gemm(char op_A, int alpha, kp_smatrix<real>& A,
                    kp_matrix<real>& B, int beta, kp_matrix<real>& C) {
...
// C := alpha*A*B + beta * y
template <typename real>
inline void kp_gemm(real alpha, kp_smatrix<real>& A, kp_matrix<real>& B,
                    real beta, kp_matrix<real>& C) {
  kp_gemm('N', alpha, A, B, beta, C);
}

template <typename real>
void kp_check_op_set_dim(int op, const kp_smatrix<real>& M, int& dim1,
                         int& dim2);

template <typename real>
void kp_gemv_core(char* transa, MKL_INT* m, MKL_INT* k, real* alpha,
                  char* matdescra, real* A_values, MKL_INT* A_rowind,
                  MKL_INT* A_colind, MKL_INT* A_nnz, real* x_data, real* beta,
                  real* y_data);

template <typename real>
void kp_gemm_core(char* transa, MKL_INT* m, MKL_INT* n, MKL_INT* k, real* alpha,
                  char* matdescra, real* A_values, MKL_INT* A_rowind,
                  MKL_INT* A_colind, MKL_INT* A_nnz, real* B_data, MKL_INT* ldb,
                  real* beta, real* C_data, MKL_INT* ldc);
