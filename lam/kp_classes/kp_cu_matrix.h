...
class kp_cu_matrix {
  template <typename T>
  friend class kp_cu_matrix;
#ifndef KP_WITH_CARMA
 public:
  kp_cu_matrix(int dim1_, int dim2_);
  template <typename T>
  kp_cu_matrix(const kp_matrix<T>& m);
 ...
 public:
  ...
  kp_cu_matrix();
  template <typename T>
  kp_cu_matrix(const kp_cu_matrix<T>& m);
  kp_cu_matrix(const kp_cu_matrix<real>& m);
  void resize(int dim1_, int dim2_);
  template <typename T>
  void operator=(const kp_cu_matrix<T>& m);
  ...
  // cublasHandle from carma_object which is used
  void gemm(cublasHandle_t handle, char op_A, char op_B, real alpha,
  ...
  void init_from_matrix(const kp_matrix<real>& M, int r1, int r2, int c1,
                        int c2);
  void init_from_matrix(const kp_cu_matrix<real>& M, int r1, int r2, int c1,
                        int c2);
...
template <typename real>
void kp_cu_getrf_core(int& m, int& n, real* dA, int& ldda, int* ipiv,
                      int* info);
...
template <typename real>
int kp_cu_get_getri_nb_core(int& dim);

template <typename real>
void kp_cu_vertcat(kp_cu_matrix<real>& rez, const vector<kp_matrix<real>*>& ms);

template <typename real>
void kp_cu_vertcat(kp_cu_matrix<real>& rez,
                   const vector<kp_cu_matrix<real>*>& ms);

template <typename real>
void kp_cu_horizcat(kp_cu_matrix<real>& rez,
                    const vector<kp_matrix<real>*>& ms);
