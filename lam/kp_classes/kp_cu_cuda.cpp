...
inline void __cudaSafeCall(cudaError err, const char *file, const int line) {
  if (cudaSuccess != err) {
    cerr<<"("<<file<<":"<<line<<") : cudaSafeCall() Runtime API error : "<<cudaGetErrorString(err)<<"."<<endl;
    exit(-1);
  }
}*/
...
{
        //cutilSafeCall(cudaMemcpy(ptr_src, ptr_dest, nb_octets, direction));
...
{
	//cutilSafeCall(cudaMemset(ptr_dev, val, nb_octets));
...
{
	//cutilSafeCall(cudaMalloc(ptr, nb_octets));
...
{
	//cutilSafeCall(cudaFree(ptr_dev));
