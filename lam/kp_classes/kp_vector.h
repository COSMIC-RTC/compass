...
class kp_vector {
 ...
 public:
  ...
  kp_vector(const kp_vector<real>& v, int i1, int i2) {
    _create(0);
    init_from_vector(v, i1, i2);
  };
 ...
 public:
  ...
  const real& operator[](int i) const {
    return *(carma_host_object->getData(i));
  };
 ...
 public:
  ...
  template <typename T>
  kp_vector(const kp_vector<T>&);
  kp_vector(const kp_vector<real>&);
  void resize(int s_);
  template <typename T>
  void operator=(const kp_vector<T>&);
