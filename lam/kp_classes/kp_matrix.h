...
class kp_matrix {
 ...
 public:
  ...
  real& operator()(int i, int j) {
    return *(carma_host_object.getData(j * dim1 + i));
  }
 ...
 public:
  ...
  // calculate trace
  double trace();
  void init_from_matrix(const kp_matrix<real>& M, int r1, int r2, int c1,
                        int c2);
  ...
  void set_from_rowsubmatrix(const kp_matrix<real>& subM,
...
template <typename real>
void kp_gemv(char op_A, real alpha, const kp_matrix<real>& A,
             const kp_vector<real>& x, real beta, kp_vector<real>& y);

template <typename real>
inline void kp_gemv(char op_A, int alpha, const kp_matrix<real>& A,
                    const kp_vector<real>& x, int beta, kp_vector<real>& y) {
  kp_gemv(op_A, (real)alpha, A, x, (real)beta, y);
}

// y := alpha*A*x + beta * y
template <typename real>
inline void kp_gemv(real alpha, const kp_matrix<real>& A,
                    const kp_vector<real>& x, real beta, kp_vector<real>& y) {
...
// C := alpha*A*B + beta * y
template <typename real>
inline void kp_gemm(real alpha, const kp_matrix<real>& A,
                    const kp_matrix<real>& B, real beta, kp_matrix<real>& C) {
...
template <typename real>
void kp_check_op_set_dim(int op, const kp_matrix<real>& M, int& dim1,
                         int& dim2);
...
template <typename real>
void kp_getrf_core(int* dim1, int* dim2, real* data, int* ipiv, int* info);
template <typename real>
void kp_getri_core(int* dim1, real* data, int* ipiv, real* wkopt, int* lwork,
                   int* info);
template <typename real>
void kp_gemv_core(char* op_A, const int* A_dim1, const int* A_dim2, real* alpha,
                  real* A_data, real* x_data, int* one, real* beta,
                  real* y_data);

template <typename real>
void kp_gemm_core(char* op_A, char* op_B, int* opA_dim1, const int* C_dim2,
                  int* opA_dim2, real* alpha, real* A_data, const int* A_dim1,
                  real* B_data, const int* B_dim1, real* beta, real* C_data,
                  const int* C_dim1);

template <typename real>
void kp_syevd_core(char* jobz, char* uplo, int* n, real* Q_data, real* DQ_data,
                   real* work, int* lwork, int* iwork, int* liwork, int* info);
